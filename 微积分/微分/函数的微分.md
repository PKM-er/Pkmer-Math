...
---
## 定义
设函数 $y=f(x)$ 在点 $x_0$​ 的某个邻域内有定义。函数从 $x_0$ 到 $x_0+\Delta x$ 的增量 $\Delta y=f(x_0+\Delta x)-f(x_0)$. 如果存在一个只与 $x_0$ 有关、与 $\Delta x$ 无关的常数 $A$ ，使得：
$$\Delta y=A\Delta x+o(\Delta x)$$
那么称函数 $y=f(x)$ 在点 $x_0$ 是**可微**的. 而 $A\Delta x$ 叫做函数 $y=f(x)$ 在点 $x_0$ 相应于自变量增量 $\Delta x$ 的**微分**, 记作 $dy$, 即
$$dy=A\Delta x$$
通常把自变量的增量 $\Delta x$ 记作 $dx$，于是微分常写作：
$$
dy=Adx
$$
...
于是由[[无穷小的比较]]可知，这时有 $\Delta y=dy+o(dy)$, 即 $dy$ 是 $\Delta y$ 的[[无穷#主部]]. 
...
## 推论
### 几何意义
微分 $dy=f'(x_0)\Delta x$ 是曲线在点 $x_0$ 处切线的纵坐标增量
### 微分形式不变性
