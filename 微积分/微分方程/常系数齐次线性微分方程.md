...
$$(r^{2}+pr+q)e^{ rx }=0$$
由于 $e^{ rx }\neq 0$, $r^{2}+pr+q=0$
由此可见, 只要 $r$ 满足这个代数方程, 这个指数函数就是微分方程的解.
这个代数方程称为微分方程的特征方程
...
记 $\alpha=-\dfrac{p}{2}$，$\beta=\dfrac{\sqrt{p^{2}-4q}}{2}$，则 $r_1=\alpha+\beta$，$r_2=\alpha-\beta$。利用[[双曲正弦]]和[[双曲余弦]]的定义：
$$\cosh x=\frac{e^x+e^{-x}}{2}, \quad \sinh x=\frac{e^x-e^{-x}}{2}$$
通解可改写为：
$$y=e^{\alpha x}\left(C_1\cosh(\beta x)+C_2\sinh(\beta x)\right)$$
### 2. $\Delta = 0$
方程有两个相等的实数根
$$r_{1}=r_{2}=-\dfrac{p}{2}$$
