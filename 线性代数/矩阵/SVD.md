...
设 $A$ 是一个 $m \times n$ 的矩阵，若记 $r=\operatorname{rank}(A)$，则其紧致奇异值分解可写为：
...
- $U$ 是一个 $m \times r$ 的列正交矩阵，包含 $A$ 的左奇异向量。
- $\Sigma$ 是一个 $r \times r$ 的对角矩阵，其对角元素是 $A$ 的正奇异值，且按从大到小排列。
- $V$ 是一个 $n \times r$ 的列正交矩阵，包含 $A$ 的右奇异向量。
