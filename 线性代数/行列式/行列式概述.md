...
author:
  ...
  - Zacharia2
---
行列式是一个数，它与一个[[矩阵概述|方阵]]相关联，用于刻画方阵的可逆性（$\det A \neq 0 \iff A$ 可逆）等性质，也是[[克拉默法则]]求解线性方程组的基础。

给定一个 $n \times n$ 的方阵 $A$，行列式可表示为 $\det(A)$ 或 $|A|$。行列式的定义可以通过递归地将方阵拆分为更小的子方阵（[[拉普拉斯展开法]]）来得到，也可由[[全排列]]直接展开定义。
...
\begin{vmatrix}
 ...
 \end{vmatrix}
 $$
- [[拉普拉斯展开法]]降阶（沿任意一行或一列展开均可，选 0 多的行/列更省力）
$$
\begin{vmatrix}
 x_{11} & x_{12} & x_{13} \\ 
 x_{21} & x_{22}  & x_{23} \\
...
$|\boldsymbol{A}|$ 除了按行递归展开，也可由[[全排列]]直接定义：每一项 $a_{1p_1}a_{2p_2}\cdots a_{np_n}$ 从每一行、每一列恰好取一个元素，符号 $(-1)^t$ 由该全排列的[[逆序数]]决定。
...
$p_{1} p_{2} \dots p_{n}$ 是数字$1 \dots n$ 的一个[[全排列]]。
...
幂指数的值被称为[[逆序数]]。它等于把一个全排列通过相邻[[对换]]变成顺序排列所需的交换次数。

下面以全排列 $2\:1\:4\:5\:3$ 为例说明逆序数。若前面的数大于后面的数, 就构成一对逆序。把这个排列通过相邻对换化为顺序排列所需的总交换次数, 就是它的逆序数。
