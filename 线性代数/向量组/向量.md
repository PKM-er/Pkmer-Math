...
$$\vec v=\begin{bmatrix}-1\\3\end{bmatrix},\vec w=\begin{bmatrix}2\\2\end{bmatrix}$$
$$\vec v-\vec w=\begin{bmatrix}-1-2\\3-2\end{bmatrix}=\begin{bmatrix}-3\\1\end{bmatrix}$$
把向量 $\vec v$ 和 $\vec w$ 看作两个点，$\vec v-\vec w$就是 $\vec v$  相对于 $\vec w$ 的位置。如果把 $\vec v$ 和 $\vec w$ 看作从原点开始的箭头。$\vec v-\vec w$ 是从 $\vec w$ 头部到 $\vec v$ 头部的箭头。

```tikz
\begin{document}
\begin{tikzpicture}[scale=1.2]
    % Grid
    \draw[very thin,gray!30] (-4.5,-0.5) grid (3.5,5.5);
    % Axes
    \draw[->] (-4.5,0) -- (3.5,0) node[right] {$x$};
    \draw[->] (0,-0.5) -- (0,5.5) node[above] {$y$};
    
    % Vector v from origin
    \draw[->,very thick,blue] (0,0) -- (-1,3) node[midway,above left] {$\vec{v} = \begin{bmatrix}-1\\3\end{bmatrix}$};
    % Vector w from origin
    \draw[->,very thick,red] (0,0) -- (2,2) node[midway,below right] {$\vec{w} = \begin{bmatrix}2\\2\end{bmatrix}$};
    % Vector v-w from w tip to v tip
    \draw[->,very thick,teal] (2,2) -- (-1,3) node[midway,above] {$\vec{v}-\vec{w}$};
    
    % Dashed construction lines
    \draw[dashed,gray] (-1,3) -- (-1,0) node[below] {$-1$};
    \draw[dashed,gray] (2,2) -- (2,0) node[below] {$2$};
...
向量减法$\vec v-\vec w$，相当于从向量 $\vec w$ 开始移动到向量 $\vec v$ 经历的步数和方向。

$\vec v-\vec w=\begin{bmatrix}-3\\1\end{bmatrix}$表示从 $\vec w$ 点开始，需要向左走3个单位、再向上走1个单位，才能到达 $\vec v$ 点。这个向量有时被称为从 $\vec w$ 到 $\vec v$ 的位移（displacement）。