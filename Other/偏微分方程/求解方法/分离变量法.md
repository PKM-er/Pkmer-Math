...
分离变量法是一种强有力的求解偏微分方程的方法，特别适用于线性齐次偏微分方程。在应用时，必须保证方程及其边界条件是可分离的，并利用初始条件和边界条件确定解的具体形式。通过分离变量法，可以将复杂的偏微分方程分解为一组常微分方程，从而大大简化求解过程。分离所得的空间特征值问题由[[Sturm-Liouville 理论]]刻画，其正交完备的特征函数系保证了级数解的合法性。