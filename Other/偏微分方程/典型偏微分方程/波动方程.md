...
然后，考虑非齐次项的特解。由于 $\sin(\pi t)\sin(\pi x)$ 本身满足齐次方程 $u_{tt}=u_{xx}$（**共振**情形），同形式的试解 $u_p=A\sin(\pi t)\sin(\pi x)$ 代入后 $u_{ptt}-u_{pxx}=0$，无法匹配右端。故试解须带因子 $t$：
$$
u_p(x,t) = t\big(A\cos(\pi t) + B\sin(\pi t)\big)\sin(\pi x)
$$
代入原方程 $u_{tt}-u_{xx}=\sin(\pi t)\sin(\pi x)$：
$$
\frac{\partial^2 u_p}{\partial t^2} = \Big(2\pi\big(-A\sin(\pi t)+B\cos(\pi t)\big)-t\pi^2\big(A\cos(\pi t)+B\sin(\pi t)\big)\Big)\sin(\pi x)
$$
$$
\frac{\partial^2 u_p}{\partial x^2} = -t\pi^2\big(A\cos(\pi t)+B\sin(\pi t)\big)\sin(\pi x)
$$
两式相减，含 $t$ 的项消去，得：
$$
2\pi\big(-A\sin(\pi t)+B\cos(\pi t)\big)\sin(\pi x) = \sin(\pi t)\sin(\pi x)
$$
比较 $\sin(\pi t)$ 与 $\cos(\pi t)$ 的系数：$-2\pi A=1$、$2\pi B=0$，故 $A=-\dfrac{1}{2\pi}$，$B=0$。

因此，特解为：
$$
u_p(x,t) = -\frac{t}{2\pi}\cos(\pi t)\sin(\pi x)
$$

最终解为齐次解和特解的叠加：
$$
u(x,t) = u_h(x,t) - \frac{t}{2\pi}\cos(\pi t)\sin(\pi x)
$$
