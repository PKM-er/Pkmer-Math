...
---
# 問2
$X$ と $Y$ を，$\{0, 1\}$ に値をとる確率変数とする．パラメータ $\alpha, \beta, \gamma \in [0, 1]$ に対して，確率を次のように定める：
- $P(X = 0) = \alpha$
- $P(X = 1) = 1 - \alpha$
- $P(Y = 0 | X = 0) = \beta$
- $P(Y = 1 | X = 0) = 1 - \beta$
...
$$
## 問い
1. 2進エントロピー関数を用いて条件エントロピー $H(Y \mid X)$ を表せ．
2. $\beta = 1 - \gamma$ のとき，相互情報量 $I(X; Y)$ を最大にする $\alpha$ の値を求めよ．また，その最大値を 2進エントロピー関数および $\alpha, \beta$ を用いて表せ．
3. $\alpha, \beta$ を固定する（$0 < \alpha < 1$）．このとき，相互情報量 $I(X; Y)$ を最小にする $\gamma$ の値と，その最小値を求めよ．
4. $\alpha, \beta$ を固定する（$0 < \alpha < 1, \beta > \frac{1}{2}$）．このとき，相互情報量 $I(X; Y)$ を最大にする $\gamma$ の値を求めよ．
---
# GPT-4o
### 解答
