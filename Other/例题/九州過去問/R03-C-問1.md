...
图 1：满足 $\delta(q_0, a) = q_0, \delta(q_0, b) = q_1$ 的四个自动机 $M$。
...
L(N_2)
=\{\epsilon\}\;\cup\;
\bigl\{\,w \in \{a,b\}^*:\text{the first symbol of $w$ (if any) sends $q_0\to q_1$, and then each $b$ in $q_1$ toggles you back to $q_0$, etc.}\bigr\}.
$$
