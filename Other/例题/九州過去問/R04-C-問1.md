...
入力記号 $a=0$ のとき，
$$
  	ilde{\delta}((q, q'), 0) = (\delta(q, 0), \delta(q', 0)).
$$
入力記号 $a=1$ のとき，
$$
  	ilde{\delta}((q, q'), 1) = (\delta(q, 0), \delta(\delta(q', 0), 0)).
$$
...
2. $n (\ge 1)$ が $5$ の倍数のとき、入力文字列 $1^n$ に対するオートマトン $\tilde{M}$ の遷移先状態が $(q_5, q_5)$ となることを示せ。
3. 長さ $2$ 以上の入力文字列 $w \in \{0, 1\}^*$ に対するオートマトン $\tilde{M}$ の遷移先状態を $(q_{iw}, q_{jw})$ とする。$iw, jw \in \{2, 3, 4, 5, 6\}$ を、$\#_0(w), \#_1(w)$ を用いた式で表せ。ただし、$\#_a(w)$ は文字列 $w$ に現れる文字 $a$ の個数を表す。
...
$$\tilde{\delta}((q, q'), a) =
\begin{cases}
(\delta(q, 0), \delta(q', 0)) & a = 0 \text{case}, \\
(\delta(q, 0), \delta(q', 0)) & a = 1 \text{case}
\end{cases}$$
...
2. 证明当 $n (\ge 1)$ 是 $5$ 的倍数时，输入字符串 $1^n$ 在自动机 $\tilde{M}$ 上的最终状态为 $(q_5, q_5)$。
3. 对于长度 $\ge 2$ 的字符串 $w \in \{0, 1\}^*$，自动机 $\tilde{M}$ 的最终状态记为 $(q_{iw}, q_{jw})$，其中 $iw, jw \in \{2, 3, 4, 5, 6\}$。请用字符串 $w$ 中 $0$ 和 $1$ 的个数（即 $\#_0(w)$ 和 $\#_1(w)$）表示 $iw$ 和 $jw$。
...
与えられた規則
- $\texttt{0}$ を読む: $\bigl(\delta(q,0),\,\delta(q',0)\bigr)$  
  - すなわち「両座標とも1ステップ進む」
- $\texttt{1}$ を読む: $\bigl(\delta(q,0),\,\delta(\delta(q',0),0)\bigr)$  
...
- **問2:**  
  ...
  $$
    \text{note}n\text{note},\quad
    \text{note}2n\text{note}
  $$
