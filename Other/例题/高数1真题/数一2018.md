...
- 问题: 比较 $M, N, K$ 的大小，转化为仅比较被积函数的大小
	- 第一步：比较 $M$与1的大小
		- 化简过程: 
			- 根据对称区间联想到奇偶性：<br>$\displaystyle M = \int_{-\frac{\pi}{2}}^{\frac{\pi}{2}} \frac{(1+x)^2}{1+x^2} \mathrm{d} x\xlongequal[]{\frac{1+x^2+2 x}{1+x^2}}\int_{-\frac{2}{2}}^{\frac{\pi}{2}} 1 d x+\int_{-\frac{2}{2}}^{\frac{2}{2}} \frac{2 x}{1+x^2} d x\xlongequal[]{x\text{note}=0}\int_{-\frac{\pi}{2}}^{\frac{\pi}{2}} 1 d x$
	...
	- 第三步：比较N与 $1$ 的大小
		...
		- 故该函数是凹函数，$f(x)>f(0)=0$
			- 则$f(x)=\mathrm{e}^x-x-1>0\xrightarrow[]{\text{rearrange}}\frac{\sqrt{1+x}}{e^x}<1$
		- 结果: $N < M$
...
- 分析概率密度 $f(x)$ 的性质：
	...
	- 由对称性可知，$P\{X < 1\} \xlongequal[]{\text{symmetry}} 0.5$。
- 计算给定区间内的概率：
	- 已知 $\displaystyle \int_{0}^{2} f(x) \, \mathrm{d}x = 0.6$，表示 $P\{0 < X < 2\} = 0.6$。
	- 根据对称性，$P\{0 < X < 1\} \xlongequal[]{\text{symmetry}} \frac{1}{2} P\{0 < X < 2\} = 0.3$。
- 合并后计算 $P\{X =1\}\xlongequal[]{\text{note}} P\{X < 1\} - P\{0 < X < 1\}$
	- $\xlongequal[]{\text{note}} 0.5 - 0.3 = 0.2$
### (8)
...
- 把指数单独拿出来算
	...
	- $\displaystyle \xlongequal[]{ln(1+x-1)\text{\sim}x-1}\lim_{x \rightarrow 0} \frac{1}{\sin kx}\left(\frac{1-\tan x}{1+\tan x}-1\right)$
...
- 使用积分的分部积分法计算 $\displaystyle \int_{0}^{1} x f^{\prime \prime}(x) \mathrm{d} x$
	...
	- $\displaystyle \int_0^1 x f^{\prime \prime}(x) \mathrm{d} x \xlongequal[]{\text{note}} \left.x f^{\prime}(x)\right|_0^1 - \int_0^1 f^{\prime}(x) \mathrm{d}x$
...
- 于是$\displaystyle  \operatorname{rot} F(1,1,0)  =\left.\left|\begin{array}{ccc}i  j  k \\\frac{\partial}{\partial x}  \frac{\partial}{\partial y}  \frac{\partial}{\partial z} \\x y  -y z  z x\end{array}\right|\right|_{(1,1,0)}$
	- $\displaystyle  \xlongequal[]{\text{note}}[0-(-y)] \vec{i}+(-1) \cdot(z-0) \vec{j} +(0-x) \vec{k}$
...
### (12)
 - 由曲线 $L$ 的方程可知,该曲线对变量 $x, y, z$ 具有轮换对称性.<br> 于是,$\displaystyle \oint_L x y \mathrm{~d} s=\oint_L y z \mathrm{~d} s=\oint_L z x \mathrm{~d} s=\frac{1}{3} \oint_L(x y+y z+z x) \mathrm{d} s .$
	- 并且 $x y+y z+z x\xlongequal[]{\text{perfect square}}\frac{1}{2}\left[(x+y+z)^2-\left(x^2+y^2+z^2\right)\right]$
...
\begin{tikzpicture}[tdplot_main_coords,scale=2.3]
  ...
  \fill[blue!8] (Zp) -- (Xp) -- (Yp) -- cycle;
  \fill[blue!8] (Zp) -- (Yp) -- (Xm) -- cycle;
  \fill[gray!10] (Zm) -- (Xp) -- (Yp) -- cycle;
  \draw[thick] (Zp) -- (Xp) -- (Yp) -- cycle;
  \draw[thick] (Zp) -- (Yp) -- (Xm) -- cycle;
  \draw[thick] (Zp) -- (Xm) -- (Ym) -- (Xp) -- cycle;
  \draw[thick] (Zm) -- (Xp) -- (Yp) -- cycle;
  \draw[thick] (Zm) -- (Yp) -- (Xm);
  \draw[dashed] (Zm) -- (Xm) -- (Ym) -- (Zm);
  \draw[dashed] (Yp) -- (Xm) (Ym) -- (Xp);
  \draw[->] (0,0,0) -- (1.35,0,0) node[below right] {$x$};
  \draw[->] (0,0,0) -- (0,1.35,0) node[below left] {$y$};
  \draw[->] (0,0,-1.25) -- (0,0,1.35) node[above] {$z$};
  \node[above left] at (Xp) {$1$};
  \node[right] at (Yp) {$1$};
  \node[left] at (Zp) {$1$};
  \node at (0.18,0.30,0.55) {$\Sigma$};
  \node[anchor=west] at (0.80,-0.15,-0.88) {$|x|+|y|+|z|=1$};
\end{tikzpicture}
\end{document}
```
...
- 又因为 $\displaystyle \Sigma$ 对变量 $x, y, z$ 具有轮换对称性, 所以
	- $\displaystyle I=\iint_{\Sigma}|y| \mathrm{d} S=\frac{1}{3} \iint_{\Sigma}(|x|+|y|+|z|) \mathrm{d} S=\frac{1}{3} \iint_{\Sigma} \mathrm{d} S=\frac{1}{3} \cdot \Sigma \text{note}$
...
- 计算 条件概率公式：$P(AC \mid AB \cup C) = \frac{P(AC(AB \cup C))}{P(AB \cup C)}$
	- 展开分子：$P(AC(AB \cup C)) = P(ABC \cup AC)\xlongequal[]{P(ABC) = 0}P(AC)\xlongequal[]{\text{note}}P(A)P(C) = \frac{1}{2}P(C)$
...
- 根据条件概率公式：$P(AC \mid AB \cup C) = \frac{P(AC \cap (AB \cup C))}{P(AB \cup C)}$
    - 展开分子：
      ...
      - $\xlongequal[]{A \text{note} C \text{note}} P(A)P(C) = \frac{1}{2}P(C)$
    - 展开分母：
...
分部积分准备: $\displaystyle \int u \, \mathrm{d}v = uv - \int v \, \mathrm{d}u$
	...
	- 选择$v = \, \mathrm{e}^{2x}$
- $\displaystyle \text{original}=\frac{1}{2} \int \arctan \sqrt{e^x-1} d\left(e^{2 x}\right)$
...
- 半椭球面 $\Sigma$ 与补面 $\Sigma'$ 的位置关系如下：

```tikz
\usepackage{tikz-3dplot}
\begin{document}
\tdplotsetmaincoords{80}{120}
\begin{tikzpicture}[tdplot_main_coords,scale=2.7]
  \def\R{0.577}
  \fill[gray!10] plot[domain=0:360,samples=100,variable=\t]
    (0,{\R*cos(\t)},{\R*sin(\t)}) -- cycle;
  \foreach \s in {-0.48,-0.32,...,0.48} {
    \draw[gray!75,thin]
      (0,{-sqrt(\R*\R-\s*\s)},\s) --
      (0,{sqrt(\R*\R-\s*\s)},\s);
  }
  \draw[thick] plot[domain=0:360,samples=100,variable=\t]
    (0,{\R*cos(\t)},{\R*sin(\t)});
  \foreach \p in {0,90,180,270} {
    \draw[blue!70,thin] plot[domain=0:90,samples=45,variable=\u]
      ({cos(\u)},{\R*sin(\u)*cos(\p)},{\R*sin(\u)*sin(\p)});
  }
  \draw[thick] plot[domain=0:90,samples=45,variable=\u]
    ({cos(\u)},{\R*sin(\u)},0);
  \draw[thick] plot[domain=0:90,samples=45,variable=\u]
    ({cos(\u)},{-\R*sin(\u)},0);
  \draw[thick] plot[domain=0:90,samples=45,variable=\u]
  ...
  \draw[thick] plot[domain=0:90,samples=45,variable=\u]
    ({cos(\u)},0,{-\R*sin(\u)});
  \draw[->] (-0.85,0,0) -- (1.35,0,0) node[below right] {$x$};
  \draw[->] (0,-0.85,0) -- (0,0.85,0) node[below left] {$y$};
  \draw[->] (0,0,-0.85) -- (0,0,0.9) node[above] {$z$};
  \draw[->,thick] (0,0,0) -- (-0.65,0,0) node[above left] {$\mathbf n$};
  \node[above right] at (0.76,-0.22,0.48) {$\Sigma$};
  \node[below left] at (0,0.38,-0.28) {$\Sigma'$};
  \node[above right] at (1,0,0) {$1$};
  \node[below left] at (0,0,0) {$O$};
  \node[anchor=west] at (0.88,0.40,-0.42) {$x^2+3y^2+3z^2=1$};
\end{tikzpicture}
\end{document}
```
- (解) 如图所示, 添加辅助平面 $\displaystyle \Sigma^{\prime}: x=0\xrightarrow[\text{note}x=0\text{note}]{x^2+3 y^2+3 z^2=1}\left(3 y^2+3 z^2 \leqslant 1\right)$, 取后侧, <br>即法向量指向 $x$ 轴负向, 则 $\displaystyle \Sigma$与 $\displaystyle \Sigma^{\prime}$ 围成一个半椭球体 $\Omega$, 且法向量指向外侧.
	- $\displaystyle \begin{aligned} & I=\iint_{\Sigma+\Sigma^{\prime}} x \mathrm{~d} y \mathrm{~d} z+\left(y^3+2\right) \mathrm{d} z \mathrm{~d} x+z^3 \mathrm{~d} x \mathrm{~d} y -\underbrace{\iint_{\Sigma^{\prime}} x \mathrm{~d} y \mathrm{~d} z+\left(y^3+2\right) \mathrm{d} z \mathrm{~d} x+z^3 \mathrm{~d} x \mathrm{~d} y .}_{=0\text{note}}\end{aligned}$
		- $\displaystyle \begin{aligned} =\iint_{\Sigma+\Sigma^{\prime}}^{\prime \prime} x \mathrm{~d} y \mathrm{~d} z+\left(y^3+2\right) \mathrm{d} z \mathrm{~d} x+z^3 \mathrm{~d} x \mathrm{~d} y\xlongequal[]{\text{note}}\iiint_{\Omega}^{\prime \prime}\left(1+3 y^2+3 z^2\right) \mathrm{d} v . \end{aligned}$
#### 柱线法
...
- $\displaystyle \begin{aligned} & \\\iiint_{\Omega}\left(1+3 y^2+3 z^2\right) \mathrm{d} v= & \int_0^1 \mathrm{~d} x \iint_{D_x}\left(1+3 y^2+3 z^2\right) \mathrm{d} y \mathrm{~d} z \end{aligned}$
	- $\displaystyle \begin{aligned}\xlongequal{\text{note}} \int_0^1 \mathrm{~d} x \int_0^{2 \pi} \mathrm{~d} \theta \int_0^{\sqrt{\frac{1-x^2}{3}}}\left(1+3 r^2\right) \cdot r \mathrm{~d} r\end{aligned}$
...
- $\displaystyle \begin{aligned}I =\iint_{\Sigma^{\prime}} z d y d x+\left(y^3+2\right) d x d z+x^3 d y d z \end{aligned}$
	- $\displaystyle \begin{aligned} \xlongequal[]{\text{note}}\iint_{\Sigma^{\prime}} x^3 d y d z+\left(y^3+2\right) d x d z+z d x d y \end{aligned}$
...
- 当 $f(x)=x$ 时,方程化为 $y^{\prime}+y=x$,是一阶线性微分方程
	...
	- $\displaystyle \begin{aligned} =C_1 \mathrm{e}^{-x}+x-1\left(C_1 \text{note}\right) . \end{aligned}$
## (19)
 (本题满分 10 分)
...
- 分析和证明过程：
	...
	- 观察到 $1 = e^0$，可以对原式运用拉格朗日中值定理，存在$\xi$属于$0 < \xi < x_1$：
	      $e^{x_2} = \frac{e^{x_1} - 1}{x_1} \xlongequal[]{1=e^{0}} \frac{e^{x_1} - e^0}{x_1-0} \xlongequal[]{\text{note}} \frac{e^\xi \cdot (x_1-0)}{x_1} \xlongequal[]{\text{note}_1} e^\xi$
	      其中 ,$x_2 = \xi$,这样，$0 < x_2 < x_1$。
- 假设 $0 < x_n < x_{n-1}$，接下来证明 $0 < x_{n+1} < x_n$：
      $e^{x_{n+1}} = \frac{e^{x_n} - 1}{x_n} \xlongequal[]{1=e^{0}} \frac{e^{x_n} - e^0}{x_n} \xlongequal[]{\text{note}} \frac{e^\eta \cdot (x_n-0)}{x_n} = e^\eta$
...
- (I) 对矩阵 $\boldsymbol{A}, \boldsymbol{B}$ 分别施以初等行变换, 得
	- $\displaystyle   \boldsymbol{A}=\left(\begin{array}{ccc}1  2  a \\1  3  0 \\2  7  -a\end{array}\right) \xrightarrow[\text{note}]{\text{note}0}\left(\begin{array}{ccc}1  0  3 a \\0  1  -a \\0  0  0\end{array}\right),$
	- $\displaystyle  \boldsymbol{B}=\left(\begin{array}{ccc}1  a  2 \\0  1  1 \\-1  1  1\end{array}\right) \xrightarrow[]{\text{note}0}\left(\begin{array}{ccc}1  0  0 \\0  1  1 \\0  0  2-a\end{array}\right) .$
...
- 故 $A X=B$ 的解为$\displaystyle \boldsymbol{X}=\left(\begin{array}{ccc}3-6 k_1 & 4-6 k_2 & 4-6 k_3 \\-1+2 k_1 & -1+2 k_2 & -1+2 k_3 \\k_1 & k_2 & k_3\end{array}\right) \text{note} k_1, k_2, k_3 \text{note}$
...
- 计算协方差
	- $\operatorname{Cov}(X, Z) = E(XZ) - E(X) \cdot E(Z)\xrightarrow[]{Z=XY}E(X \cdot X Y)-E X \cdot E(X Y)$
		- $\displaystyle \xlongequal[]{\text{note}}E\left(X^2\right) \cdot E(Y)-E^2(X) \cdot E(Y)$
			- 计算 $E(X)$ , $E(X^2)$,$E(Y)$
...
- 三件套(常规思路):
	...
	- $\xlongequal[\text{note}]{\text{note}}P\{X Y \leq z, X=1\}+P\{X Y \leqslant z, X=-1\}$
	- $\xlongequal[]{\text{note}}P\{X Y \leq z\} \cdot P\{X=1\}+P\{XY \leq z\} \cdot P\{X=-1\}$
...
- 写出似然函数为$\displaystyle L(\sigma)=\prod_{i=1}^n f\left(x_1 ; \sigma\right)=\left(\frac{1}{2}\right)^n \cdot \frac{1}{\sigma^n} \cdot e^{-\frac{1}{\sigma} \sum_{i=1}^n\left|x_i\right|}\text{,}-\infty<x_i<+\infty$
	...
	- 求导找可能的极值点(驻点)，得 $\displaystyle \frac{d \ln L(\sigma)}{d \sigma}=-\frac{n}{\sigma}+\frac{1}{\sigma^{2}} \sum_{i=1}^{n}\left|x_{i}\right|=0$,
		- 解得 $\displaystyle \sigma=\frac{1}{n} \sum_{i=1}^{n}\left|x_{i}\right|$, 所以 $\displaystyle \xrightarrow[]{\text{note}}\hat{\sigma}=\frac{1}{n} \sum_{i=1}^{n}\left|X_{i}\right|$
(II) 由于
