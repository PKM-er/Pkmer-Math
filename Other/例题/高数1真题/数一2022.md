...
- 方法一：$\displaystyle  D(2X - Y + 1) = 4DX + DY - 2Cov(2X, Y)$
	...
	- $\displaystyle  \xlongequal[D(Y)=\text{λ}]{D(X)=\frac{(a-b)^2}{12} } 4 \cdot \frac{3^2}{12} + 2 - 2 \cdot 2 \cdot (-1) = 9$
- 方法二：
	- 应用方差的性质  
...
- 公式 $D(X)=E\left(X^2\right)-[E(X)]^2$.
	- $\displaystyle  E\left(\frac{1}{n}\sum_{i=1}^n X_i^2\right) \xlongequal[]{\text{提系数}} \frac{1}{n} E\left(\sum_{i=1}^n X_i^2\right) \xlongequal[]{\text{独立同分布}}\frac{1}{n} \cdot n E\left(X_1^2\right) =\mu_2$
...
- $\displaystyle  \xlongequal[\text{将}x=0\text{代入}L_2\text{，将}y=0\text{代入}L_3]{\text{将}z=0\text{代入}L_1}\int_{L_1}\underbrace{(-\cos 0)}_{=-1} \mathrm{d} x+0+\int_{L_3}\underbrace{(-\cos z) \mathrm{d} x+x \sin z \mathrm{~d} z}_{(-x \cos z)=-\cos z+x \sin z}$
...
- 使用泰勒公式在 $x = \frac{a+b}{2}$ 处展开 $f(x)$。  
	- 展开式：$\displaystyle f(x) = f\left(\frac{a+b}{2}\right) + f^{\prime}\left(\frac{a+b}{2}\right)\left(x - \frac{a+b}{2}\right) + \frac{1}{2}f^{\prime \prime}(\xi)\left(x - \frac{a+b}{2}\right)^2$。<br> $\xi$ 介于 $x$ 与 $\frac{a+b}{2}$ 之间 
		...
		- 对不等式两端从 $a$ 到 $b$ 积分。  
			- 得到 $\displaystyle \int_a^b f(x) \mathrm{d}x \text{≥} f\left(\frac{a+b}{2}\right)(b-a) +f^{\prime}\left(\frac{a+b}{2}\right) \int_a^b\left(x-\frac{a+b}{2}\right) \mathrm{d} x$  
...
- $\displaystyle (14 E-A) x=0 \text{，得} 14 E-A=\left(\begin{array}{ccc}13 & -2 & -3 \\ -2 & 10 & -6 \\ -3 & -6 & 5\end{array}\right)$
	- $\displaystyle \xrightarrow [\text{抹去第一行}]{\text{秩为}2\text{，任意一行抹去}}\left(\begin{array}{ccc}1 & -5 & 3 \\ -3 & -6 & 5 \\ 0 & 0 & 0\end{array}\right)\xrightarrow{r_2+3 r_1}\left(\begin{array}{ccc}1 & -5 & 3 \\0 & -21 & 14 \\0 & 0 & 0\end{array}\right)$
		- 定 $x_3=3$, 代入2行  
...
- 当 $\lambda_2 = \lambda_3 = 0$时，  考虑 $(0 \boldsymbol{E}-\boldsymbol{A}) \boldsymbol{x}=\mathbf{0}$.  
	- $\displaystyle -\boldsymbol{(0-A)}=\left(\begin{array}{lll}-1  -2  -3 \\-2  -4  -6 \\-3  -6  -9\end{array}\right) \xlongequal[\text{抹去任意两行}]{\text{任意两行成比例，秩为一}}\left(\begin{array}{lll}1  2  3 \\0  0  0 \\0  0  0\end{array}\right)$  
...
- 下面计算 $D(\hat{\theta})$.
	...
	- $\displaystyle  \xlongequal[E(Y)=2 \theta \text{→} D(Y)=4 \theta^2]{E(X)=\theta \text{→} D(X)=\theta^2}\frac{1}{(m+n)^2}$·$n$·$\theta^2+\frac{1}{4(m+n)^2}$·$m \cdot 4 \theta^2=\frac{\theta^2}{m+n}$
