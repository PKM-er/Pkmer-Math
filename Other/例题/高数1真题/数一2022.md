...
数一2022  
 设 $f(u)$ 可导, $z=x y f\left(\frac{y}{x}\right)$, 若 $x \frac{\partial z}{\partial x}+y \frac{\partial z}{\partial y}=y^{2}(\ln y-\ln x)$, 则 ( )  
    ...
    (D) $f(1)=0, f^{\prime}(1)=1$.  
### (2)  
- 乘法求导:先一后二<br>![|200](https://obsidian-picture-1314838930.cos.ap-beijing.myqcloud.com/20241205131412.png)
- 计算偏导数：  
...
- 对 $\begin{aligned} (C) \end{aligned}$，$\begin{aligned} \left[\begin{array}{cc}A & B \\ O & B\end{array}\right] \xrightarrow[]{c1-c2} \left[\begin{array}{cc}A & O \\ O & B\end{array}\right], \quad \left[\begin{array}{cc}B & A \\ O & A\end{array}\right] \xrightarrow[]{c1-c2} \left[\begin{array}{cc}B & O \\ O & A\end{array}\right] \end{aligned}$
	- 因为 $\begin{aligned} Ax = 0 \end{aligned}$ 与 $\begin{aligned} Bx = 0 \end{aligned}$ 同解，<br>注意行变换是同解的，列变换是不同解的
		...
		- 实际上，$\begin{aligned} \left[\begin{array}{cc}A & AB \\ O & B\end{array}\right] x = 0 \end{aligned}$，和 $\begin{aligned} \left[\begin{array}{cc} B & BA \\ O & A\end{array}\right] x = 0 \end{aligned}$ 也同解
- D选项是列变换，错误
## (7)  
数一2022  
...
- 设随机变量 $X$ 的数学期望 $E(X)=\mu$, 方差 $D(X)=\sigma^2$, 则由切比雪夫不等式, 有 $P\{|X-\mu| \geq 3 \sigma\} \leq$ $\qquad$ (1989 年数学三试题)
...
- 求内部的驻点  
	...
	- 令 $f_x^{\prime}(x, y)=0$，$f_y^{\prime}(x, y)=0$ 得  
		- $\begin{aligned} 2 x-x^2-y^2=0 \\2 y-x^2-y^2=0 \end{aligned}$  
...
## (14)  
数一2022  
 已知级数 $\sum_{n=1}^{\infty} \frac{n !}{n^{n}} \mathrm{e}^{-n x}$ 的收敛域为 $(a,+\infty)$, 则 $a=$  
1. 思路分析:  
- 确定题目考查的是函数项级数收敛域的计算。  
- 确定级数为正项级数，考虑使用比值审敛法  
	- 若 $\lim _{n \rightarrow \infty} \frac{u_{n+1}}{u_n}=\rho\left\{\begin{array}{l}<1, \text { 级数 } \sum_{n=1}^{\infty} u_n \text { 收玫, } \\ >1, \text { 级数 } \sum_{n=1}^{\infty} u_n \text { 发散, } \\ =1, \text { 此判别法失效. }\end{array}\right.$  
  
- 第一步: 使用比值审敛法。  
- 定义 $u_n = \frac{n !}{n^n} e^{-nx}$，并求其极限。  
	- 计算 $\lim_{n \to \infty} \frac{u_{n+1}}{u_n}$。  
		- 求 $u_n$ 和 $u_{n+1}$ 表达式  
			- $u_n=\frac{n!}{n^n}\mathrm{e}^{-nx}$  
			- $u_{n+1}=\frac{(n+1)!}{{(n+1)}^{n+1}}\mathrm{e}^{-(n+1)x}$  
			- 对系数化简$\begin{aligned} \frac{(n+1)!}{\left(n+1\right)^{n+1}}&=\frac{\left(n+1\right)n!}{\left(n+1\right)^{n+1}}=\frac{n!}{\left(n+1\right)^{n}}. \end{aligned}$  
- 代入计算  
	- $\lim_{n \to \infty} \frac{u_{n+1}}{u_n}$=$\lim_{n\to\infty}\frac{\frac{n!}{(n+1)^n}}{\frac{n!}{n^n}}\cdot\frac{\mathrm{e}^{-(n+1)x}}{\mathrm{e}^{-nx}}$<br>=$\lim _{n \rightarrow \infty} \frac{n !}{(n+1)^n} \cdot \frac{n^n}{n !} \cdot \frac{\mathrm{e}^{-(n+1) x}}{\mathrm{e}^{-n x}}$<br>=$\lim_{n\to\infty}\frac{n^n}{\left(n+1\right)^n}\cdot\frac{\mathrm{e}^{-(n+1)x}}{\mathrm{e}^{-nx}}$=$\mathrm{e}^{-x}\lim_{n\to\infty}\frac{n^n}{\left(n+1\right)^n}$  
		- 进一步简化得 $e^{-x} \lim_{n \to \infty} \frac{1}{(1+\frac{1}{n})^n}$。  
		- 由$\lim_{n\to\infty}\left(1+\frac1n\right)^n=\mathrm{e}$最终得 $e^{-1-x}$。  
...
- 分析积分区域 $D$ 并画出图形。<br>![300](https://obsidian-picture-1314838930.cos.ap-beijing.myqcloud.com/20241030104812.png)  )
...
- ![300](https://obsidian-picture-1314838930.cos.ap-beijing.myqcloud.com/img_v3_02g1_8e919614-6d08-41f3-b875-a1c7e5b1b02g.jpg))
	- 如果不可偏导，则不能用斯托克斯
#### 不用斯托克斯
...
- 令$x_2, x_3$为自由元
	-  $\left(\begin{array}{l}x_2 \\ x_3\end{array}\right)=\left(\begin{array}{l}1 \\ 0\end{array}\right)$, 可得 $\boldsymbol{\xi}_2=\left(\begin{array}{c}-2 \\ 1 \\ 0\end{array}\right)$ 
	- $\begin{aligned} \binom{x_2}{x_3}=\binom{0}{1} \end{aligned}$，可得$\boldsymbol{\xi}_3=\left(\begin{array}{c}-3 \\ 0 \\ 1\end{array}\right)$  
...
- $\begin{aligned} & \boldsymbol{\beta}_1=\boldsymbol{\xi}_1, \end{aligned}$
- $\begin{aligned} \boldsymbol{\beta}_2=\boldsymbol{\xi}_2, \end{aligned}$
- $\begin{aligned} \boldsymbol{\beta}_3=\boldsymbol{\xi}_3-\frac{\left(\boldsymbol{\beta}_2, \boldsymbol{\xi}_3\right)}{\left\|\boldsymbol{\beta}_2\right\|^2} \boldsymbol{\beta}_2=\left(\begin{array}{c}-3 \\0 \\1\end{array}\right)-\frac{6}{5}\left(\begin{array}{c}-2 \\1 \\0\end{array}\right)=\left(\begin{array}{c}-\frac{3}{5} \\-\frac{6}{5} \\1\end{array}\right) . \end{aligned}$  
	- 去分母，正交向量长一点，短一点没有关系，因为之后还要单位化  
	- 求得 $\boldsymbol{\beta}_2 = (-2, 1, 0), \boldsymbol{\beta}_3 = (-3, -6, 5)$。  
- 单位化  
	- 将 $\boldsymbol{\beta}_1, \boldsymbol{\beta}_2, \boldsymbol{\beta}_3$ 单位化, 可得$\boldsymbol{\varepsilon}_1=\frac{\boldsymbol{\beta}_1}{\left\|\boldsymbol{\beta}_1\right\|}=\frac{1}{\sqrt{14}}\left(\begin{array}{l}1 \\2 \\3\end{array}\right), \quad \boldsymbol{\varepsilon}_2=\frac{\boldsymbol{\beta}_2}{\left\|\boldsymbol{\beta}_2\right\|}=\frac{1}{\sqrt{5}}\left(\begin{array}{c}-2 \\1 \\0\end{array}\right), \quad \boldsymbol{\varepsilon}_3=\frac{\boldsymbol{\beta}_3}{\left\|\boldsymbol{\beta}_3\right\|}=\frac{1}{\sqrt{70}}\left(\begin{array}{c}-3 \\-6 \\5\end{array}\right)$  
   - 并形成正交矩阵 $\boldsymbol{Q}$   
      - 令 $\boldsymbol{Q}=\left(\boldsymbol{\varepsilon}_1, \boldsymbol{\varepsilon}_2, \boldsymbol{\varepsilon}_3\right)$, 可得 $\boldsymbol{Q}^{-1} \boldsymbol{A} \boldsymbol{Q}=\boldsymbol{Q}^{\mathrm{T}} \boldsymbol{A} \boldsymbol{Q}=\left(\begin{array}{ccc}14 & 0 & 0 \\ 0 & 0 & 0 \\ 0 & 0 & 0\end{array}\right)$,  
...
- $f\left(x_1, x_2, x_3\right) \stackrel{\boldsymbol{x}=\boldsymbol{Q} \boldsymbol{y}}{=} 14 y_1^2=0$. 解出 : $y_1=0$.此时 $y_2, y_3$ 为任意常数.  
    - 因此, 所求的通解$x=Qy$为 $\left(\begin{array}{c}x_1 \\ x_2 \\ x_3\end{array}\right)=\boldsymbol{Q}\left(\begin{array}{c}0 \\ y_2 \\ y_3\end{array}\right)=\left(\boldsymbol{\varepsilon}_1, \boldsymbol{\varepsilon}_2, \boldsymbol{\varepsilon}_3\right)·\left(\begin{array}{c}0 \\ y_2 \\ y_3\end{array}\right)$  
       - $\begin{aligned} & =\left(\begin{array}{ccc}\frac{1}{\sqrt{14}} & -\frac{2}{\sqrt{5}} & -\frac{3}{\sqrt{70}} \\\frac{2}{\sqrt{14}} & \frac{1}{\sqrt{5}} & -\frac{6}{\sqrt{70}} \\\frac{3}{\sqrt{14}} & 0 & \frac{5}{\sqrt{70}}\end{array}\right)\left(\begin{array}{c}0 \\y_2 \\y_3\end{array}\right) \end{aligned}$
- $\begin{aligned} =\left(\begin{array}{c}-\frac{2}{\sqrt{5}} y_2-\frac{3}{\sqrt{70}} y_3 \\\frac{1}{\sqrt{5}} y_2-\frac{6}{\sqrt{70}} y_3 \\\frac{5}{\sqrt{70}} y_3\end{array}\right) \end{aligned}$
- $\begin{aligned} \begin{array}{c}\text { 取 } y_2=\sqrt{5} k_1 \\y_3=\sqrt{70} k_2 \end{array}，通解=\left(\begin{array}{c}-2 k_1-3 k_2 \\k_1-6 k_2 \\5 k_2\end{array}\right), k_1, k_2 \text { 为任意常数. } \end{aligned}$ 
...
- 指数分布
	...
	- 若 $X$ 服从参数为 $\lambda$ 的指数分布, 则 $E(X)=\frac{1}{\lambda}, D(X)=\frac{1}{\lambda^2}$.
		- 参数和均值互为倒数
- 写出概率密度
	- $X_1, X_2, \cdots, X_n$ ,均值 $E(X)=\theta$, $X$ 的概率密度函数为$f_X(x)= \begin{cases}\frac{1}{\theta} \mathrm{e}^{-\frac{x}{\theta}}, & x>0 \\ 0, & x \leqslant 0\end{cases}$
	- $Y_1, Y_2, \cdots, Y_m$ ,均值$E(Y)=2 \theta$ , $Y$ 的概率密度函数为$f_Y(y)= \begin{cases}\frac{1}{2 \theta} \mathrm{e}^{-\frac{y}{2 \theta}}, & y>0 \\ 0, & y \leqslant 0 .\end{cases}$
- 设 $x_1, x_2, \cdots, x_n, y_1, y_2, \cdots, y_m$ 是相应于样本 $X_1, X_2, \cdots, X_n, Y_1, Y_2, \cdots, Y_m$ 的一组样本值, <br>则似然函数为$\begin{aligned} L(\theta)= \begin{cases}\frac{1}{2^m \theta^{m+n}} \mathrm{e}^{-\frac{\sum_{i=1}^n x_i}{\theta}-\frac{\sum_{i=1}^m y_j}{2 \theta}}, & x_i>0, y_j>0, i=1,2, \cdots, n, j=1,2, \cdots, m, \\ 0, & \text { 其他. }\end{cases} \end{aligned}$
- 当 $x_i>0, y_j>0, i=1,2, \cdots, n, j=1,2, \cdots, m$ 时, 取对数得<br>$\ln L(\theta)=-m \ln 2-(m+n) \ln \theta-\frac{\sum_{i=1}^n x_i}{\theta}-\frac{\sum_{j=1}^m y_j}{2 \theta} .$
	- 令 $\frac{\mathrm{d}[\ln L(\theta)]}{\mathrm{d} \theta}=0$, <br>即 $\xrightarrow[]{\ln \theta=\frac{1}{\theta}}0-\frac{m+n}{\theta}+\frac{\sum_{i=1}^n x_i}{\theta^2}+\frac{\sum_{j=1}^m y_j}{2 \theta^2}=0$,
	- $\begin{aligned}  \xrightarrow[]{同乘2\theta^2}-2(m+n) \theta+2 \sum_{i=1}^n x_i+\sum_{j=1}^m y_j=0\end{aligned}$ 解得 $\xrightarrow[]{移项}\theta=\frac{2 \sum_{i=1}^n x_i+\sum_{j=1}^m y_j}{2(m+n)}$.
- 因此, $\theta$ 的最大似然估计量： $\xrightarrow[将y换成Y]{将x换成X}\hat{\theta}=\frac{2 \sum_{i=1}^n X_i+\sum_{j=1}^m Y_j}{2(m+n)}$.
- 下面计算 $D(\hat{\theta})$.
	- <br>$\begin{aligned} D(\hat{\theta}) \xlongequal[]{\hat{\theta}代入}D\left[\frac{2 \sum_{i=1}^n X_i+\sum_{j=1}^m Y_j}{2(m+n)}\right]\xlongequal[]{提出系数}\frac{1}{(m+n)^2} D\left(\sum_{i=1}^n X_i\right)+\frac{1}{4(m+n)^2} D\left(\sum_{j=1}^m Y_j\right) \end{aligned}$
	- $\begin{aligned} \xlongequal[]{提出∑}\frac{1}{(m+n)^2} \sum_{i=1}^n D\left(X_i\right)+\frac{1}{4(m+n)^2} \sum_{j=1}^m D\left(Y_j\right)\end{aligned}$
	- $\begin{aligned} \xlongequal[E(Y)=2 \theta → D(Y)=4 \theta^2]{E(X)=\theta → D(X)=\theta^2}\frac{1}{(m+n)^2}·n· \theta^2+\frac{1}{4(m+n)^2}·m \cdot 4 \theta^2=\frac{\theta^2}{m+n} \end{aligned}$
