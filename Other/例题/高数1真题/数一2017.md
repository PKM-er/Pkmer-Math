...
- 比较 $g(1)$ 和 $g(-1)$ 的大小
	- 由$1>-1$
		...
		- $\displaystyle \xrightarrow[]{g(x) = \frac{1}{2} f^2(x)}\frac{1}{2} f^2(1) > \frac{1}{2} f^2(-1)$
			- 进一步得到 $f^2(1) > f^2(-1)\xrightarrow[]{\text{note}}|f(1)| > |f(-1)|$
## (3)
...
## (7)
 设 $A, B$ 为随机事件. 若 $0<P(A)<1,0<P(B)<1$, 则 $\underbrace{P(A \mid B)>P(A \mid \bar{B})}_{\text{note}}$ 的充分必要条件 是 $(\quad)$
 (A) $\underbrace{P(B \mid A)>P(B \mid \bar{A})}_{\text{note}}$.
...
解 应选 (A).
解法 1 
- 题设条件 $P(A \mid B)>P(A \mid \bar{B})\xrightarrow[]{\text{note}}\frac{P(A B)}{P(B)}>\frac{P(A \bar{B})}{P(\bar{B})}\xlongequal[]{\text{note}}\frac{P(A)-P(A B)}{1-P(B)},$
	- 则$\displaystyle \frac{P(A B)}{P(B)}>\frac{P(A)-P(A B)}{1-P(B)}$
		- $P(A B)[1-P(B)]>P(B)[P(A)-P(A B)]$
			- $\displaystyle \xrightarrow[P(B) P(A B)]{\text{note}}P(A B)>P(A) P(B)$
...
- 对于A选项:$P(B \mid A)>P(B \mid \bar{A})$.
	...
	-  $\displaystyle  \xrightarrow[]{\text{note}}P(A B)[1-P(A)]  >P(A)[P(B)-P(A B)]$
	- 从而 $P(B A)>P(B) P(A)$,
- 则充要条件为 $P(B \mid A)>P(B \mid \bar{A})$. 答案应选 $(A)$
## (8)
 设 $X_{1}, X_{2}, \cdots, X_{n}(n \geqslant 2)$ 为来自$\underbrace{\text{note} N(\mu, 1) \text{note}}_{n\text{note}}$, 记 $\displaystyle \bar{X}=\frac{1}{n} \sum_{i=1}^{n} X_{i}$, 则下列结论中 ==不正确的==是 $(\quad)$
...
- $\displaystyle S(x)=\sum_{n=1}^{\infty}(-1)^{n-1} n x^{n-1}$
	- $\displaystyle \xrightarrow[]{\text{note}}\int_0^x s(t) d t=\int_0^x \sum_{n=1}^{\infty}(-1)^{n-1} n t^{n-1} d t$
		- $\displaystyle \xlongequal[]{\text{note}}  \sum_{n=1}^{\infty} \int_0^x(-1)^{n-1} n t^{n-1} d t$
		- $\displaystyle \xlongequal[]{\text{integrate}}  \sum_{n=1}^{\infty}(-1)^{n-1} x^n=\sum_{n=0}^{\infty}(-1)^n x^{n+1}$
		- $\displaystyle \xlongequal[]{\text{factor}x}  x \sum_{n=0}^{\infty}(-1)^n x^n\xlongequal[]{\text{note}}\frac{x}{1+x}$
...
### (13)
- 读题
	- $\displaystyle \underbrace{\text{note} \boldsymbol{A}=\left(\begin{array}{lll}1 & 0 & 1 \\ 1 & 1 & 2 \\ 0 & 1 & 1\end{array}\right),}_{\text{note}A\text{note}}$ ，$\displaystyle \underbrace{\begin{aligned}\boldsymbol{\alpha}_{1}, \boldsymbol{\alpha}_{2}, \boldsymbol{\alpha}_{3}\end{aligned}\text{note} 3 \text{note}}_{\text{note}}$,则向量组 $\displaystyle \underbrace{\begin{aligned}\boldsymbol{A} \boldsymbol{\alpha}_{1}, \boldsymbol{A} \boldsymbol{\alpha}_{2}, \boldsymbol{A} \boldsymbol{\alpha}_{3}\end{aligned}}_{\text{note}}$ 的秩 为
- 答 应填 2.
...
- 解:由于 $\boldsymbol{\alpha}_1, \boldsymbol{\alpha}_2, \boldsymbol{\alpha}_3$ 线性无关, 故矩阵 $\left(\boldsymbol{\alpha}_1, \boldsymbol{\alpha}_2, \boldsymbol{\alpha}_3\right)$ 可逆, 从而
	- $r\left(\boldsymbol{A \boldsymbol { \alpha } _ { 1 }}, \boldsymbol{A} \boldsymbol{\alpha}_2, \boldsymbol{A} \boldsymbol{\alpha}_3\right)=r\left(\boldsymbol{A}\underbrace{\left(\boldsymbol{\alpha}_1, \boldsymbol{\alpha}_2, \boldsymbol{\alpha}_3\right)}_{\text{note}}\right)=r(\boldsymbol{A}) .$
...
### (14)
- 由分布函数求得概率密度
	- 分布函数为 $F(x)=0.5 \Phi(x)+0.5 \Phi\left(\frac{x-4}{2}\right)\xrightarrow[\text{note}]{\text{note}}f(x)=F^{\prime}(x)=\frac{1}{2} \varphi(x)+\frac{1}{4} \varphi\left(\frac{x-4}{2}\right) .$
- 在求谁的期望:就在谁前面乘上概率密度然后积分
	- $\displaystyle E(X)=\int_{-\infty}^{+\infty} x f(x) \mathrm{d} x\xlongequal[]{\text{substitute}f(x)}\frac{1}{2} \underbrace{\int_{-\infty}^{+\infty} x \varphi(x) \mathrm{d} x}_{=0}+\frac{1}{4} \underbrace{\int_{-\infty}^{+\infty} x \varphi\left(\frac{x-4}{2}\right) \mathrm{d} x}_{\text{note}}$
		- $\displaystyle \frac{1}{4} \int_{-\infty}^{+\infty} x \varphi\left(\frac{x-4}{2}\right) \mathrm{d} \xrightarrow[]{\frac{1}{4}\text{note}\frac{1}{2}\times\frac{1}{2}}x\xlongequal[d\left(\frac{x-4}{2}\right)=\frac{1}{2} \mathrm{~d} x \text{.}]{\text{note}}\int_{-\infty}^{+\infty} \frac{x-4+4}{2} \varphi\left(\frac{x-4}{2}\right) \mathrm{d}\left(\frac{x-4}{2}\right)$
			- $\displaystyle \xlongequal[]{t\frac{x-4}{2}}\int_{-\infty}^{+\infty}(t+2) \varphi(t) \mathrm{d} t\xlongequal[]{\text{note}}\underbrace{\int_{-\infty}^{+\infty} t \varphi(t) \mathrm{d} t}_{=0}+2 \underbrace{\int_{-\infty}^{+\infty} \varphi(t) \mathrm{d} t}_{=1}=2$
	- 合并结果:$E(X)=0+2=2$
# 三、解答题
(本题共 9 小题,共 94 分,解答应写出文字说明、证明过程或演算步骤.)}
## (15)
 (本题满分 10 分)
设函数 $f(u, v)$ 具有 2 阶连续偏导数, $\underbrace{y=f\left(\mathrm{e}^{x}, \cos x\right), \text{note} \left.\frac{\mathrm{d} y}{\mathrm{~d} x}\right|_{x=0},\left.\frac{\mathrm{d}^{2} y}{\mathrm{~d} x^{2}}\right|_{x=0}}_{\text{in}x\text{note}}$
```
    y
   / \
  eˣ  cosx
...
- 计算函数 $y = f(\mathrm{e}^{x}, \cos x)$ 的一阶导数 $\frac{\mathrm{d} y}{\mathrm{d} x}$。
	- 使用复合函数求导法则得：$\frac{\mathrm{d} y}{\mathrm{d} x} = f_1^{\prime} \cdot \mathrm{e}^x+f_2^{\prime} \cdot(-\sin x)\xrightarrow[]{\text{set}x=0\text{note}}\left.\frac{\mathrm{d} y}{\mathrm{~d} x}\right|_{x=0}=f_1^{\prime}(1,1)$
	- 利用导数乘法求二阶偏导，得$\frac{\mathrm{d}^2 y}{\mathrm{~d} x^2}=\underbrace{\left[f_{11}^{\prime \prime} \cdot \mathrm{e}^x+f_{12}^{\prime \prime} \cdot(-\sin x)\right] \mathrm{e}^x+f_1^{\prime} \cdot \mathrm{e}^x}_{f_1^{\prime} \cdot \mathrm{e}^x\text{in}x\text{note}}-\underbrace{\left[f_{21}^{\prime \prime} \cdot \mathrm{e}^x+f_{22}^{\prime \prime} \cdot(-\sin x)\right] \sin x-f_2^{\prime} \cdot \cos x}_{f_2^{\prime} \cdot(-\sin x)\text{note}}$
		- $\xlongequal[\text{note}]{f_{21}^{\prime \prime}=f_{12}^{\prime \prime}}f_{11}^{\prime \prime} \cdot \mathrm{e}^{2 x}+f_{22}^{\prime \prime} \cdot \sin ^2 x+f_1^{\prime} \cdot \mathrm{e}^x-f_2^{\prime} \cdot \cos x-2 f_{12}^{\prime \prime} \cdot \sin x \cdot \mathrm{e}^x, \quad$
...
- 问题: 求 $\displaystyle \lim _{n \rightarrow \infty} \sum_{k=1}^{n} \frac{k}{n^{2}} \ln \left(1+\frac{k}{n}\right)$
	...
	- 第二步：化成udv形式，运用分部积分
		-  $\displaystyle I = \frac{1}{2} \int_0^1 \ln (1+x) \, \mathrm{d}(x^2)\xlongequal[]{\text{note}}\frac{1}{2} [\left. x^2 \ln (1+x)\right|_0 ^1-\int_0^1 \frac{x^2}{1+x} \mathrm{~d} x]$
...
## (17)
 (本题满分 10 分)
已知函数 $y(x)$ $\underbrace{\text{note} x^{3}+y^{3}-3 x+3 y-2=0 \text{note}}_{\text{note}}$,求 $y(x)$ 的极值. 
### (17)
- - 求驻点
	- 对方程两边同时对 $x$ 求导
		- $3x^2 + 3y^2y' - 3 + 3y' = 0$
...
## (18) 
(本题满分 10 分)
设函数 $f(x)$ 在区间 $[0,1]$ 上具有 2 阶导数, $\displaystyle \underbrace{\text{note} f(1)>0, \lim _{x \rightarrow 0^{+}} \frac{f(x)}{x}<0.}_{\text{note}}$ 证明:
( I ) 方程 $f(x)=0$ 在区间 $(0,1)$ 内至少存在一个实根;
( II )$\underbrace{\text{note} f(x) f^{\prime \prime}(x)+\left[f^{\prime}(x)\right]^{2}=0}_{\text{note}}$在区间 $(0,1)$ 内$\underbrace{\text{note}}_{\text{note}}$.
### (18)
...
### (19)
- 读题
	- 设薄片型物体 $S$ 是$\underbrace{\text{note} z=\sqrt{x^{2}+y^{2}} \text{note} z^{2}=2 x \text{note}}_{\text{sketch}}$, 其上任一点的密度为 $\mu(x, y, z)=9 \sqrt{x^{2}+y^{2}+z^{2}}$. 记$\underbrace{\text{note} C}_{\text{note}}$.<br>(I ) $\underbrace{\text{note} C \text{note} x O y \text{note}}_{\text{note}z=0\text{note}xOy\text{note}}$;<br>(II) $\underbrace{\text{note} S \text{note} M}_{\text{note}S\text{note}}$.(==抽象题==)
...
- 圆锥面上的有限部分 $S$、交线 $C$ 及其投影区域 $D$ 如图所示：
...
\begin{tikzpicture}
  \begin{scope}[tdplot_main_coords,scale=1.45]
    \draw[->] (0,0,0) -- (2.7,0,0) node[below right] {$x$};
    \draw[->] (0,-1.7,0) -- (0,1.9,0) node[below left] {$y$};
    \draw[->] (0,0,0) -- (0,0,2.7) node[above] {$z$};
    \draw[dashed] plot[domain=0:360,samples=100,variable=\t]
      ({2*cos(\t)},{2*sin(\t)},0);
    \draw[thin] plot[domain=0:360,samples=100,variable=\t]
      ({2*cos(\t)},{2*sin(\t)},2);
    \foreach \a in {120,300} {
      \draw[thin] (0,0,0) -- ({2*cos(\a)},{2*sin(\a)},2);
    }
    \draw[densely dashed] plot[domain=0:2,samples=60,variable=\u]
      ({\u*\u/2},-1.25,\u);
    \draw[densely dashed] plot[domain=0:2,samples=60,variable=\u]
      ({\u*\u/2},1.25,\u);
    \foreach \u in {0.5,1,1.5,2} {
      \draw[densely dashed] ({\u*\u/2},-1.25,\u) -- ({\u*\u/2},1.25,\u);
    }
    \fill[blue!10] plot[domain=-90:90,samples=120,variable=\t]
      ({2*cos(\t)*cos(\t)},{2*cos(\t)*sin(\t)},{2*cos(\t)}) -- cycle;
    \foreach \a in {-75,-50,-25,0,25,50,75} {
      \draw[blue!65,thin] (0,0,0) --
        ({2*cos(\a)*cos(\a)},{2*cos(\a)*sin(\a)},{2*cos(\a)});
    }
    \draw[thick] plot[domain=-90:90,samples=120,variable=\t]
      ({2*cos(\t)*cos(\t)},{2*cos(\t)*sin(\t)},{2*cos(\t)});
    \node at (0.72,0.25,1.05) {$S$};
    \node[right] at (2,0,2) {$C$};
    \node[above] at (1.75,-1.15,2.35) {$z^2=2x$};
    \node[above] at (-0.85,1.20,2.15) {$z=\sqrt{x^2+y^2}$};
    \node[below left] at (0,0,0) {$O$};
  \end{scope}
  \begin{scope}[xshift=7.1cm,scale=1.55]
    \begin{scope}
      \clip (1,0) circle (1);
      \fill[blue!10] (-0.1,-1.1) rectangle (2.1,1.1);
      \foreach \s in {-1.8,-1.55,...,1.8} {
        \draw[blue!70,thin] (-0.1,\s) -- (2.1,{\s+2.2});
      }
    \end{scope}
    \draw[->] (-0.25,0) -- (2.45,0) node[right] {$x$};
    \draw[->] (0,-1.35) -- (0,1.35) node[above] {$y$};
    \draw[thick] (1,0) circle (1);
    \node at (1,0) {$D$};
    \node[above right] at (1.55,0.82) {$x^2+y^2=2x$};
...
- $\displaystyle \begin{aligned}\int_0^{\frac{\pi}{2}} \sin ^n x \mathrm{~d} x=\int_0^{\frac{\pi}{2}} \cos ^n x \mathrm{~d} x= \begin{cases}\frac{n-1}{n} \cdot \frac{n-3}{n-2} \cdots \cdot \frac{1}{2} \cdot \frac{\pi}{2}, & n \text{note} n \geqslant 2, \\ \frac{n-1}{n} \cdot \frac{n-3}{n-2} \cdots \cdot \frac{2}{3}, & n \text{note} n \geqslant 3 .\end{cases}\end{aligned}$
## (20)
 (本题满分 11 分)
...
### (20)
- 读题:设 3 阶矩阵 $\boldsymbol{A}=\left(\boldsymbol{\alpha}_{1}, \boldsymbol{\alpha}_{2}, \boldsymbol{\alpha}_{3}\right)$ $\underbrace{\text{note} 3 \text{note}}_{\text{note}0\text{note}0}$, 且 $\underbrace{\boldsymbol{\alpha}_{3}=\boldsymbol{\alpha}_{1}+2 \boldsymbol{\alpha}_{2}}_{\text{note}}$.<br>( I ) 证明 $\underbrace{r(\boldsymbol{A})=2}_{\text{note}0}$;<br>( II ) 设 $\underbrace{\boldsymbol{\beta}=\boldsymbol{\alpha}_{1}+\boldsymbol{\alpha}_{2}+\boldsymbol{\alpha}_{3}}_{\text{note}}$, 求$\underbrace{\text{note} \boldsymbol{A x}=\boldsymbol{\beta} \text{note}}_{\text{note}+\text{note}}$.
...
- (I)由于 $\boldsymbol{A}$ 有 3 个不同的特征值 $\lambda_1, \lambda_2, \lambda_3$, 因此至多仅有一个零特征值. 
	...
	- 因此, $r(\boldsymbol{A})=2$.
- $\displaystyle  \alpha_1+2 \alpha_2-\alpha_3=0 \xrightarrow[]{\text{note}}\left(\alpha_1, \alpha_2, \alpha_3\right)\left(\begin{array}{c}1 \\2 \\-1\end{array}\right)=0 \quad A\left(\begin{array}{c}1 \\2 \\-1\end{array}\right)=0$
...
-  $\displaystyle  \beta=\alpha_1+\alpha_2+\alpha_3 \xrightarrow[]{\text{note}}\left(\alpha_1, \alpha_2 ,\alpha_3\right)\left(\begin{array}{c}1 \\1 \\1\end{array}\right)=\beta \quad A\left(\begin{array}{l}1 \\1 \\1\end{array}\right)=\beta$
...
### (21)
- 读题
	- 设二次型 $\underbrace{f\left(x_{1}, x_{2}, x_{3}\right)=2 x_{1}^{2}-x_{2}^{2}+a x_{3}^{2}+2 x_{1} x_{2}-8 x_{1} x_{3}+2 x_{2} x_{3}}_{\text{note}}$ 在正交变换 $\boldsymbol{x}=\boldsymbol{Q y}$ 下的$\underbrace{\text{note} \lambda_{1} y_{1}^{2}+\lambda_{2} y_{2}^{2}}_{\text{note}0\text{note}0}$, $\underbrace{\text{note} a \text{note}}_{\text{note}|A|=0\text{note}}$及$\underbrace{\text{note} \boldsymbol{Q}}_{A\text{note}Q}$.
- 二次型都可以转化为实对称矩阵的问题
...
## (22)
 (本题满分 11 分)
设随机变量 $X, Y$ 相互独立, 且 $X$ 的概率分布为 $P\{X=0\}=P\{X=2\}=\frac{1}{2}, Y$ 的概率密度为 $\displaystyle f(y)= \begin{cases}2 y, & 0<y<1, \\ 0, & \text{otherwise.}\end{cases}$
(I) 求 $P\{Y \leqslant E(Y)\}$;
(II) 求 $Z=X+Y$ 的概率密度.
### (22)
- 读题<br>设随机变量 $X, Y$ 相互独立, 且 $X$ 的概率分布为 $P\{X=0\}=P\{X=2\}=\frac{1}{2}$ ，$\displaystyle \underbrace{Y\text{note} f(y)= \begin{cases}2 y, & 0<y<1, \\ 0, & \text{otherwise.}\end{cases}}_{\text{note}Y\text{mean}}$<br>(I) 求 $\underbrace{P\{Y \leqslant E(Y)\}}_{\text{note}E(Y)\text{note}}$;<br>(II) 求 $\underbrace{Z=X+Y \text{note}}_{\text{note}}$.
- 由于 $X$ 为离散型随机变量,
...
- 先求$Z$ 的分布函数 $F_Z(z)$
	...
	- $\xlongequal[\text{note}]{\text{note}}P\{X+Y \leqslant z , X=0\} + P\{X+Y \leqslant z , X=2\}$
	- $\xlongequal[]{X\text{note}Y\text{note}}P\{X+Y \leqslant z \}\text{·}P\{X=0\} + P\{X+Y \leqslant z \}\text{·}P\{X=2\}$
	- $\xlongequal[]{\text{set}X\text{note}}\frac{1}{2} P\{Y \leqslant z\} + \frac{1}{2} P\{Y \leqslant z-2\}$
- 根据$\text{note}y\text{note}x$的概率密度的取值范围区域:画图， $z$ 的不同范围计算 $F_Z(z)$
	- $Z$ 的密度只在 $(0,1)$ 与 $(2,3)$ 上非零，可直接画成下图；原先含混的一维草图与分段图重复，故合并为这一张：

```tikz
\usepackage{tikz}
\begin{document}
\begin{tikzpicture}[x=1.45cm,y=1.8cm]
  \draw[->] (-0.25,0) -- (3.55,0) node[right] {$z$};
  \draw[->] (0,-0.15) -- (0,1.35) node[above] {$f_Z(z)$};
  \draw[very thick,blue!65] (-0.2,0) -- (0,0)
    (1,0) -- (2,0) (3,0) -- (3.45,0);
  \draw[thick] (0,0) -- (1,1);
  \draw[thick] (2,0) -- (3,1);
  \draw[dashed] (1,0) -- (1,1);
  \draw[dashed] (3,0) -- (3,1);
  \draw[fill=white] (1,1) circle (1.7pt);
  \draw[fill=white] (3,1) circle (1.7pt);
  \filldraw (0,0) circle (1.3pt);
  \filldraw (1,0) circle (1.3pt);
  \filldraw (2,0) circle (1.3pt);
  \filldraw (3,0) circle (1.3pt);
  \foreach \x in {1,2,3} {
    \draw (\x,0.06) -- (\x,-0.06) node[below] {$\x$};
  }
  \draw (0.06,1) -- (-0.06,1) node[left] {$1$};
  \node[above] at (0.50,1.10) {$f_Z(z)=z$};
  \node[above] at (2.50,1.10) {$f_Z(z)=z-2$};
  \node[below left] at (0,0) {$O$};
\end{tikzpicture}
\end{document}
```
...
- 求 $Z$ 的概率密度 $f_Z(z)$:也就是对分布函数求导
	- $f_Z(z) = F_Z^{\prime}(z)$
	- $\displaystyle f_Z(z) = \begin{cases}z, & 0<z<1, \\ z-2, & 2<z<3, \\ 0, & \text{otherwise.}\end{cases}$
## (23)
 (本题满分 11 分)
...
- 当 $z<0$ 时, $F_{Z_1}(z)=0$.
- 当 $z \geqslant 0$ 时,
	- $\displaystyle  F_{Z_1}(z) \xlongequal[]{\text{note}}P\left\{Z_1 \leqslant z\right\}\xlongequal[]{Z_{i}=\left|X_{i}-\mu\right|}P\left\{\left|X_1-\mu\right| \leqslant z\right\}$
	- $\displaystyle  \xlongequal[]{\text{standardize}}P\left\{\left|\frac{X_1-\mu}{\sigma}\right| \leqslant \frac{z}{\sigma}\right\}\xlongequal[]{\text{remove abs}}P\left\{-\frac{z}{\sigma} \leqslant \frac{X_1-\mu}{\sigma} \leqslant \frac{z}{\sigma}\right\}$
...
- (2)
	-  $\displaystyle  E\left(Z_1\right)=\int_{-\infty}^{+\infty} z f_{Z_1}(z) \mathrm{d} z\xlongequal[]{\text{note}z}\int_0^{+\infty} z \cdot \sqrt{\frac{2}{\pi}} \cdot \frac{1}{\sigma} \cdot e^{-\frac{z^2}{2 \sigma^2}} d z$
	- $\displaystyle  \xlongequal[]{\text{note}}\int_0^{+\infty} \sqrt{\frac{2}{\pi}} \cdot \sigma \cdot e^{-\frac{z^2}{2 \sigma^2}} d\left(\frac{z^2}{2 \sigma^2}\right)$
	- $\displaystyle  \xlongequal[]{\text{Newton-Leibniz}}\sqrt{\frac{2}{\pi}} \cdot \sigma \cdot-\left.e^{-\frac{z^2}{2 \sigma^2}}\right|_0 ^{+\infty}$
...
- 因此， $\displaystyle \sigma$ 的最大似然估计量<br>为$\displaystyle \xrightarrow[]{\text{note}}\hat{\sigma}=\sqrt{\frac{1}{n} \sum_{i=1}^n Z_i^2} .$
