...
- 由随机变量 $X$ 的分布函数求导，得概率密度$F'(x)$  
   - 对两边求导，得 $F'(x) = 0.3 \Phi'(x) + 0.35 \Phi'\left(\frac{x-1}{2}\right)$。  
...
- 对概率密度乘X积分得到期望:$E(X)$（$X$ 的期望值）  
    ...
    - 记标准正态分布的概率密度为 $\varphi(x)$，则  
      $$
      E(X)=\int_{-\infty}^{+\infty} 0.3 x \varphi(x) \, \mathrm{d} x+\int_{-\infty}^{+\infty} 0.35 x \varphi\left(\frac{x-1}{2}\right) \, \mathrm{d} x.
      $$
       - 第一项可提出常数，且 $\displaystyle \int_{-\infty}^{+\infty} x \varphi(x) \, \mathrm{d} x=0$，所以这一项为 $0$。  
       - 第二项令 $u=\frac{x-1}{2}$，则 $x=2u+1$、$\mathrm{d}x=2\,\mathrm{d}u$，从而化为 $\displaystyle \int_{-\infty}^{+\infty} 0.7(2 u+1) \varphi(u) \, \mathrm{d} u=0.7$。  
## (8)
...
- 答案选择 B 的理由  
    ...
    - 计算 $F_Z(z)$ 的过程  
       - 使用全概率公式  
          - 由 $Z=XY$，有 $F_Z(z) =P\{Z\leq z\}=P\{XY \leq z\}$  
             - 全集分解为 $Y = 0$ 和 $Y = 1$ 的情况  
                - 当 $Y = 0$ 时，代入可得 $P\{XY \leq z \mid Y = 0\}= P\{0 \leq z\}$，再乘以 $P\{Y=0\}=\frac12$。  
                - 当 $Y = 1$ 时，代入可得 $P\{XY \leq z \mid Y = 1\}=P\{X \leq z\}$，再乘以 $P\{Y=1\}$。  
          - 结合各种情况  
...
- 计算 $z=f(x, xy)$ 的一阶和二阶偏导数。  
    ...
    - 计算二阶混合偏导数 $\frac{\partial^2 z}{\partial x \partial y}$。  
       - 对 $\frac{\partial z}{\partial x}$ 关于 $y$ 求偏导得：  
          - $\frac{\partial^2 z}{\partial x \partial y} = \underbrace{f_{11}^{\prime \prime} \cdot 0 + x f_{12}^{\prime \prime} }_{f_1^{\prime}\text{in}y\text{note}}+ \underbrace{f_2^{\prime} + y(f_{21}^{\prime \prime} \cdot 0 + x f_{22}^{\prime \prime})}_{y f_2^{\prime}\text{in}y\text{note}}$。  
...
- 求 $S_1$ 和 $S_2$ 的方程。  
    ...
    - 求圆锥面 $S_2$ 的方程。  
       ...
       - 切线绕x轴旋转得到圆锥面 $S_2$ 的方程。  
          ...
          - 根据切线方程得到 $S_2$ 的方程：$\left(\frac{x}{4}-1\right)^2=\frac{y^2+z^2}{4} \text{note}(x-4)^2-4 y^2-4 z^2=0 \text{.}$  
...
- $\displaystyle \boldsymbol{\xi}_2=\left(-\frac{1}{2}, \frac{1}{2}, 0\right)^{\mathrm{T}}+C_1(1,-1,2)^{\mathrm{T}} \text{note} \boldsymbol{\xi}_2=\left(\begin{array}{c}-\frac{1}{2}+C_1 \\\frac{1}{2}-C_1 \\2 C_1\end{array}\right) \text{,}$其中 $C_1$ 为任意常数.  
...
- $\displaystyle \xi_3=C_2(-1,1,0)^{\mathrm{T}}+C_3(0,0,1)^{\mathrm{T}}+\left(-\frac{1}{2}, 0,0\right)^{\mathrm{T}} \text{note} \xi_3=\left(\begin{array}{c}-\frac{1}{2}-C_2 \\C_2 \\C_3\end{array}\right),$其中 $C_2, C_3$ 为任意常数.  
...
## (23)  
(本题满分 11 分)  
设总体 $X$ 的概率密度为 $\displaystyle f(x)=\left\{\begin{array}{ll}\lambda^{2} x \mathrm{e}^{-\lambda x}, & x>0, \\ 0, & \text{otherwise,}\end{array}\right.$ 其中参数 $\lambda(\lambda>0)$ 未知, $X_{1}, X_{2}, \cdots, X_{n}$ 是 来自总体 $X$ 的简单随机样本.  
( I ) 求参数 $\lambda$ 的矩估计量;  
