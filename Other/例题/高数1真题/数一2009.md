...
## (1)   
当 $\displaystyle x \rightarrow 0$ 时, $f(x)=x-\sin a x$ 与 $g(x)=x^{2} \ln (1-b x)$ 是等价无穷小量, 则 ()  
(A) $a=1, b=-\frac{1}{6}$.  
(B) $a=1, b=\frac{1}{6}$.  
( C) $a=-1, b=-\frac{1}{6}$.  
(D) $a=-1, b=\frac{1}{6}$.  
...
- 第一步：要找出 $a$ 和 $b$ 的值，使得 $f(x)$ 和 $g(x)$ 在 $x \rightarrow 0$ 时是等价无穷小量。  
  
- 第二步：使用泰勒公式展开 $\sin ax$ 和 $\ln(1-bx)$。  
  1. 展开 $f(x)$：$f(x) = x - (ax - \frac{1}{6}(ax)^3 + o(x^3)) = (1-a)x + \frac{a^3}{6}x^3 + o(x^3)$。  
  2. 展开 $g(x)$：$g(x) \sim -bx^3$。  
  
- 第三步：比较 $f(x)$ 和 $g(x)$ 的展开式，求解 $a$ 和 $b$。  
  - 计算步骤  
    1. 比较系数：同指数的系数必须相等  
    2. 求解方程：解方程组 $\left\{\begin{array}{l} 1-a=0, \\ \frac{1}{6}a^3=-b \end{array}\right.$   
    - 得 $a=1, b=-\frac{1}{6}$。  
