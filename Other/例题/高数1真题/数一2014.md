	...
	- 判断选项 (C) $y = x + \sin \frac{1}{x}$
		...
		- 铅直渐近线
			...
			- $\displaystyle \lim_{x \to 0} y =\lim _{x \rightarrow 0}\left(x+\sin \frac{1}{x}\right)\xlongequal[]{x=0} \lim_{x \to 0} \sin \frac{1}{x} \neq \infty$
				- 不存在铅直渐近线
		- 斜渐近线
			- 计算斜率 $\displaystyle \lim_{x \to \infty} \frac{y}{x} =\lim _{x \rightarrow \infty} \frac{x+\sin \frac{1}{x}}{x} \xlongequal[]{\text{note}}\lim _{x \rightarrow \infty}\left(1+\frac{\sin \frac{1}{x}}{x}\right)\xlongequal[]{\frac{\sin\frac{1}{\infty}}{\infty}=0}1+0=1$
...
- 答 应选(D).
- 题中问的是，
	- 当 $f^{\prime}(x)\geqslant0$ 时，只能确定 $f$ 单调不减，$f$ 可以在弦 $g$ 的上下两侧，不能确定统一的大小关系。
	- 当 $f^{\prime\prime}(x)\geqslant0$ 时，$f$ 为凸函数，因而在 $[0,1]$ 上 $f(x)\leqslant g(x)$。
...
\begin{tikzpicture}[scale=1.05]
  \begin{scope}
    ...
    \draw[thick] (0,0.3) -- (2,1.8) node[pos=0.62,above left] {$g$};
    \draw[very thick,smooth,domain=0:1,samples=100,variable=\t]
      plot ({2*\t},{0.3+1.5*\t+0.18*sin(360*\t)});
    \node at (0.55,1.02) {$f$};
    \fill (0,0.3) circle (1.3pt) node[left] {$A$};
    \fill (2,1.8) circle (1.3pt) node[right] {$B$};
    \node[below] at (1.05,-0.28) {$f'\geq0$};
  \end{scope}
...
### (3)
- 答 应选(D).
- AB选项是交换 $x$ 和 $y$ 的积分次序。下图同时标出直角坐标的竖直分割和极坐标的射线分割，两种写法对应同一区域 $D$。

```tikz
\usepackage{tikz}
\begin{document}
\begin{tikzpicture}[scale=1.55]
  \fill[black!8] (-1,0) arc[start angle=180,end angle=90,radius=1]
    -- (1,0) -- cycle;
  \begin{scope}
    \clip (-1,0) arc[start angle=180,end angle=90,radius=1]
      -- (1,0) -- cycle;
    \foreach \s in {-1.8,-1.6,...,1.4} {
      \draw[black!55,thin] (-1.1,\s) -- (1.1,{\s+2.2});
    }
  \end{scope}
  \draw[->] (-1.3,0) -- (1.35,0) node[right] {$x$};
  \draw[->] (0,-0.12) -- (0,1.3) node[above] {$y$};
  \draw[thick] (-1,0) arc[start angle=180,end angle=90,radius=1];
  \draw[thick] (0,1) -- (1,0);
  \draw[->,thin] (-0.6,0.05) -- (-0.6,0.76);
  \draw[->,thin] (0.45,0.05) -- (0.45,0.51);
  \draw[thin] (0,0) -- ({cos(120)},{sin(120)});
  \draw[thin] (0,0) -- (0.5,0.5);
  \node at (-0.45,0.38) {$D$};
  \node[above left] at (-0.7,0.72) {$x^2+y^2=1$};
  \node[above right] at (0.55,0.48) {$x+y=1$};
  \node[below] at (-1,0) {$-1$};
  \node[below] at (1,0) {$1$};
  \node[left] at (0,1) {$1$};
\end{tikzpicture}
...
```
	...
	- 在直角坐标系下，原式 $\displaystyle =\int_{-1}^0 \mathrm{~d} x \int_0^{\sqrt{1-x^2}} f(x, y) \mathrm{d} y+\int_0^1 \mathrm{~d} x \int_0^{1-x} f(x, y) \mathrm{d} y$
- CD选项是将直角坐标系化为极坐标系，使用上图中从原点出发的射线分割。
...
- $\displaystyle \begin{aligned} \int_{-\pi}^\pi(x-a \cos x-b \sin x)^2 \mathrm{~d} x & \end{aligned}$
	- $\displaystyle \begin{aligned}\xlongequal[]{\text{perfect square}}\int_{-\pi}^\pi\left(x^2+a^2 \cos ^2 x+b^2 \sin ^2 x-2 a x \cos x-2 b x \sin x+2 a b \sin x \cos x\right) \mathrm{d} x\end{aligned}$
	- $\displaystyle \begin{aligned} \xlongequal[\text{note}0]{\text{symmetry}} 2 \int_0^\pi x^2 \mathrm{~d} x+\underbrace{2 \int_0^\pi\left(a^2 \cos ^2 x+b^2 \sin ^2 x-2 b x \sin x\right) \mathrm{d} x}_{\text{note}} \end{aligned}$
...
- 直接按行或按列展开，而不是去两两交换某一行或某一列
	- $\displaystyle \left|\begin{array}{llll}0 & a & b & 0 \\ a & 0 & 0 & b \\ 0 & c & d & 0 \\ c & 0 & 0 & d\end{array}\right|\xlongequal[\text{note}]{\text{note}}\underbrace{a(-1)^{2+1}\left|\begin{array}{lll}a & b & 0 \\ c & d & 0 \\ 0 & 0 & d\end{array}\right|}_{\text{note}}+\underbrace{c(-1)^{4+1}\left|\begin{array}{lll}a & b & 0 \\ 0 & 0 & b \\ c & d & 0\end{array}\right|}_{\text{note}}$
...
- 先写出$P(A-B)$和$P(B-A)$的公式(要掌握概率的减法公式)
	-  $P(A-B) = P(A) - P(AB) =P(A \mid \bar{B})\xlongequal[]{A\text{note}B\text{note}} P(A) - P(A)P(B)$
		- 代入 $P(A-B) = 0.3$ 和 $P(B) = 0.5$
		- 解得 $P(A) = 0.6$
	- $P(B-A) = P(B) - P(AB) =P (B |\bar{A})\xlongequal[]{\text{note}} P(B) - P(A)P(B)$
...
- 原方程化为 $u+x \frac{\mathrm{d} u}{\mathrm{~d} x}=u \ln u$, 
	- $\displaystyle \begin{aligned}\xrightarrow[\text{separate variables}]{\int \frac{d u}{f(u)-u}=\int \frac{d x}{x}}\frac{d u}{u(\ln u-1)}=\frac{d x}{x}\end{aligned}$
...
### (12)
- 答 应填 $\pi$.
- $Σ$ 取平面 $y+z=0$ 在柱面内的部分。指定的环向对应法向量的 $z$ 分量为正，因此取 $\boldsymbol n=(0,1,1)$。
...
\begin{tikzpicture}[scale=1.15]
  \begin{scope}[tdplot_main_coords]
    \draw[->] (0,0,0) -- (1.65,0,0) node[below right] {$x$};
    \draw[->] (0,0,0) -- (0,1.65,0) node[below left] {$y$};
    \draw[->] (0,0,-1.75) -- (0,0,1.85) node[above] {$z$};

    \draw[dashed] plot[variable=\t,domain=0:360,samples=80,smooth]
      ({cos(\t)},{sin(\t)},-1.5);
    \draw[thin] plot[variable=\t,domain=0:360,samples=80,smooth]
      ({cos(\t)},{sin(\t)},1.5);
    \foreach \ang in {110,310} {
      \draw[thick] ({cos(\ang)},{sin(\ang)},-1.5)
        -- ({cos(\ang)},{sin(\ang)},1.5);
    }

    \fill[black!8]
      plot[variable=\t,domain=0:360,samples=100,smooth]
        ({cos(\t)},{sin(\t)},{-sin(\t)}) -- cycle;
    \begin{scope}
      \clip plot[variable=\t,domain=0:360,samples=100,smooth]
        ({cos(\t)},{sin(\t)},{-sin(\t)}) -- cycle;
      \foreach \s in {-1.2,-0.95,...,1.3} {
        \draw[black!60,thin] (-1.2,\s,{-\s}) -- (1.2,\s,{-\s});
      }
    \end{scope}
    \draw[thick] plot[variable=\t,domain=0:360,samples=100,smooth]
      ({cos(\t)},{sin(\t)},{-sin(\t)}) -- cycle;
    \draw[->,very thick]
      plot[variable=\t,domain=15:70,samples=25,smooth]
        ({cos(\t)},{sin(\t)},{-sin(\t)});
    \draw[->,thick] (0,0,0) -- (0,0.72,0.72)
      node[above right] {$\mathbf{n}$};
    \node at (-0.25,0.2,-0.2) {$\Sigma$};
    \node[right] at (0,0.95,-0.95) {$L$};
...
- 
	- 可知$\displaystyle \begin{aligned} \oint_L z \mathrm{~d} x+y \mathrm{~d} z & =\iint_{\Sigma}\left|\begin{array}{ccc}0 & \frac{1}{\sqrt{2}} & \frac{1}{\sqrt{2}} \\\frac{\partial}{\partial x} & \frac{\partial}{\partial y} & \frac{\partial}{\partial z} \\z & 0 & y\end{array}\right| \mathrm{d} S\xlongequal[]{\text{expand first row}}\frac{1}{\sqrt{2}} \iint_{\Sigma} \mathrm{d} S \end{aligned}$
		- $\displaystyle \begin{aligned}\xlongequal[]{\text{note}}\iint_D \frac{\sqrt{2}}{2} \sqrt{z_x^2+z_y^2+1} d x\end{aligned}$
		- $\displaystyle \begin{aligned}\xlongequal[]{z=-y\text{note}}\frac{1}{\sqrt{2}} \iint_{D_{x y}} \sqrt{0^2+(-1)^2+1} \mathrm{~d} x \mathrm{d} y\end{aligned}$
...
## (14)
 设总体 $X$ 的概率密度为 $\displaystyle f(x ; \theta)=\left\{\begin{array}{ll}\frac{2 x}{3 \theta^{2}}, & \theta<x<2 \theta, \\ 0, & \text{otherwise,}\end{array}\right.$ 其中 $\theta$ 是未知参数, $X_{1}, X_{2}, \cdots, X_{n}$ 为来自 总体 $X$ 的简单随机样本, 若 $\displaystyle c \sum_{i=1}^{n} X_{i}^{2}$ 是 $\theta^{2}$ 的无偏估计, 则 $c=$
### (14)
...
#### (15)
- $\displaystyle \begin{aligned} \lim_{x \to +\infty} \frac{\int_{1}^{x} \left[ t^2 \left( e^{\frac{1}{t}} - 1 \right) - t \right] dt}{x^2 \ln \left( 1 + \frac{1}{x} \right)} \xlongequal[\text{note}]{\text{note}} \lim_{x \to +\infty} \frac{\int_{1}^{x} \left[ t^2 \left( e^{\frac{1}{t}} - 1 \right) - t \right] dt}{x} \end{aligned}$
	- 只要分母趋于 $\displaystyle \begin{aligned} \infty \end{aligned}$，不必检查分子是否趋于 $\displaystyle \begin{aligned} \infty \end{aligned}$。
	- $\displaystyle \begin{aligned} \xlongequal[]{\text{note}} \lim_{x \to +\infty} \left[ x^2 \left( e^{\frac{1}{x}} - 1 \right) - x \right] \end{aligned}$
	- $\displaystyle \begin{aligned} \left( \text{note} \frac{1}{x} = t \right) \xlongequal[]{\text{note}} \lim_{t \to 0^{+}} \left( \frac{e' - 1}{t^2} - \frac{1}{t} \right) \end{aligned}$
	- $\displaystyle \begin{aligned}\xlongequal[]{\text{note}}\lim_{t \to 0^{+}} \frac{e^t - 1 - t}{t^2} \xlongequal[]{\text{note}} \left( \lim_{t \to 0^{+}} \frac{1}{2} t^2 \right) = \frac{1}{2}.\end{aligned}$
## (16)
...
- 将 $\frac{\partial^2 z}{\partial x^2}, \frac{\partial^2 z}{\partial y^2}$ 代入已知等式得到一个微分方程。
	...
	- 则$\displaystyle \begin{aligned} e^{2x} f''(e^x \cos y) = (4z + e^x \cos y) e^{2x} \end{aligned}$
		- $\displaystyle \begin{aligned} \xrightarrow[]{\text{note}e^{2x}}f''(e^x \cos y) = 4f(e^x \cos y) + e^x \cos y \end{aligned}$
...
- 解微分方程求得 $f(u)$.
	...
	- 求特解
		...
		- 将$y\text{note}y''$代入原方程$f^{\prime \prime}(u)-4 f(u)=u$
			- 得$0-4 C_3 u-4 C_4=u$. 
...
#### (18)
-   
- 曲面 $Σ:z=x^2+y^2$ 在 $z\leqslant1$ 时的边界为 $z=1$ 上的单位圆，在 $xOy$ 面上的投影为单位圆盘 $D$。
...
\begin{tikzpicture}[scale=1.15]
  \begin{scope}[tdplot_main_coords]
    \draw[->] (0,0,0) -- (1.45,0,0) node[below right] {$x$};
    \draw[->] (0,0,0) -- (0,1.45,0) node[below left] {$y$};
    \draw[->] (0,0,0) -- (0,0,1.45) node[above] {$z$};
    \foreach \ang in {0,90,180,270} {
      \draw[dashed] plot[variable=\r,domain=0:1,samples=60,smooth]
        ({\r*cos(\ang)},{\r*sin(\ang)},{\r*\r});
    }
    \foreach \ang in {110,310} {
      \draw[thick] plot[variable=\r,domain=0:1,samples=60,smooth]
        ({\r*cos(\ang)},{\r*sin(\ang)},{\r*\r});
    }
    \draw[thick] plot[variable=\t,domain=0:360,samples=80,smooth]
      ({cos(\t)},{sin(\t)},1);
    \node[anchor=west] at (0,1.20,1.08) {$z=1$};
    \node at (0.45,-0.55,0.48) {$\Sigma$};
  \end{scope}
  \begin{scope}[xshift=4.5cm]
    ...
    \begin{scope}
      ...
      \foreach \s in {-1.8,-1.55,...,1.8} {
        \draw[black!60,thin] (-1.2,\s) -- (1.2,{\s+2.4});
      }
    \end{scope}
...
- $\displaystyle \begin{aligned} \xlongequal[\text{note}0]{\text{note}x,y\text{note}} \iint_{D} \left[-6x^2 - 2x^4 - 6y^2 - 2y^4 + x^2 + y^2 - 1\right] \, dx \, dy \end{aligned}$
	- $\displaystyle \begin{aligned} & =\iint_D\left[-5\left(x^2+y^2\right)-2\left(x^4+y^4\right)-1\right] d x d y \end{aligned}$
	- $\displaystyle \begin{aligned}\xlongequal[\text{note}4\text{note}]{\text{note}}-20 \iint_{D_1}\left(x^2+y^2\right) d x d y-8 \iint_{D_1}\left(x^4+y^4\right) d x d y-\pi \end{aligned}$
		-  $\displaystyle \begin{aligned} \iint_{D_1} (x^2 + y^2) \, dx \, dy = \int_{0}^{\frac{\pi}{2}} d\theta \int_{0}^{1} r^3 \, dr = \frac{\pi}{8} \end{aligned}$
		- $\displaystyle \begin{aligned} \iint_{D_1} (x^4 + y^4) \, dx \, dy\xlongequal[]{\text{note}}2 \iint_{D_1} x^4 \, dx \, dy = 2 \int_{0}^{\frac{\pi}{2}} d\theta \int_{0}^{1} r^4 \cos^4 \theta rdr \end{aligned}$
...
#### 高斯
- 在上图 $Σ$ 的边界圆 $x^2+y^2=1, z=1$ 上补平面圆盘 $\Sigma_1$，即得到包围 $\Omega_1$ 的闭曲面 $\Sigma\cup\Sigma_1$，无需重复绘制同一抛物面。
- $\displaystyle \begin{aligned} \Sigma_1: \left\{(x, y, z) \mid x^2 + y^2 \leq 1, \, z = 1\right\}, \, \text{note}, \end{aligned}$
	- 负方向的高斯，结果外面要加负号
	- $\displaystyle \begin{aligned} \Omega_1 = \Sigma_1 \text{note} \Sigma \text{note}. \end{aligned}$
- $\displaystyle \begin{aligned} I = \iint_{\Sigma} = \iint_{\Sigma + \Sigma_1} - \iint_{\Sigma_1} =-4 \pi\end{aligned}$
	- $\displaystyle \begin{aligned} \oint_{\Sigma + \Sigma_1} = -\iiint_{\Omega} (Px + Qy + Rz) \, dV \end{aligned}$
		...
		- $\displaystyle \begin{aligned}\xlongequal[]{\text{expand}}-\iiint_{\Omega}\left[3 x^2-6 x+3+3 y^2-6 y+3+1\right] d V\end{aligned}$
		- $\displaystyle \begin{aligned}=-\iiint_{\Omega}\left[3\left(x^2+y^2\right)+7\right] d V\xrightarrow[\text{note}]{\text{note}}\end{aligned}$
			- 对固定的 $z\in[0,1]$，横截面是圆盘 $D_z=\{(x,y)\mid x^2+y^2\leqslant z\}$，其极坐标范围为 $0\leqslant r\leqslant\sqrt z, 0\leqslant\theta\leqslant2\pi$。
...
- 计算分布函数(三件套)
	...
	- $\xlongequal[]{\text{note}}P\{Y \leqslant y ,X=1\} + P\{Y \leqslant y , X=2\}$
	- $\xlongequal[]{\text{note}}P\{Y \leqslant y \}\text{·}P\{X=1\} +P\{Y \leqslant y \}\text{·}P\{X=2\}$
		- $\displaystyle \xlongequal[]{\text{note}}\frac{1}{2} \int_{-\infty}^y f_{Y \mid X}(y \mid 1) \mathrm{d} y+\frac{1}{2} \int_{-\infty}^y f_{Y \mid X}(y \mid 2) \mathrm{d} y .$
			- 随机发量 $Y$ 服从均匀分布 $U(0, i)(i=1,2)$
				- 概率密度$\displaystyle f(x)=\left\{\begin{array}{cl}\frac{1}{b-a}, & a<x<b \\ 0, & \text{otherwise}\end{array}\right.$
			- 当 $X=1$ 时,概率密度 $\displaystyle f_{Y \mid X}(y \mid 1)=\left\{\begin{array}{ll}1, & 0<y<1, \\ 0, & \text{otherwise.}\end{array}\right.$
			- $X=2$ 时,概率密度 $\displaystyle f_{Y \mid X}(y \mid 2)=\left\{\begin{array}{ll}\frac{1}{2}, & 0<y<2, \\ 0, & \text{otherwise.}\end{array}\right.$
		- 条件分布的支持区间如下：$Y\mid X=1$ 只落在 $(0,1)$，$Y\mid X=2$ 只落在 $(0,2)$。

```tikz
\usepackage{tikz}
\begin{document}
\begin{tikzpicture}[xscale=1.6,yscale=1]
  \draw[->] (-0.2,-0.45) -- (2.35,-0.45) node[right] {$y$};
  \foreach \x in {0,1,2} {
    \draw (\x,-0.38) -- (\x,-0.52) node[below] {$\x$};
    \draw[dashed,black!55] (\x,-0.25) -- (\x,1.35);
  }
  \draw[very thick] (0,1) -- (1,1);
  \draw[fill=white] (0,1) circle (1.5pt);
  \draw[fill=white] (1,1) circle (1.5pt);
  \node[left] at (-0.1,1) {$Y\mid X=1$};
  \draw[very thick] (0,0.25) -- (2,0.25);
  \draw[fill=white] (0,0.25) circle (1.5pt);
  \draw[fill=white] (2,0.25) circle (1.5pt);
  \node[left] at (-0.1,0.25) {$Y\mid X=2$};
\end{tikzpicture}
\end{document}
```
			- 一图三吃:吃01,吃转化,吃吃积分限
...
- 计算 $Y$ 的概率密度
	- $\displaystyle f_Y(y) = F_Y^{\prime}(y) = \begin{cases} \frac{3}{4}, & 0 < y < 1, \\ \frac{1}{4}, & 1 < y < 2, \\ 0, & \text{otherwise} \end{cases}$
- 计算 $E(Y)$
	- $\displaystyle E(Y) = \int_{-\infty}^{+\infty} y f_Y(y) dy\xlongequal[]{\text{note}}\int_0^1 \frac{3}{4} y dy + \int_1^2 \frac{1}{4} y dy= \frac{3}{4}$
...
#### (23)
- (I ) 总体 $X$ 的概率密度为 $\displaystyle f(x ; \theta)= \begin{cases}\frac{2 x}{\theta} \mathrm{e}^{-\frac{x^2}{\theta}}, & x>0, \\ 0, & \text{otherwise.}\end{cases}$
...
- 陷阱:题中给出的是分布函数，因此需要求导得到概率密度
	- 总体 $X$ 的概率密度为 $\displaystyle f(x ; \theta)=\left\{\begin{array}{ll}\frac{2 x}{\theta} e^{-\frac{x^{2}}{\theta}}, & x \geq 0 \\ 0, & \text{otherwise}\end{array}\right.$,
- 设 $x_{1}, x_{2}, \cdots x_{n}$ 为样本观测值,
- 写出似然函数:
	-  $\displaystyle L(\theta)=\prod_{i=1}^{n} f\left(x_{i}\right)=\left\{\begin{array}{cc}\underbrace{\frac{2^{n} }{\theta^{n}} \text{·}x_{1} x_{2} \cdots x_{n}\text{·}\overbrace{e^{-\frac{1}{\theta} \sum_{i=1}^{n} x_{i}^{2}}}^{\text{set}\theta\text{note}}}_{\text{note}}& x_{1}, x_{2}, \cdots x_{n}>0 \\ 0, & \end{array}\right.$
