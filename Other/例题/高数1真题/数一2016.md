...
- <br>![|400](https://obsidian-picture-1314838930.cos.ap-beijing.myqcloud.com/c0a4fffa2e77454d49c7336c9b826886.png)
- <br>![|400](https://obsidian-picture-1314838930.cos.ap-beijing.myqcloud.com/20241214083341.png)
 答 应选(B).
解 二次型矩阵
由
...
### (15)
- [00:06](file:///C:/Users/wangpanfeng/Videos/01.%E6%95%B0%E5%AD%A6%E4%B8%80/03.2016%E5%B9%B4%E6%95%B0%E4%B8%80%E7%9C%9F%E9%A2%98/03.2016%E5%B9%B4%E8%80%83%E7%A0%94%E6%95%B0%E5%AD%A6%E7%9C%9F%E9%A2%98-%E8%A7%A3%E7%AD%94%E9%A2%981%EF%BC%88%E6%95%B0%E4%B8%80%EF%BC%89%EF%BC%8815~18%E9%A2%98%EF%BC%89.mp4#t=6.68) 解 
- <br>![|200](https://obsidian-picture-1314838930.cos.ap-beijing.myqcloud.com/20241214080003.png)
...
- $\begin{aligned} \int_0^{+\infty} y(x) \mathrm{d} x & \xlongequal[]{代入y(x)}\int_0^{+\infty}\left(C_1 \mathrm{e}^{\lambda_1 x}+C_2 \mathrm{e}^{\lambda_2 x}\right) \mathrm{d} x\end{aligned}$
...
- <br>![|200](https://obsidian-picture-1314838930.cos.ap-beijing.myqcloud.com/20241214224239.png)，![|200](https://obsidian-picture-1314838930.cos.ap-beijing.myqcloud.com/20241214224944.png)，![|150](https://obsidian-picture-1314838930.cos.ap-beijing.myqcloud.com/20241214232807.png)
...
-  $\begin{aligned} I = \iint_{\Omega}(x^{2}+1) dy dz - 2y dz dx + 3z dx dy \end{aligned}$
	...
	- $\begin{aligned}=\iiint_{\Omega} 2 x d V+V_{\Omega}\end{aligned}$
...
- $\boldsymbol{A}^{99}=\boldsymbol{P}\left(\begin{array}{ccc}(-1)^{99} & 0 & 0 \\0 & (-2)^{99} & 0 \\0 & 0 & 0\end{array}\right) \boldsymbol{P}^{-1}=\left(\begin{array}{lll}1 & 1 & 3 \\1 & 2 & 2 \\0 & 0 & 2\end{array}\right)\left(\begin{array}{ccc}(-1)^{99} & 0 & 0 \\0 & (-2)^{99} & 0 \\0 & 0 & 0\end{array}\right)\left(\begin{array}{ccc}2 & -1 & -2 \\-1 & 1 & \frac{1}{2} \\0 & 0 & \frac{1}{2}\end{array}\right)$$=\left(\begin{array}{ccc}2^{99}-2 & 1-2^{99} & 2-2^{98} \\2^{100}-2 & 1-2^{100} & 2-2^{99} \\0 & 0 & 0\end{array}\right)$(II)因为 $B^2=B A$, 所以
- $B^{100}=B^{98} B^2=B^{99} A=B^{97} B^2 A=B^{98} A^2=\cdots=B A^{99},$即 $\left(\boldsymbol{\beta}_1, \boldsymbol{\beta}_2, \boldsymbol{\beta}_3\right)=\left(\boldsymbol{\alpha}_1, \boldsymbol{\alpha}_2, \boldsymbol{\alpha}_3\right)\left(\begin{array}{ccc}2^{99}-2 & 1-2^{99} & 2-2^{98} \\ 2^{100}-2 & 1-2^{100} & 2-2^{99} \\ 0 & 0 & 0\end{array}\right)$, 所以 $\left\{\begin{array}{l}\boldsymbol{\beta}_1=\left(2^{99}-2\right) \boldsymbol{\alpha}_1+\left(2^{100}-2\right) \boldsymbol{\alpha}_2, \\ \boldsymbol{\beta}_2=\left(1-2^{99}\right) \boldsymbol{\alpha}_1+\left(1-2^{100}\right) \boldsymbol{\alpha}_2, \\ \boldsymbol{\beta}_3=\left(2-2^{98}\right) \boldsymbol{\alpha}_1+\left(2-2^{99}\right) \boldsymbol{\alpha}_2 .\end{array}\right.$
## (22)
 (本题满分 11 分)
...
### (22)
- [37:29](file:///C:/Users/wangpanfeng/Videos/01.%E6%95%B0%E5%AD%A6%E4%B8%80/03.2016%E5%B9%B4%E6%95%B0%E4%B8%80%E7%9C%9F%E9%A2%98/04.2016%E5%B9%B4%E8%80%83%E7%A0%94%E6%95%B0%E5%AD%A6%E7%9C%9F%E9%A2%98-%E8%A7%A3%E7%AD%94%E9%A2%982%EF%BC%88%E6%95%B0%E4%B8%80%EF%BC%89%EF%BC%8819-23%E9%A2%98%EF%BC%89.mp4#t=37:29) 
解
- 均匀分布的概率密度怎么求？先求面积，取面积的倒数就是概率密度
	- 先通过区域画图，用定积分计算面积$S_{D}=\int_{0}^{1}(\sqrt{x}-x^{2})dx=\frac{1}{3}$
	- 取面积的倒数得到概率密度∶$f(x,y)=\left\{ \begin{array}{cc} 3, & 0 < x < 1, x^2 < y < \sqrt{x} \\ 0, & \text{其他} \end{array} \right.$
...
- (II) 判断 $U$ 和 $X$ 是否独立，通过$P(AB)=P(A)P(B)$则独立来反证
	- 计算 $P\{X \leq \frac{1}{2}\}$ 和 $P\{U＝0\}$
		...
		- 计算 $P\{X \leq \frac{1}{2}\} = \frac{1}{\sqrt{2}} - \frac{1}{8}$
			- $\begin{aligned} & \text { 如图所示 } P\left\{X \leqslant \frac{1}{2}\right\}=\int_0^{\frac{1}{2}} \mathrm{~d} x \int_{x^2}^{\sqrt{x}} 3 \mathrm{~d} y \end{aligned}$
- $\begin{aligned} \xlongequal[]{对y积分}3 \int_0^{\frac{1}{2}}\left(\sqrt{x}-x^2\right) \mathrm{d} x \end{aligned}$
- $\begin{aligned} =\left.3\left(\frac{2}{3} x^{\frac{3}{2}}-\frac{x^3}{3}\right)\right|_0 ^{\frac{1}{2}}=\frac{1}{\sqrt{2}}-\frac{1}{8} ; \end{aligned}$
	- 计算 $P\{X \leq \frac{1}{2}, U ＝ 0\}$的联合分布，通过用二重积分计算区域图形的面积
		- $\begin{aligned} & \text { 如图所示, } P\left\{X \leqslant \frac{1}{2}, U ＝0\right\} \end{aligned}$
- $\begin{aligned} =P\left\{X \leqslant \frac{1}{2}, X>Y\right\} \end{aligned}$
- $\begin{aligned} =\int_0^{\frac{1}{2}} \mathrm{~d} x \int_{x^2}^x 3 \mathrm{~d} y\xlongequal[]{对y积分}3 \int_0^{\frac{1}{2}}\left(x-x^2\right) \mathrm{d} x \end{aligned}$
- $\begin{aligned} =\left.3\left(\frac{1}{2} x^2-\frac{1}{3} x^3\right)\right|_0 ^{\frac{1}{2}}=\frac{1}{4} \end{aligned}$
...
- $f_T(z)= \begin{cases}\frac{9 z^8}{\theta^9}, & 0<z<\theta, \\ 0, & \text { 其他. }\end{cases}$(II) $E T=\int_{-\infty}^{+\infty} z f_T(z) \mathrm{d} z=\int_0^\theta \frac{9 z^9}{\theta^9} \mathrm{~d} z=\frac{9}{10} \theta$, 从而 $E(a T)=\frac{9}{10} a \theta$, 由 $E(a T)=\theta$, 得 $a=\frac{10}{9}$. 所以当 $a=\frac{10}{9}$ 时, $a T$ 为 $\theta$ 的无偏估计.
