...
- $\displaystyle \int_0^{1} \frac{1}{x^a(1+x)^b} \, \mathrm{d} x\xrightarrow[\text{note}]{\lim _{x \rightarrow 0}(1+x)^b=1} \frac{1}{x^a}\xrightarrow[]{x\to0\text{,}a<1}\text{note}$
	- 结论：由$\displaystyle \int_0^1 \frac{1}{x^p} d x=\left\{\begin{array}{l}p<1,\text{note}\\ p \geqslant 1,\text{note}\end{array}\right.$,得$\displaystyle \int_0^1 \frac{1}{x^a} \, \mathrm{d} x$ 收敛,则 $a<1$ ，<br>所以原积分的这一部分在 $a<1$ 时收敛。
- $\displaystyle \int_1^{+\infty} \frac{1}{x^a(1+x)^b} \, \mathrm{d} x\xrightarrow[\frac{1}{x^a} \cdot \frac{1}{x^b} \frac{1}{\left(1+\frac{1}{x}\right)^b}]{\lim _{x \rightarrow \infty}(1+x)^b=x^b}\int_1^{+\infty} \frac{1}{x^{a+b}} d x\xrightarrow[]{x\to\infty, a+b>1}\text{note}$
	- 结论：由$\displaystyle \int_1^{+\infty} \frac{1}{x^p} d x=\left\{\begin{array}{l}p>1,\text{note}\\ p \leqslant 1,\text{note}\end{array}\right.$,得$\displaystyle \int_1^{+\infty} \frac{1}{x^{a+b}} \, \mathrm{d} x$ 收敛时,$a+b>1$ ，<br>所以原积分的这一部分在 $a+b>1$ 时收敛。
...
- 计算分子 $\operatorname{Cov}(X, Y)$
	...
	- 首先求 $E(XY)$：
		- 先求(X,Y)的分布函数
			- 找取值,然后求概率
				- $XY=1\xrightarrow[]{\text{note}}(X=1,Y=1)$
					- $P\{XY=1\} = P\{X=1, Y=1\} = 2 \times \frac{1}{3} \times \frac{1}{3} = \frac{2}{9}$
				- $XY=0\xrightarrow[]{\text{note}}1-\frac29=\frac79$
...
- $\displaystyle  \lim _{x \rightarrow 0} \frac{\int_0^x t \ln (1+t \sin t) \mathrm{d} t}{1-\cos x^2} \xlongequal[]{1-\cos \Delta \sim \frac{1}{2} \Delta^2}\lim _{x \rightarrow 0} \frac{\int_0^x t \ln (1+t \sin t) \mathrm{d} t}{\frac{1}{2} x^4}$
	- $\displaystyle  \xlongequal[]{\text{note}}\lim _{x \rightarrow 0} \frac{x \ln (1+x \sin x)}{2 x^3}$
...
- $\boldsymbol{A}(x, y, z)=(x+y+z) \boldsymbol{i}+x y \boldsymbol{j}+z \boldsymbol{k}$
	- $\displaystyle \operatorname{rot} A=\left|\begin{array}{ccc}i & j & \cdot k \\ \frac{\partial}{\partial x} & \frac{\partial}{\partial y} & \frac{\partial}{\partial z} \\ P & Q & R\end{array}\right|=\left|\begin{array}{ccc}i & j & k \\ \frac{\partial}{\partial x} & \frac{\partial}{\partial y} & \frac{\partial}{\partial z} \\ x+y+z & x y & z\end{array}\right|$
		- $\displaystyle  \xlongequal[]{\text{expand first row}}0\vec{i} + (-1)(0-1)\vec{j} + (y-1)\vec{k}$
...
### (13)
-   
- 答 应填 $4+3 \lambda+2 \lambda^2+\lambda^3+\lambda^4$.解 利用行列式展开定理.$\displaystyle   D=\left|\begin{array}{cccc}\lambda  -1  0  0 \\0  \lambda  -1  0 \\0  0  \lambda  -1 \\4  3  2  \lambda+1\end{array}\right| \frac{\text{note} 4 \text{note}}{\text{expand}} 4(-1)^{4+1}\left|\begin{array}{ccc}-1  0  0 \\\lambda  -1  0 \\0  \lambda  -1\end{array}\right|+3(-1)^{4+2}\left|\begin{array}{ccc}\lambda  0  0 \\0  -1  0 \\0  \lambda  -1\end{array}\right|+$
...
\begin{tikzpicture}[scale=0.85]
  \begin{scope}
    \clip plot[domain=90:-90,samples=120,variable=\t]
      ({2*(1+cos(\t))*cos(\t)},{2*(1+cos(\t))*sin(\t)})
      -- plot[domain=-90:90,samples=100,variable=\t]
      ({2*cos(\t)},{2*sin(\t)}) -- cycle;
    \fill[blue!10] (-0.2,-2.2) rectangle (4.2,2.2);
    \foreach \s in {-3.5,-3.1,...,4.5} {
      \draw[blue!70,thin] (-0.2,\s) -- (4.3,{\s+4.5});
    }
  \end{scope}
  \draw[->] (-0.4,0) -- (4.7,0) node[right] {$x$};
  \draw[->] (0,-2.6) -- (0,2.7) node[above] {$y$};
  \draw[thick] plot[domain=-90:90,samples=100,variable=\t]
    ({2*cos(\t)},{2*sin(\t)});
  ...
  \draw (0.12,-2) -- (-0.12,-2) node[left] {$-2$};
  \node at (2.75,1.05) {$D$};
  \node[right] at (3.45,1.75) {$r=2(1+\cos\theta)$};
  \node at (0.70,1.25) {$r=2$};
  \node[below left] at (0,0) {$O$};
\end{tikzpicture}
\end{document}
```
- 本题主要考查二重积分的计算. 由于题设中区域 $D$ 的表示为极坐标形式, <br>故考虑将二重积分化为极坐标系下的二次积分后再计算. 本题需要用到华里士公式:<br>$\displaystyle I_n=\int_0^{\frac{\pi}{2}} \sin ^n x \mathrm{~d} x=\int_0^{\frac{\pi}{2}} \cos ^n x \mathrm{~d} x= \begin{cases}\frac{n-1}{n} \cdot \frac{n-3}{n-2} \cdot \cdots \cdot \frac{3}{4} \cdot \frac{1}{2} \cdot \frac{\pi}{2}, & n \text{note} \\ \frac{n-1}{n} \cdot \frac{n-3}{n-2} \cdots \cdots \cdot \frac{4}{5} \cdot \frac{2}{3}, & n \text{note} 1 \text{note}\end{cases}$
...
-  $\displaystyle  \iint_D x \, dx \, dy = \int_{-\frac{\pi}{2}}^{\frac{\pi}{2}} d\theta \int_2^{2(1+\cos\theta)} r \cos \theta \cdot r \, dr$
	- $\displaystyle  \xlongequal[]{\text{note}r\text{note}\cos\theta\text{factor}} \int_{-\frac{\pi}{2}}^{\frac{\pi}{2}} \cos \theta \, d\theta \int_2^{2(1+\cos\theta)} r^2 \, dr$
	- $\displaystyle  \xlongequal[]{\text{note}} \int_{-\frac{\pi}{2}}^{\frac{\pi}{2}} \left[ \cos \theta \cdot \frac{r^3}{3} \bigg|_2^{2(1+\cos\theta)} \right] d\theta$
	...
	- $\displaystyle \xlongequal[]{\text{perfect square}}\frac{8}{3} \int_{-\frac{\pi}{2}}^{\frac{\pi}{2}} \left(\cos^{4}\theta + 3\cos^{3}\theta + 3\cos^{2}\theta\right) d\theta$
	- $\displaystyle  \xlongequal[]{\text{note}}\frac{16}{3} \int_{0}^{\frac{\pi}{2}} \left(\cos^{4}\theta + 3\cos^{3}\theta + 3\cos^{2}\theta\right) d\theta$
	- $\displaystyle   \xlongequal{\text{note}} \frac{16}{3} \times \frac{3}{4} \times \frac{1}{2} \times \frac{\pi}{2}+16 \times \frac{2}{3}+16 \times \frac{1}{2} \times \frac{\pi}{2}$
...
- $\displaystyle  \int_0^{+\infty} y(x) \mathrm{d} x  \xlongequal[]{\text{substitute}y(x)}\int_0^{+\infty}\left(C_1 \mathrm{e}^{\lambda_1 x}+C_2 \mathrm{e}^{\lambda_2 x}\right) \mathrm{d} x$
	- $\displaystyle \xlongequal[\int e^{\lambda x} d x=\frac{e^{\lambda x}}{\lambda}]{\text{Newton-Leibniz}}\left.\left(\frac{C_1}{\lambda_1} \mathrm{e}^{\lambda_1 x}+\frac{C_2}{\lambda_2} \mathrm{e}^{\lambda_2 x}\right)\right|_0 ^{+\infty}$
...
(II) 
- $\displaystyle  y+ 2y' + ky = 0 \xrightarrow[]{\text{note}} y = -\frac{1}{k}(y'' + 2y')$
	- $\displaystyle  \xrightarrow[]{\text{note}}\int_{0}^{+\infty} y(x) \, dx = \frac{1}{k} \int_{0}^{+\infty} [y''(x) + 2y'(x)] \, dx$
...
\begin{tikzpicture}
  \begin{scope}[tdplot_main_coords,scale=2.4]
    \coordinate (O) at (0,0,0);
    \coordinate (X) at (1,0,0);
    \coordinate (Y) at (0,2,0);
    \coordinate (Z) at (0,0,1);
    \fill[blue!10] (X) -- (Y) -- (Z) -- cycle;
    \begin{scope}
      \clip (X) -- (Y) -- (Z) -- cycle;
      \foreach \s in {-1.0,-0.75,...,2.0} {
        \draw[blue!70,thin] (-0.2,\s,{1.2-0.5*\s}) -- (1.2,\s,{-0.2-0.5*\s});
      }
    \end{scope}
    \draw[thick] (X) -- (Y) -- (Z) -- cycle;
    \draw[dashed] (O) -- (X) (O) -- (Y) (O) -- (Z);
    \draw[->] (O) -- (1.35,0,0) node[below left] {$x$};
    \draw[->] (O) -- (0,2.3,0) node[below left] {$y$};
    \draw[->] (O) -- (0,0,1.35) node[above] {$z$};
    \node[above right] at (X) {$1$};
    \node[right] at (Y) {$2$};
    \node[left] at (Z) {$1$};
    \node[below left] at (O) {$O$};
    \node[anchor=west] at (-0.05,1.35,0.90) {$2x+y+2z=2$};
    \draw[thick,->] (0.25,0.55,0) -- (0.25,0.55,0.475);
  \end{scope}
  \begin{scope}[xshift=7.3cm,scale=1.8]
    \begin{scope}
      \clip (0,0) -- (1,0) -- (0,2) -- cycle;
      \fill[blue!10] (-0.1,-0.1) rectangle (1.2,2.2);
      \foreach \s in {-0.9,-0.7,...,2.0} {
        \draw[blue!70,thin] (-0.1,\s) -- (1.2,{\s+1.3});
      }
    \end{scope}
    \draw[->] (-0.1,0) -- (1.35,0) node[right] {$x$};
    \draw[->] (0,-0.1) -- (0,2.35) node[above] {$y$};
    \draw[thick] (0,0) -- (1,0) -- (0,2) -- cycle;
    \node[below] at (1,0) {$1$};
    \node[left] at (0,2) {$2$};
    \node at (0.30,0.72) {$D$};
    \node[right] at (0.58,1.16) {$2x+y=2$};
    \node[below left] at (0,0) {$O$};
  \end{scope}
\end{tikzpicture}
\end{document}
```
- 根据高斯公式$\displaystyle \begin{aligned}\iint_{\Sigma_{\text{note}}} P d y d z+Q d z d x+R d x d y=\iiint_{\Omega}\left(\frac{\partial P}{\partial x}+\frac{\partial Q}{\partial y}+\frac{\partial R}{\partial z}\right) d V\end{aligned}$
...
-  $\displaystyle  I = \iint_{\Omega}(x^{2}+1) dy dz - 2y dz dx + 3z dx dy$
	- $\displaystyle  \xlongequal[]{\text{note}}\iint_{\Omega} (2x - 2 + 3) dV= \iint_{\Omega} (2x + 1) dV$
...
- $\displaystyle \iiint_{\Omega} 2 x d V = \int_{0}^{1} 2x \, dx \int_{0}^{2-2x} dy \int_{0}^{\frac{2-2x-y}{2}} dz$
	- $\displaystyle  \xlongequal[]{\text{in}z\text{integrate}}\int_{0}^{1} 2x \left[ \int_{0}^{2-2x} \frac{2-2x-y}{2} \, dy \right] dx$
		- $\displaystyle  \xlongequal[]{\text{note}2} \int_{0}^{1} x \left[ \int_{0}^{2-2x} (2-2x-y) \, dy \right] dx$
	- $\displaystyle  \xlongequal[]{\text{in}y\text{integrate}} \int_{0}^{1} x \left[ \left( (2-2x)y - \frac{y^2}{2} \right) \Bigg|_{0}^{2-2x} \right] dx$
		- $\displaystyle  \xlongequal[]{\text{note}}\int_{0}^{1} x \left( (2-2x)^2 - \frac{(2-2x)^2}{2} \right) dx$
		- $\displaystyle  \xlongequal[]{\text{simplify}} \int_{0}^{1} x \cdot \frac{(2-2x)^2}{2} \, dx \xlongequal[]{\text{simplify}}\int_{0}^{1} x \cdot 2(1-x)^2 \, dx$
		- $\displaystyle  \xlongequal[]{\text{perfect square}}2 \int_{0}^{1} x(x^2 - 2x + 1) \, dx \xlongequal[]{\text{expand}}2 \int_{0}^{1} (x^3 - 2x^2 + x) \, dx$
	- $\displaystyle  \xlongequal[]{\text{in}x\text{integrate}} 2 \left( \frac{x^4}{4} - \frac{2x^3}{3} + \frac{x^2}{2} \right) \Bigg|_{0}^{1}$
...
- 当 $a=-2$ 时, 由于 $\displaystyle \boldsymbol{C}=\left(\begin{array}{ccc:cc}1 & -1 & -1 & 2 & 2 \\0 & 0 & 3 & -3 & -6 \\0 & 0 & -3 & 3 & 0\end{array}\right) \rightarrow\left(\begin{array}{ccc:cc}1 & -1 & -1 & 2 & 2 \\0 & 0 & 1 & -1 & 0 \\0 & 0 & 0 & 0 & 1\end{array}\right)$, 因此 $\boldsymbol{A X}=\boldsymbol{B}$ 无解.
## (21) 
(本题满分 11 分)
...
- 均匀分布的概率密度怎么求？先求面积，取面积的倒数就是概率密度
	...
	- 取面积的倒数得到概率密度∶$\displaystyle f(x,y)=\left\{ \begin{array}{cc} 3, & 0 < x < 1, x^2 < y < \sqrt{x} \\ 0, & \text{otherwise} \end{array} \right.$
...
- $\displaystyle  \xlongequal[]{\text{in}y\text{integrate}}3 \int_0^{\frac{1}{2}}\left(\sqrt{x}-x^2\right) \mathrm{d} x$
...
- $\displaystyle  =\int_0^{\frac{1}{2}} \mathrm{~d} x \int_{x^2}^x 3 \mathrm{~d} y\xlongequal[]{\text{in}y\text{integrate}}3 \int_0^{\frac{1}{2}}\left(x-x^2\right) \mathrm{d} x$
...
- (III) 求 $Z=U+X$ 的分布函数 $F(z)$
	...
	- 用另一种方式来计算二维联合概率，而不是拆开变成一维，将$\displaystyle U=\left\{\begin{array}{l}1, X \leq Y \\ 0, X>Y\end{array}\right.$代入
		- $=P(X \leq z, X>Y)+P(X \leq z-1, X \leq Y)$∶
			- 到这里题型就变了，变成了区域的二重积分，即$Z=g(x,y)$，然后$X\text{note}Y$都是连续型的
				- $z$从$-\infty{\text{note}}+\infty$移动
				- 逗号，表示的是交集
...
## (23)
 (本题满分 11 分)
设总体 $X$ 的概率密度为 $\displaystyle f(x ; \theta)=\left\{\begin{array}{ll}\frac{3 x^{2}}{\theta^{3}}, & 0<x<\theta, \\ 0, & \text{otherwise,}\end{array}\right.$ 其中 $\theta \in(0,+\infty)$ 为未知参数, $X_{1}, X_{2}, X_{3}$ 为来自总体 $X$ 的简单随机样本, 令 $T=\max \left\{X_{1}, X_{2}, X_{3}\right\}$.
(I) 求 $T$ 的概率密度;
...
- $\displaystyle f_T(z)= \begin{cases}\frac{9 z^8}{\theta^9}, & 0<z<\theta, \\ 0, & \text{otherwise.}\end{cases}$(II) $\displaystyle E T=\int_{-\infty}^{+\infty} z f_T(z) \mathrm{d} z=\int_0^\theta \frac{9 z^9}{\theta^9} \mathrm{~d} z=\frac{9}{10} \theta$, 从而 $E(a T)=\frac{9}{10} a \theta$, 由 $E(a T)=\theta$, 得 $a=\frac{10}{9}$. 所以当 $a=\frac{10}{9}$ 时, $a T$ 为 $\theta$ 的无偏估计.
