...
- 计算每个 $p_i$
	...
	- 对于 $X_2 \sim N\left(0,2^{2}\right)$，标准正态为：$\frac{x_2-0}{2}=\frac{x_2}{2} \backsim N(0\text{,}1)$
		- 标准化后有 $\displaystyle p_2=P\left\{-1 \leqslant \frac{X_2}{2} \leqslant 1\right\}=\Phi(1)-\Phi(-1)$。
	- 对于 $X_3 \sim N\left(5,3^{2}\right)$标准正态为：$\frac{x_3-5}{3} \backsim N(0,1)$
		- 标准化后有 $\displaystyle p_3=P\left\{-\frac{7}{3} \leqslant \frac{X_3-5}{3} \leqslant-1\right\}=\Phi(-1)-\Phi\left(-\frac{7}{3}\right)$。

- 画图比较 $p_1, p_2, p_3$：由对应标准区间宽度可知 $p_1>p_2>p_3$。
...
- 在 $t = \frac{\pi}{4}$ 时计算 $\frac{d^2 y}{dx^2}$
	- 代入 $t = \frac{\pi}{4}$，得 $\frac{d^2 y}{dx^2} = \frac{1}{\cos \frac{\pi}{4}}=\sqrt{2}$
## (12)
 $\displaystyle \int_{1}^{+\infty} \frac{\ln x}{(1+x)^{2}} \mathrm{~d} x=$
### (12)
-  - 将积分写成分部积分形式：$\displaystyle -\int_1^{+\infty} \ln x \, \mathrm{d}\left(\frac{1}{1+x}\right)=-\left[\left.\frac{\ln x}{1+x}\right|_1 ^{+\infty}-\int_1^{+\infty} \frac{1}{x(1+x)} \mathrm{d} x\right]$
	- 计算第一部分： $-\left.\frac{\ln x}{1+x}\right|_1^{+\infty}=0-0=0$
	- 计算第二部分：先拆成 $\displaystyle \int_1^{+\infty}\left(\frac{1}{x}-\frac{1}{x+1}\right) \mathrm{d} x$，因此
	  $\displaystyle \int_1^{+\infty} \frac{\mathrm{d} x}{x(1+x)}=\left.[\ln x-\ln (1+x)]\right|_1 ^{+\infty}=\left.\ln \frac{x}{1+x}\right|_1^{+\infty}=\ln 1-\ln \frac{1}{2}=\ln 2$
- 合并:$=-[0-(\ln 2)]=\ln 2$
## (13)
...
### (13)
- 答 应填 $-1$。
- 解：由 $a_{i j}+A_{i j}=0(i, j=1,2,3)$ 知, $\boldsymbol{A}$ 的伴随矩阵 $\boldsymbol{A}^{\cdot}$ 满足 $\boldsymbol{A}^*=-\boldsymbol{A}^{\mathrm{T}}$, 即 $\left|\boldsymbol{A}^*\right|=(-1)^3\left|\boldsymbol{A}^{\mathrm{T}}\right|=-|\boldsymbol{A}|$。再由 $\left|\boldsymbol{A}^*\right|=|\boldsymbol{A}|^{3-1}=|\boldsymbol{A}|^2$, 得 $|\boldsymbol{A}|^2+|\boldsymbol{A}|=0$, 即 $|\boldsymbol{A}|=-1$ 或 $|\boldsymbol{A}|=0$。最后,不妨设 $a_{11} \neq 0$, 由行列式的展开定理得 $|\boldsymbol{A}|=a_{11} A_{11}+a_{12} A_{12}+a_{13} A_{13}=-\left(a_{11}^2+a_{12}^2+a_{13}^2\right)<0$, 所以 $|A|=0$ 舍去,故 $|A|=-1$。
## (14)
 设随机变量 $Y$ 服从参数为 1 的指数分布, $a$ 为常数且大于零, 则 $P\{Y \leqslant a+1 \mid Y>a\}=$
### (14)
- 可联系“指数分布的无记忆性”来理解这一题。
- 解法 1：
	...
	- 使用条件概率和积分来计算:计算 $P\{Y \leqslant a+1 \mid Y>a\}$
		- 由条件概率定义，$\displaystyle P\{Y \leqslant a+1 \mid Y>a\}= \frac{P\{a<Y \leqslant a+1\}}{P\{Y>a\}}$。
			- 何处算概率，何处算积分
			- 分子为 $\displaystyle P\{a<Y \leqslant a+1\}= \int_a^{a+1} \mathrm{e}^{-x} \, \mathrm{d} x$。
			- 分母为 $\displaystyle P\{Y>a\}= \int_a^{+\infty} \mathrm{e}^{-x} \, \mathrm{d} x$。
		- 合并后可得 $\displaystyle \frac{\int_a^{a+1} \mathrm{e}^{-x} \, \mathrm{d} x}{\int_a^{+\infty} \mathrm{e}^{-x} \, \mathrm{d} x}=\frac{\mathrm{e}^{-a}-\mathrm{e}^{-a-1}}{\mathrm{e}^{-a}}=1-\frac{1}{\mathrm{e}}$。
- 解法 2：使用指数分布的无记忆性质
	- 要掌握各种分布的写法
		- 离散型的要掌握分布律
		- 连续型的要掌握概率密度怎么写
	...
	- 利用减法公式与无记忆性，$\displaystyle P\{Y \leqslant a+1\mid Y>a\}=1-P\{Y>a+1\mid Y>a\}=1-P\{Y>1\}=1-\int_1^{+\infty} \mathrm{e}^{-x} \, \mathrm{d} x=1-\frac{1}{\mathrm{e}}$。
...
- $P\{Y \leqslant a+1 \mid Y>a\}$
	- 由条件概率定义，$\displaystyle P\{Y \leqslant a+1 \mid Y>a\}= \frac{P\{a<Y \leqslant a+1\}}{P\{Y>a\}}$
		- 分子为 $\displaystyle P\{a<Y \leqslant a+1\} = \int_a^{a+1} e^{-x} \, \mathrm{d} x$
			- $Y$服从指数分布
				- 指数分布的概率密度函数为 $e^{-x}$
		- 分母为 $\displaystyle P\{Y>a\} = \int_a^{+\infty} e^{-x} \, \mathrm{d} x$
			- 同样，使用指数分布的概率密度函数
		- 化简后得 $\displaystyle \frac{\int_a^{a+1} e^{-x} \, \mathrm{d} x}{\int_a^{+\infty} e^{-x} \, \mathrm{d} x} = 1 - \frac{1}{e}$
- $P\{Y \leqslant a+1 \mid Y>a\} = 1 - \frac{1}{e}$

此外，解法 2 基于指数分布的无记忆性质提供了另一个视角：

- $P\{Y \leqslant a+1 \mid Y>a\}$
	- 由指数分布的无记忆性，它等于 $P\{Y \leqslant 1\}$
		- 直接计算得 $1 - \frac{1}{e}$
...
- 计算定积分 $\displaystyle \int_{0}^{1} \frac{f(x)}{\sqrt{x}} \, dx$，其中 $\displaystyle f(x) = \int_{1}^{x} \frac{\ln(t + 1)}{t} \, dt$
	- $\displaystyle \begin{aligned}\int_0^1 \frac{f(x)}{\sqrt{x}} d x=2 \int_0^1 f(x) d(\sqrt{x})\end{aligned}$
		- 对 $\sqrt{x}$ 做分部积分，得 $\displaystyle 2\left[\underbrace{\left.\sqrt{x} \cdot f(x)\right|_0 ^1}_{0-0=0}-\underbrace{\int_0^1 \sqrt{x} \, d\left[f(x)\right]}_{\int_0^1 \sqrt{x} \cdot \frac{\ln (x+1)}{x} \, d x}\right]$
		- 整理得 $\displaystyle -2\int_0^1 \frac{\ln (x+1)}{\sqrt{x}} \, d x$
		...
		- 分部积分：$\displaystyle =-4\left[\left.\sqrt{x} \ln (x+1)\right|_0 ^1-\int_0^1 \frac{\sqrt{x}}{x+1} \mathrm{~d} x\right]$
			...
			- 计算第二部分时令 $x=t^2$，则 $dx=2t\,dt$，于是 
			  $$
			  \int_0^1 \frac{\sqrt{x}}{x+1} \, \mathrm{d} x=\int_0^1 \frac{t}{1+t^2} \cdot 2 t \, \mathrm{d} t=2 \int_0^1 \frac{t^2}{1+t^2} \, \mathrm{d} t.
			  $$
				- 再将分子拆成 $t^2=(t^2+1)-1$，得 $\displaystyle \int_0^1 \frac{t^2}{1+t^2} \, dt=\int_0^1 1 \, dt-\int_0^1 \frac{1}{1+t^2} \, dt=1-\frac{\pi}{4}$。
...
### (16)
- ( I ) 证法 1：由题设得 $a_{2 n}=\frac{3}{(2 n) !}, a_{2 n+1}=\frac{1}{(2 n+1) !}$, 所以 $\displaystyle \sum_{n=0}^{\infty} a_n x^n$ 的收玫半径为 $+\infty$。因为 $\displaystyle S(x)=\sum_{n=0}^{\infty} a_n x^n$, 所以 $\displaystyle S^{\prime}(x)=\sum_{n=1}^{\infty} n a_n x^{n-1},\ S^{\prime \prime}(x)=\sum_{n=2}^{\infty} n(n-1) a_n x^{n-2}$。由于 $a_{n-2}-n(n-1) a_n=0$, 所以 $\displaystyle S^{\prime \prime}(x)=\sum_{n=2}^{\infty} a_{n-2} x^{n-2}=\sum_{n=0}^{\infty} a_n x^n$，故 $S^{\prime \prime}(x)-S(x)=0$。
- ( I ) 证法 2：由题设得 $a_{2 n}=\frac{3}{(2 n) !}, a_{2 n+1}=\frac{1}{(2 n+1) !}$, 所以 $\displaystyle \sum_{n=0}^{\infty} a_n x^n$ 的收玫半径为 $+\infty$。因为 $\displaystyle \mathrm{e}^x=\sum_{n=0}^{\infty} \frac{x^n}{n !},\ \mathrm{e}^{-x}=\sum_{n=0}^{\infty}(-1)^n \frac{x^n}{n !}$, 所以 $\displaystyle \mathrm{e}^x+\mathrm{e}^{-x}=2 \sum_{n=0}^{\infty} \frac{x^{2 n}}{(2 n) !},\ \mathrm{e}^x-\mathrm{e}^{-x}=2 \sum_{n=0}^{\infty} \frac{x^{2 n+1}}{(2 n+1) !}$，从而 $\displaystyle \begin{aligned} S(x) & =\sum_{n=0}^{\infty} a_n x^n=3 \sum_{n=0}^{\infty} \frac{1}{(2 n) !} x^{2 n}+\sum_{n=0}^{\infty} \frac{1}{(2 n+1) !} x^{2 n+1} \end{aligned}$
...
- 求函数 $f(x, y)$ 在驻点的二阶偏导数。
	- 由乘法法则可得 $\displaystyle A = f_{xx}^{\prime\prime}(x, y)=\mathrm{e}^{x+y}\left(y + \frac{x^3}{3} + 2x^2 + 2x\right)$
	- 同理 $\displaystyle B = f_{xy}^{\prime\prime}(x, y)=\mathrm{e}^{x+y}\left(y + \frac{x^3}{3} + x^2 + 1\right)$
	- 且 $\displaystyle C = f_{yy}^{\prime\prime}(x, y)= \mathrm{e}^{x+y}\left(y + \frac{x^3}{3} + 2\right)$
...
### (18)
- (I) 证法 1：因为 $f(x)$ 是奇函数, 所以 $f(0)=0$。又因为函数 $f(x)$ 在区间 $[0,1]$ 上可导, 且 $f(1)=1$, 所以由拉格朗日中值定理可知, 存在 $\xi \in(0,1)$, 使得 $f^{\prime}(\xi)=f(1)-f(0)=1$。证法 2：令 $F(x)=f(x)-x$, 则 $F(0)=f(0)=0,\ F(1)=f(1)-1=0$, 由罗尔定理得, 存在 $\xi \in(0,1)$, 使得 $F^{\prime}(\xi)=0$, 即 $f^{\prime}(\xi)=1$。
- (II) 证法 1：因为 $f(x)$ 是区间 $[-1,1]$ 上的奇函数, 所以 $f^{\prime}(x)$ 是 $[-1,1]$ 上的偶函数。结合 (I), 有 $f^{\prime}(-\xi)=f^{\prime}(\xi)=1$。令 $F(x)=\left[f^{\prime}(x)-1\right] \mathrm{e}^x$, 则函数 $F(x)$ 在 $[-1,1]$ 上可导, 且 $F(-\xi)=F(\xi)=0$。根据罗尔定理, 存在 $\eta \in(-\xi, \xi) \subset(-1,1)$, 使得 $F^{\prime}(\eta)=0$。又因为 $F^{\prime}(\eta)=\left[f^{\prime \prime}(\eta)+f^{\prime}(\eta)-1\right] \mathrm{e}^\eta$ 且 $\mathrm{e}^\eta \neq 0$, 所以 $f^{\prime \prime}(\eta)+f^{\prime}(\eta)=1$。
...
- $\displaystyle \begin{aligned} & L: \frac{x-1}{-1}=\frac{y}{1}=\frac{z}{1}=t \end{aligned}$
	...
	- 从而曲面 $\displaystyle \Sigma$ 的方程为 $x^2+y^2=(1-z)^2+z^2$, 
		...
		- $x^2+y^2-2\left(z-\frac{1}{2}\right)^2=\frac{1}{2}$
			- $x^2+y^2-2\left(z-\frac12\right)^2=\frac12$ 中两个正号项与一个负号项构成单叶双曲面；其最小横截面在 $z=\frac12$。

#### 第二问
- 直线 $L$ 上的 $A,B$ 两点及其绕 $z$ 轴旋转得到的曲面如下。图中截面半径严格取 $r(z)=\sqrt{2z^2-2z+1}$。
...
\begin{tikzpicture}[scale=1.15]
  \begin{scope}[tdplot_main_coords]
    ...
    \draw[->] (0,0,-0.15) -- (0,0,2.55) node[above] {$z$};

    \foreach \zz in {0,0.5,1,2} {
      \pgfmathsetmacro{\rr}{sqrt(2*\zz*\zz-2*\zz+1)}
      \draw[thin] plot[variable=\t,domain=0:360,samples=80,smooth]
        ({\rr*cos(\t)},{\rr*sin(\t)},\zz);
    }

    \foreach \ang in {110,310} {
      \draw[thick] plot[variable=\zz,domain=0:2,samples=80,smooth]
        ({sqrt(2*\zz*\zz-2*\zz+1)*cos(\ang)},
         {sqrt(2*\zz*\zz-2*\zz+1)*sin(\ang)},\zz);
    }

    \draw[very thick] (1,0,0) -- (0,1,1) -- (-1,2,2);
    \fill (1,0,0) circle (1.4pt) node[below right] {$A$};
    \fill (0,1,1) circle (1.4pt) node[right] {$B$};
    \node[left] at (0,0,0.5) {$\frac12$};
    \node[left] at (0,0,2) {$2$};
    \node at (0.9,-0.9,1.25) {$\Sigma$};
  \end{scope}
\end{tikzpicture}
\end{document}
```
...
- $\displaystyle \left\{\begin{array}{l}-x_2+a x_3=0 \\-a x_1+x_2+a x_4=1 \\x_1-x_3-x_4=1 \\x_2-a x_3=b .\end{array}\right.\text{note}$, 得
...
设随机变量 $X$ 的概率密度为 $\displaystyle f(x)=\left\{\begin{array}{ll}\frac{1}{9} x^{2}, & 0<x<3, \\ 0, & \text{otherwise.}\end{array}\right.$ 令随机变量 $\displaystyle Y= \begin{cases}2, & X \leqslant 1, \\ X, & 1<X<2, \\ 1, & X \geqslant 2 .\end{cases}$
(I) 求 $Y$ 的分布函数;
...
- 画图
	...
	\begin{tikzpicture}[scale=1]
	  ...
	  \node[above] at (0.5,2) {$Y=2$};
	  \node[above left] at (1.58,1.58) {$Y=X$};
	  \node[below] at (2.55,1) {$Y=1$};
	\end{tikzpicture}
	\end{document}
	```
- 写出 $Y$ 的分布函数 $F_Y(y)=P(Y \leq y)$
	- 分段讨论
		- 当 $y < 1$ 时
			- $F_Y(y) = 0$
		- 当 $1 \leqslant y < 2$ 时
			- 计算 $P\{Y \leqslant y\}$
				- 当 $1 \le y < 2$ 时，由反函数关系，$\displaystyle F_Y(y)=P\{Y \leqslant y\}= P\{X \geqslant 2\} + P\{1 < X \leqslant y\}=\int_2^3 \frac{x^2}{9} dx + \int_1^y \frac{x^2}{9} dx = \frac{y^3 + 18}{27}$
		- 当 $y \geqslant 2$ 时
			- $F_Y(y) = 1$
	- 得出 $F_Y(y)$ 的分段表达式
...
- 问题 II：求概率 $P\{X \leqslant Y\}$
	- 已知条件
		- 根据 $X$ 和 $Y$ 的关系
			- 由图可知事件 $\{X \le Y\}$ 等价于 $\{X<2\}$，所以 $\displaystyle P\{X \leqslant Y\}=\int_0^2 \frac{x^2}{9} \, dx =\frac{8}{27}$
...
设总体 $X$ 的概率密度为 $\displaystyle f(x ; \theta)=\left\{\begin{array}{ll}\frac{\theta^{2}}{x^{3}} \mathrm{e}^{-\frac{\theta}{x}}, & x>0, \\ 0, & \text{otherwise,}\end{array}\right.$ 其中 $\theta$ 为未知参数且大于零. $X_{1}, X_{2}, \cdots, X_{n}$ 为来自总体 $X$ 的简单随机样本.
...
- 求矩估计就是求期望:求谁的期望，就在谁的前面乘x进行积分
	- 求样本均值=也就是求总体期望
		- 由于密度只在 $(0,+\infty)$ 上非零，所以 $\displaystyle EX=\int_0^{+\infty} x \cdot \frac{\theta^2}{x^3} \mathrm{e}^{-\frac{\theta}{x}} \, \mathrm{d} x$
			- 再凑出微分 $\left(-\frac{\theta}{x}\right)^{\prime}=\frac{\theta}{x^2}$，令 $t=-\frac{\theta}{x}$，则 
			  $$
			  EX=\int_0^{+\infty} \theta \mathrm{e}^{-\frac{\theta}{x}} \, \mathrm{d}\left(-\frac{\theta}{x}\right)=\int_{-\infty}^0 \theta \mathrm{e}^t \, \mathrm{d} t=\theta.
			  $$
	- 所以 $\theta$ 的矩估计量由总体期望等于样本均值得到，即 $\hat{\theta}=\bar{X}$，其中样本均值为 $\displaystyle \bar{X}=\frac{1}{n} \sum_{i=1}^n X_i$.
(II) 
- 设 $x_{1}, x_{2}, \cdots, x_{n}$ 为样本观测值,
- 写出似然函数:$\displaystyle L(\theta)=\prod_{i=1}^{n} f(x ; \theta)=\left\{\begin{array}{cc}\underbrace{{\theta^{2 n}}.\frac1{x_{1} ^3x_{2}^3 \cdots x_{n}^3}. e^{-\theta \sum_{i=1}^{n} \frac{1}{x_{i}}}}_{\text{note}} & x_{1}, x_{2}, \cdots x_{n}>0 . \\ 0, & \text{otherwise}\end{array}\right.$
- 取对数:$\displaystyle \ln L(\theta)=\ln \theta^{2 n}+\underbrace{\ln \frac{1}{x_1^3 \cdots x_n^3}}_{\text{note}0}-\theta \sum_{i=1}^n \frac{1}{x_i}$.
	- 为什么取对数?
		- 乘法变成加法
		- 指数变成系数
		- 容易求导:$\ln x=\frac1x$
- 求导求驻点:令 $\displaystyle \frac{d \ln L(\theta)}{d \theta}=\frac{2 n}{\theta}-\sum_{i=1}^{n} \frac{1}{x_{i}}=0$, 
	- 解方程后得 $\displaystyle \theta=\frac{2 n}{\sum_{i=1}^n \frac{1}{x_i}}$
