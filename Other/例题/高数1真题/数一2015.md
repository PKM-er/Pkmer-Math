...
## (1)
 设函数 $f(x)$ 在 $(-\infty,+\infty)$ 上连续, 其 2 阶导函数 $f^{\prime \prime}(x)$ 的图形如下图所示：

```tikz
\usepackage{tikz}
\begin{document}
\begin{tikzpicture}[x=1.15cm,y=0.8cm]
  \draw[->] (-3,0) -- (2.8,0) node[right] {$x$};
  \draw[->] (0,-2.4) -- (0,3.1) node[above] {$f''(x)$};
  \draw[thick,smooth]
    (-2.7,1.6) .. controls (-2.45,0.35) and (-2.05,0) .. (-1.65,0)
    .. controls (-1.05,0) and (-0.35,0.75) .. (-0.14,2.75);
  \draw[thick,smooth]
    (0.14,-2.2) .. controls (0.28,-1.2) and (0.55,-0.25) .. (0.82,0)
    .. controls (1.25,0.75) and (1.85,1.35) .. (2.35,1.65);
  \draw (-1.65,0.10) -- (-1.65,-0.10) node[below] {$A$};
  \draw (0.82,0.10) -- (0.82,-0.10) node[below] {$B$};
  \node[below left] at (0,0) {$O$};
\end{tikzpicture}
\end{document}
```

则曲线 $y=f(x)$ 的拐点个数为 $(\quad)$(A) 0 .
( B ) 1 .
(C) 2 .
( D) 3 .
### (1)
...
- 将特解 $y^*=x \mathrm{e}^x$，求$y'\text{note}y''$代入原方程得
...
\begin{tikzpicture}[scale=3]
  \draw[->] (-0.05,0) -- (1.35,0) node[right] {$x$};
  \draw[->] (0,-0.05) -- (0,1.35) node[above] {$y$};
  \begin{scope}
    \clip plot[domain=0.38:0.5,samples=80] (\x,{1/(4*\x)})
      -- (0.707,0.707)
      -- plot[domain=0.707:0.537,samples=80] (\x,{1/(2*\x)}) -- cycle;
    \fill[blue!10] (0.25,0.25) rectangle (0.85,1.05);
    \foreach \s in {-0.2,-0.12,...,1.2} {
      \draw[blue!70,thin] (0.2,\s) -- (0.95,{\s+0.75});
    }
  \end{scope}
  \draw[thick] plot[domain=0.31:1.25,samples=100] (\x,{1/(2*\x)});
  \draw[thick] plot[domain=0.23:1.25,samples=100] (\x,{1/(4*\x)});
  \draw[thick] (0,0) -- (1.15,1.15) node[right] {$y=x$};
  \draw[thick] (0,0) -- (0.66,1.143) node[above right] {$y=\sqrt{3}x$};
  \node[right] at (0.78,0.66) {$2xy=1$};
  \node[right] at (0.90,0.30) {$4xy=1$};
  \node at (0.54,0.71) {$D$};
  \node[below left] at (0,0) {$O$};
\end{tikzpicture}
\end{document}
...
- 确定正确选项
	- 由 $P(AB) \leqslant P(A)$  和$P(AB) \leqslant P(B)$
		- 两者相加后移项，得 $\displaystyle P(AB) \leqslant \frac{P(A) + P(B)}{2}$
		- 正确选项是 (C)
## (8)
...
\begin{tikzpicture}[tdplot_main_coords,scale=2.4]
  ...
  \begin{scope}
    \clip (X) -- (Y) -- (Z) -- cycle;
    \foreach \s in {-0.8,-0.6,...,1.8} {
      \draw[blue!70,thin] (-0.2,\s,{1.2-\s}) -- (1.2,\s,{-0.2-\s});
    }
  \end{scope}
  \draw[thick] (X) -- (Y) -- (Z) -- cycle;
  \draw[dashed] (O) -- (X) (O) -- (Y) (O) -- (Z);
  \draw[->] (O) -- (1.35,0,0) node[below right] {$x$};
  \draw[->] (O) -- (0,1.35,0) node[below left] {$y$};
  \draw[->] (O) -- (0,0,1.35) node[above] {$z$};
  \node[above left] at (X) {$1$};
  \node[right] at (Y) {$1$};
  \node[left] at (Z) {$1$};
  \node[below left] at (O) {$O$};
  \node[anchor=west] at (0.20,0.58,0.78) {$x+y+z=1$};
\end{tikzpicture}
\end{document}
```
...
\begin{tikzpicture}[scale=2.2]
  \def\a{1.25}
  \begin{scope}
    \clip (0,0) -- (\a,0) -- (0,\a) -- cycle;
    \fill[blue!10] (-0.1,-0.1) rectangle (1.4,1.4);
    \foreach \s in {-0.8,-0.6,...,1.4} {
      \draw[blue!70,thin] (-0.1,\s) -- (1.4,{\s+1.5});
    }
  \end{scope}
  \draw[->] (-0.1,0) -- (1.55,0) node[right] {$x$};
  \draw[->] (0,-0.1) -- (0,1.55) node[above] {$y$};
  \draw[thick] (0,0) -- (\a,0) -- (0,\a) -- cycle;
  \node[below] at (\a,0) {$1-z$};
  \node[left] at (0,\a) {$1-z$};
  \node at (0.43,0.43) {$D_z$};
  \node[below left] at (0,0) {$O$};
\end{tikzpicture}
\end{document}
```
...
- 由分配律，$\displaystyle P\{XY - Y < 0\}=P\{(X-1)Y < 0\}$
	- 因为X-1和Y相互独立，所以：
		- 乘积为负当且仅当一正一负，所以 $\displaystyle P\{(X-1)Y < 0\}=P\{X-1 < 0, Y > 0\} + P\{X-1 > 0, Y < 0\}$
		- 又由于 $X-1$ 和 $Y$ 相互独立，故上式等于 $\displaystyle P\{X-1 < 0\} \cdot P\{Y > 0\} + P\{X-1 > 0\} \cdot P\{Y < 0\}$
...
- 由于 $f(x)$ 和 $g(x) = k x^3$ 在 $x \rightarrow 0$ 时等价，<br>比较 $f(x)$ 和 $g(x)$同次幂的系数
 ...
 - 求解 $a$, $b$, 和 $k$:
	...
	- 将$a\text{note}b$代入,从 $\frac{a}{3} - \frac{b}{6} = k$ 得到 $k = -\frac{1}{3}$。
## (16)
...
\begin{tikzpicture}[x=1.25cm,y=0.8cm]
  \def\xzero{1.5}
  \begin{scope}
    \clip (-1,0) -- (\xzero,0) -- (\xzero,3.2) -- cycle;
    \fill[blue!10] (-1.1,-0.1) rectangle (1.7,3.4);
    \foreach \s in {-1.2,-0.85,...,3.5} {
      \draw[blue!70,thin] (-1.2,\s) -- (1.8,{\s+3});
    }
  \end{scope}
  \draw[->] (-1.5,0) -- (2.6,0) node[right] {$x$};
  \draw[->] (0,-0.25) -- (0,4.6) node[above] {$y$};
  \draw[thick,smooth,domain=-1.15:2.15,samples=100,variable=\x]
    plot (\x,{8/(4-\x)}) node[above right] {$y=f(x)$};
  \draw[thick] (-1,0) -- (1.5,3.2);
  \draw[thick] (1.5,0) -- (1.5,3.2);
  \fill (1.5,3.2) circle (1.2pt);
  \draw (1.5,0.12) -- (1.5,-0.12) node[below] {$x_0$};
  \draw (0.10,2) -- (-0.10,2) node[left] {$2$};
  \node[left] at (1.5,3.2) {$f(x_0)$};
  \node[below left] at (0,0) {$O$};
\end{tikzpicture}
\end{document}
```
- (1) 写出切线方程：$\displaystyle y - f(x_0) = f'(x_0)(X - x_0)$
- (2) 计算区域面积。
...
$$
S=\frac{1}{2}\times \left|f\left(x_0\right)\right| \times \left|\frac{f\left(x_0\right)}{f^{\prime}\left(x_0\right)}\right|=\frac{1}{2} \frac{\left[f\left(x_0\right)\right]^2}{\left|f^{\prime}\left(x_0\right)\right|} = 4
$$
	- 代入 $y=f(x_0)$ 后可得 $8 y^{\prime}=y^2$。
...
- 作拉格朗日函数:$\displaystyle \begin{aligned} L(x, y, \lambda)=g(x, y)+\lambda \varphi(x, y)=(1+y)^2+ & (1+x)^2+\lambda\left(x^2+y^2+x y-3\right) \end{aligned}$
	- $\displaystyle \begin{aligned} \left\{\begin{array}{l}(1)\ L_x^{\prime}=2(1+x)+\lambda(2 x+y)=0, \\(2)\ L_y^{\prime}=2(1+y)+\lambda(2 y+x)=0, \\(3)\ L_\lambda^{\prime}=x^2+y^2+x y-3=0 .\end{array}\right. \end{aligned}$
		- $(1)-(2)$, 得
	...
	- 分情况讨论
		- (1) $x=y$ :
			- $3 x^2=3$
			- $\displaystyle \Rightarrow\left\{\begin{array} { l } { x = 1 } \\{ y = 1 }\end{array} \text{note} \left\{\begin{array}{l}x=-1 \\y=-1 .\end{array}\right.\right.$
		- (2) $\lambda=-2$ : 代入$(1)$
...
-   
#### 斯托克斯(一阶连续偏导)
- (2)看题目要求是否说明有一阶连续偏导，否则用投代积。曲线、补线与曲面的位置关系如下：
...
\begin{tikzpicture}[tdplot_main_coords,scale=1.8]
  \def\R{1.414}
  \draw[->] (0,0,0) -- (1.8,0,0) node[below right] {$x$};
  \draw[->] (0,0,0) -- (0,1.8,0) node[below left] {$y$};
  \draw[->] (0,0,0) -- (0,0,1.7) node[above] {$z$};
  \draw[dashed] plot[domain=0:360,samples=100,variable=\t]
    ({\R*cos(\t)},{\R*sin(\t)},0);
  \draw[thin] plot[domain=0:180,samples=80,variable=\t]
    ({\R*cos(\t)},0,{\R*sin(\t)});
  \fill[blue!10]
    plot[domain=90:-90,samples=100,variable=\t]
      ({cos(\t)},{\R*sin(\t)},{cos(\t)}) -- cycle;
  \foreach \s in {-1.2,-0.9,...,1.2} {
    \draw[blue!70,thin] (0,\s,0) -- ({sqrt(1-\s*\s/2)},\s,{sqrt(1-\s*\s/2)});
  }
  \draw[thick,->] plot[domain=90:0,samples=60,variable=\t]
    ({cos(\t)},{\R*sin(\t)},{cos(\t)});
  \draw[thick] plot[domain=0:-90,samples=60,variable=\t]
    ({cos(\t)},{\R*sin(\t)},{cos(\t)});
  \draw[thick,dashed,->] (0,-\R,0) -- (0,\R,0) node[midway,below right] {$L_1$};
  \node[right] at (0,\R,0) {$A$};
  \node[left] at (0,-\R,0) {$B$};
  \node at (0.48,0.10,0.62) {$\Sigma$};
  \draw[->,thick] (0.45,0,0.45) -- (0.78,0,0.12) node[above right] {$\mathbf n$};
\end{tikzpicture}
\end{document}
```
...
- 解题思路
	- 由斯托克斯公式，$\displaystyle \oint_{L+L^{\prime}} = \iint_{\Sigma} \mid \quad\mid d s$。
...
- $\displaystyle \begin{aligned} \oint_{L+L_1}(y+z) \mathrm{d} x+\left(z^2-x^2+y\right) \mathrm{d} y+x^2 y^2 \mathrm{~d} z & \end{aligned}$. 由斯托克斯公式知,
	...
	- 按第一行展开可得 $\displaystyle \iint_{\Sigma}\left[\frac{1}{\sqrt{2}}\left(2 x^2 y-2 z\right)+\left(-\frac{1}{\sqrt{2}}\right)(-2 x-1)\right] d S$
	- 提出公因子 $\displaystyle \frac{1}{\sqrt{2}}$ 后，化为 $\displaystyle \frac{1}{\sqrt{2}} \iint_{\Sigma}\left(2 x^2 y-2 z+2 x+1\right) d S$
	- 再代入 $z=x$，得 $\displaystyle \frac{1}{\sqrt{2}} \iint_{\Sigma}\left(2 x^2 y+1\right) \mathrm{d} S$
	...
	- 由于半圆面积 $\displaystyle S=\frac{1}{2} \cdot \pi \cdot(\sqrt{2})^2$，可得结果 $\displaystyle \frac{\sqrt{2}}{2} \pi$
...
- 如何写出参数方程:已知 $\displaystyle \begin{aligned} L: \left\{\begin{array}{l}z = \sqrt{2 - x^2 - y^2} \\z = x\end{array}\right. \end{aligned}$
	- 由 $z=x$ 代入得 $\displaystyle x = \sqrt{2 - x^2 - y^2}$，两边平方后有 $\displaystyle x^2 = 2 - x^2 - y^2$
	- 移项可得 $\displaystyle 2x^2 + y^2 = 2$，进一步化为椭圆方程 $\displaystyle x^2 + \frac{y^2}{2} = 1$
		- 因此可取参数方程 $\displaystyle \left\{\begin{array}{l}x = \cos t \\y = \sqrt{2} \sin t \\z = \cos t\end{array}\right.$
...
- $\displaystyle I=\int_{L}(y+z) \mathrm{d} x+\left(z^{2}-x^{2}+y\right) \mathrm{d} y+x^{2} y^{2} \mathrm{~d} z$.
	...
	- 整理后积分为 $\displaystyle \int_{\frac{\pi}{2}}^{-\frac{\pi}{2}}\left(-\sqrt{2} \sin ^2 t-\sin t \cos t+2 \sin t \cos t-2 \sin ^3 t \cos ^2 t\right) d t$
	- 再整理为 $\displaystyle \int_{\frac{\pi}{2}}^{\frac{\pi}{2}}\left(-\sqrt{2} \sin ^2 t+\sin t \cos t-2 \sin ^3 t \cos ^2 t\right) d t$
	- 由区间对称性知奇函数项积分为 $0$，再利用偶函数翻倍，得 $\displaystyle 2 \sqrt{2} \int_0^{\frac{\pi}{2}} \sin ^2 t d t$
		- 进一步计算得 $\displaystyle 2 \sqrt{2} \times \frac{1}{2} \times \frac{\pi}{2}=\frac{\sqrt{2}}{2} \pi$
		- 也可以用二倍角公式完成同样计算。
## (20)
 (本题满分 11 分)
...
- 找取值，求概率
	- 找取值:直到第 2 个大于 3 的观测值出现时停止, 记 $Y$ 为观测次数
		- Y可能为2,3,4,5,一直到n,确定$B\text{\sim}(n,p)$
...
- $Y$ 的概率分布(几何分布,不用考虑组合情况)
	...
	- 由组合数 $\displaystyle C_{k-1}^1=k-1$，得 $\displaystyle P\{Y=k\}=\frac{1}{64}(k-1)\left(\frac{7}{8}\right)^{k-2}$，其中 $k = 2, 3, \cdots$
(II) 求 $E(Y)$（$Y$ 的期望值）
- 解法 2：使用几何分布(7×2表格)
...
## (23)
 (本题满分 11 分)
设总体 $X$ 的概率密度为
- $\displaystyle f(x ; \theta)= \begin{cases}\frac{1}{1-\theta}, & \theta \leqslant x \leqslant 1, \\ 0, & \text{otherwise,}\end{cases}$
