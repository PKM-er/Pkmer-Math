...
## (1)
 设函数 $f(x)$ 在 $(-\infty,+\infty)$ 上连续, 其 2 阶导函数 $f^{\prime \prime}(x)$ 的图形 如右图所示,<br> ![|150](20241213074414.png)<br>则曲线 $y=f(x)$ 的拐点个数为 $(\quad)$(A) 0 .
( B ) 1 .
(C) 2 .
( D) 3 .
...
- $\displaystyle \begin{aligned}y =\frac{1}{2} e^{2 x}+\left(x-\frac{1}{3}\right) e^x \end{aligned}$
...
### (4)
- 答 应选(B).
- <br>![|200](20241213134707.png)
- 区域 $D$ 如图所示. 作极坐标变换, 将 $\iint_D f(x, y) \mathrm{d} x \mathrm{~d} y$ 化为二次积分.
	-  $\displaystyle \begin{aligned} 2xy = 1 \Rightarrow 2r^2\sin\theta\cos\theta = 1 \end{aligned}$
		- $\displaystyle \begin{aligned} r^2\sin2\theta = 1 \end{aligned}$
		- $\displaystyle \begin{aligned} r = \frac{1}{\sqrt{\sin2\theta}} \end{aligned}$
	- $\displaystyle \begin{aligned} 4xy = 1 \Rightarrow 4r^2\sin\theta\cos\theta = 1 \end{aligned}$
...
- 由轮换对称性知，$\iiint_{\Omega} x \mathrm{~d} x \mathrm{~d} y \mathrm{~d} z=\iiint_{\Omega} y \mathrm{~d} x \mathrm{~d} y \mathrm{~d} z=\iiint_{\Omega} z \mathrm{~d} x \mathrm{~d} y \mathrm{~d} z .$
	...
	- 其中 $D_x=\{(x, y) \mid x+y \leqslant 1-z, y \geqslant 0, z \geqslant 0\}$, 
		- 其面积为 $\frac{(1-z)^2}{2}$,<br>投影<br>![|100](20241214095651.png)
...
- $\displaystyle \begin{aligned}S=\frac{1}{2}底×高=\frac{1}{2}\left|f\left(x_0\right)\right| \cdot\left|\frac{f\left(x_0\right)}{f^{\prime}\left(x_0\right.}\right|=\frac{1}{2} \frac{\left[f\left(x_0\right)\right]^2}{f^{\prime}\left(x_0\right)} = 4\end{aligned}$
...
- 作拉格朗日函数:$\displaystyle \begin{aligned} L(x, y, \lambda)=g(x, y)+\lambda \varphi(x, y)=(1+y)^2+ & (1+x)^2+\lambda\left(x^2+y^2+x y-3\right) \end{aligned}$
	- $\displaystyle \begin{aligned} \left\{\begin{array}{l}①L_x^{\prime}=2(1+x)+\lambda(2 x+y)=0, \\②L_y^{\prime}=2(1+y)+\lambda(2 y+x)=0, \\③L_\lambda^{\prime}=x^2+y^2+x y-3=0 .\end{array}\right. \end{aligned}$
		- $(1)-(2)$, 得
	...
	- 分情况讨论
		...
		- (2) $\lambda=-2$ : 代入$(1)$
			...
			- $\displaystyle \begin{aligned} x^2+(1-x)^2+x(1-x)=3 \end{aligned}$
				- $x^2-x-2=0$
				- 得$x=-1，x=2$
	- 则$\displaystyle \begin{aligned} g(x, y)=(1+x)^2+(1+y)^2 \end{aligned}$有四个可能的极值点$\displaystyle \begin{aligned} (1，1)& (-1,-1),(2,-1),(-1,2) \end{aligned}$
...
-   
#### 斯托克斯(一阶连续偏导)
- (2)看题目要求是否说明有一阶连续偏导，否则用投代积<br>![|300](20241214171039.png)
...
- 如何写出参数方程:已知 $\displaystyle \begin{aligned} L: \left\{\begin{array}{l}z = \sqrt{2 - x^2 - y^2} \\z = x\end{array}\right. \end{aligned}$
	...
	- $\displaystyle \begin{aligned}\xrightarrow[]{移项} 2x^2 + y^2 = 2 \xrightarrow[]{化为椭圆方程} x^2 + \frac{y^2}{2} = 1 \end{aligned}$
...
- $\displaystyle I=\int_{L}(y+z) \mathrm{d} x+\left(z^{2}-x^{2}+y\right) \mathrm{d} y+x^{2} y^{2} \mathrm{~d} z$.
	...
	- $\displaystyle \begin{aligned} \xlongequal[奇函数为0]{区间对称}-\sqrt{2} \int_{\frac{\pi}{2}}^{-\frac{\pi}{2}} \sin ^2 t d t\xlongequal[]{偶函数翻倍}2 \sqrt{2} \int_0^{\frac{\pi}{2}} \sin ^2 t d t \end{aligned}$
