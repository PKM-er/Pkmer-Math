...
- - 用对tan用泰勒展开，得$\displaystyle   \tan x=x+\frac{x^3}{3}+o\left(x^3\right)$
	- $\displaystyle  \xrightarrow[]{\text{note}}x-\tan x=-\frac{x^3}{3}+o\left(x^3\right)$
## (2)
...
- 可导必然连续，连续不一定可导
	...
	- 左右导数不相等，则不可导
- 通过画图来判断函数f(x)是否是极值点
	- $\displaystyle \begin{aligned} \begin{cases}-\delta < x < 0 \text{case}, f(x) = x|x| < 0 \\0 < x < \delta \text{case}, f(x) = x\ln x < 0\end{cases} \end{aligned}$
	- 于是 $\displaystyle \begin{aligned} x \in (-\delta, \delta) \end{aligned}$ 时，有 $\displaystyle \begin{aligned} f(x) < f(0) \end{aligned}$，这是极大值：

```tikz
\usepackage{tikz}
\begin{document}
\begin{tikzpicture}[x=2cm,y=1.35cm]
  \draw[->] (-1.35,0) -- (1.15,0) node[right] {$x$};
  \draw[->] (0,-1.55) -- (0,0.55) node[above] {$y$};
  \draw[thick,smooth,domain=-1.2:0,samples=80,variable=\x]
    plot (\x,{-\x*\x}) node[below left] {$-x^2$};
  \draw[thick,smooth,domain=0.001:0.92,samples=120,variable=\x]
    plot (\x,{\x*ln(\x)}) node[below right] {$x\ln x$};
...
- 由于 $u_{n+1}^2-u_n^2=\left(u_{n+1}+u_n\right)\left(u_{n+1}-u_n\right)$ ，而 $\left\{u_n\right\}$ 有界，
	- 故存在正数 $M$ ，使得$\displaystyle \left|u_{n+1}^2-u_n^2\right| \leqslant 2 M\left|u_{n+1}-u_n\right| \xlongequal{u_n \text{note}} 2 M\left(u_{n+1}-u_n\right)$.
...
- 设 $\boldsymbol{A}$ 的特征值为 $\lambda$, 由 $\boldsymbol{A}^2+\boldsymbol{A}=2 \boldsymbol{E}$, 
	- $\displaystyle  A^2 + A - 2E = 0 \xrightarrow[]{\text{note}\alpha} (A^2 + A - 2E)\alpha = 0$
		- $\displaystyle \xrightarrow[]{E\text{note}a} A^2\alpha + A\alpha - 2\alpha = 0$
		- $\displaystyle  \xrightarrow[A^2 \alpha=\lambda^2 \alpha]{A \alpha=\lambda \alpha}\lambda^2\alpha + \lambda\alpha - 2\alpha = 0 \rightarrow (\lambda^2 + \lambda - 2)\alpha = 0$
			- $\displaystyle  \lambda^2 + \lambda - 2 = 0 \xrightarrow[]{\text{factorize}} \lambda = 1 \quad$或$\quad -2$
...
\begin{tikzpicture}[tdplot_main_coords,scale=1.7]
  \tikzset{
    planeA/.style={draw=blue!70,thick},
    planeB/.style={draw=red!70,thick,dashed},
    planeC/.style={draw=black,thick,densely dotted}
  }
  \draw[->] (-1.5,0,0) -- (1.5,0,0) node[below right] {$x$};
  \draw[->] (0,-1.5,0) -- (0,1.5,0) node[below left] {$y$};
  \draw[->] (0,0,0) -- (0,0,1.5) node[above] {$z$};
  \draw[planeA] (-1.2,-1.2,0) -- (-1.2,1.2,0) -- (1.2,1.2,0) -- (1.2,-1.2,0) -- cycle;
  \draw[planeB] (-1.2,0,0) -- (-1.2,0,1.2) -- (1.2,0,1.2) -- (1.2,0,0) -- cycle;
  \draw[planeC] (0,-1.2,0) -- (0,-1.2,1.2) -- (0,1.2,1.2) -- (0,1.2,0) -- cycle;
  \node at (0.85,0.8,0) {$\Pi_1$};
  \node at (0.8,0,0.9) {$\Pi_2$};
  \node at (0,0.75,0.9) {$\Pi_3$};
...
\begin{tikzpicture}[tdplot_main_coords,scale=1.7]
  ...
  \draw[->] (0,-1.5,0) -- (0,1.5,0) node[below left] {$y$};
  \draw[->] (0,0,0) -- (0,0,1.5) node[above] {$z$};
  \draw[planeA] (-1.2,0,0) -- (-1.2,0,1.2) -- (1.2,0,1.2) -- (1.2,0,0) -- cycle;
  \draw[planeB] (0,-1.2,0) -- (0,-1.2,1.2) -- (0,1.2,1.2) -- (0,1.2,0) -- cycle;
  \draw[planeC] (-1.2,1.2,0) -- (1.2,-1.2,0) -- (1.2,-1.2,1.2) -- (-1.2,1.2,1.2) -- cycle;
  \draw[line width=1.2pt] (0,0,0) -- (0,0,1.35);
  \node at (0.82,0,0.92) {$\Pi_1$};
  \node at (0,0.82,0.92) {$\Pi_2$};
  \node at (0.55,-0.55,0.68) {$\Pi_3$};
  \node[below left] at (0,0,0) {$O$};
\end{tikzpicture}
\end{document}
```
...
\begin{tikzpicture}[tdplot_main_coords,scale=1.7]
  ...
  \draw[->] (0,0,0) -- (0,0,1.5) node[above] {$z$};
  \draw[blue!70,thick]
    (-1.2,0,0) -- (-1.2,0,1.2) -- (1.2,0,1.2) -- (1.2,0,0) -- cycle;
  \draw[red!70,thick,dashed]
    (0,-1.2,0) -- (0,-1.2,1.2) -- (0,1.2,1.2) -- (0,1.2,0) -- cycle;
  \draw[line width=1.2pt] (0,0,0) -- (0,0,1.35);
  \node at (0.75,0,0.92) {$\Pi_1=\Pi_2$};
  \node at (0,0.78,0.92) {$\Pi_3$};
  \node[below left] at (0,0,0) {$O$};
\end{tikzpicture}
\end{document}
```
...
\begin{tikzpicture}[tdplot_main_coords,scale=1.7]
  ...
  \draw[blue!70,thick]
    (-1.2,0,0) -- (-1.2,0,1.2) -- (1.2,0,1.2) -- (1.2,0,0) -- cycle;
  \draw[blue!70,thin] (-0.9,0,0.25) -- (0.9,0,0.25);
  \draw[blue!70,thin] (-0.9,0,0.55) -- (0.9,0,0.55);
  \draw[blue!70,thin] (-0.9,0,0.85) -- (0.9,0,0.85);
  \node at (0.65,0,0.92) {$\Pi_1=\Pi_2=\Pi_3$};
  \node[below left] at (0,0,0) {$O$};
\end{tikzpicture}
\end{document}
```
...
\tdplotsetmaincoords{80}{120}
\begin{tikzpicture}[tdplot_main_coords,scale=1.7]
  \draw[->] (-0.8,0,0) -- (1.6,0,0) node[below right] {$x$};
  \draw[->] (0,-0.8,0) -- (0,1.6,0) node[below left] {$y$};
  \draw[->] (0,0,0) -- (0,0,1.5) node[above] {$z$};
  \draw[blue!70,thick]
    (0,-0.5,0) -- (0,1.3,0) -- (0,1.3,1.2) -- (0,-0.5,1.2) -- cycle;
  \draw[red!70,thick,dashed]
    (-0.5,0,0) -- (1.3,0,0) -- (1.3,0,1.2) -- (-0.5,0,1.2) -- cycle;
  \draw[black,thick,densely dotted]
    (-0.2,1.2,0) -- (1.2,-0.2,0) -- (1.2,-0.2,1.2) -- (-0.2,1.2,1.2) -- cycle;
  \draw[line width=1.1pt] (0,0,0) -- (0,0,1.25);
  \draw[line width=1.1pt] (1,0,0) -- (1,0,1.25);
  \draw[line width=1.1pt] (0,1,0) -- (0,1,1.25);
  \node at (0,0.95,0.92) {$\Pi_1$};
  \node at (0.95,0,0.92) {$\Pi_2$};
  \node at (0.70,0.30,0.63) {$\Pi_3$};
\end{tikzpicture}
\end{document}
```
...
\begin{tikzpicture}[tdplot_main_coords,scale=1.7]
  ...
  \draw[->] (0,0,0) -- (0,0,1.5) node[above] {$z$};
  \draw[blue!70,thick]
    (-1.2,-0.8,0) -- (1.2,-0.8,0) -- (1.2,-0.8,1.2) -- (-1.2,-0.8,1.2) -- cycle;
  \draw[red!70,thick,dashed]
    (-1.2,0.8,0) -- (1.2,0.8,0) -- (1.2,0.8,1.2) -- (-1.2,0.8,1.2) -- cycle;
  \draw[black,thick,densely dotted]
    (0,-1.25,0) -- (0,1.25,0) -- (0,1.25,1.2) -- (0,-1.25,1.2) -- cycle;
  \draw[line width=1.1pt] (0,-0.8,0) -- (0,-0.8,1.25);
  \draw[line width=1.1pt] (0,0.8,0) -- (0,0.8,1.25);
  \node at (0.82,-0.8,0.92) {$\Pi_1$};
  \node at (-0.82,0.8,0.95) {$\Pi_2$};
  \node at (0,-0.18,0.42) {$\Pi_3$};
\end{tikzpicture}
\end{document}
```
...
- (D)$P(A B)=P(\bar{A} \bar{B})$.
	- $\displaystyle  P(\bar{A} \bar{B})  =P(\bar{A} \cap \bar{B})\xlongequal[]{\text{note}}P(\overline{A \cup B})=1-P(A \cup B)$
...
- - 计算 $P\{|X-Y|<1\}$
	...
	- 分析 $X - Y$ 的分布，有限个正态分布的线性组合还是正态分布
		- 则$X - Y$ 遵循 $\displaystyle N(0, 2\sigma^2)$
			- $\displaystyle \xrightarrow[]{\text{standardize}}\frac{X-Y-0}{\sqrt{2}\sigma} \sim N(0, 1)$
			- 碰到非标准的正态分布习惯上将它转为标准正态
	- 概率也转换为标准正态概率
		- $\displaystyle P\{|X-Y|<1\} \xlongequal[\text{standardize}]{\frac{X-Y-\mu}{\sqrt{2} \sigma}} P\left\{\left|\frac{X-Y}{\sqrt{2} \sigma}\right|<\frac{1}{\sqrt{2} \sigma}\right\}\xlongequal[]{\text{note}} 2\Phi\left(\frac{1}{\sqrt{2} \sigma}\right) - 1$
...
\begin{tikzpicture}[x=1.3cm,y=2.4cm]
  \def\a{1.15}
  \begin{scope}
    \clip (-\a,0) -- plot[domain=-\a:\a,samples=100,variable=\x]
      (\x,{exp(-\x*\x/2)}) -- (\a,0) -- cycle;
    \fill[blue!10] (-\a,0) rectangle (\a,1.05);
    \foreach \x in {-1.05,-0.8,...,1.05} {
      \draw[blue!70,thin] (\x,0) -- (\x,{exp(-\x*\x/2)});
    }
  \end{scope}
  \draw[->] (-3,0) -- (3,0) node[right] {$u$};
  \draw[->] (0,-0.05) -- (0,1.2) node[above] {$\varphi(u)$};
  \draw[thick,smooth,domain=-2.8:2.8,samples=120,variable=\x]
    plot (\x,{exp(-\x*\x/2)});
  \draw[dashed] (-\a,0) -- (-\a,{exp(-\a*\a/2)});
  \draw[dashed] (\a,0) -- (\a,{exp(-\a*\a/2)});
  \node[below] at (-\a,0) {$-\frac{1}{\sqrt{2}\sigma}$};
  \node[below] at (\a,0) {$\frac{1}{\sqrt{2}\sigma}$};
...
- 将$\displaystyle  \frac{\partial z}{\partial x}$和$\frac{\partial z}{\partial y}$代入到$\displaystyle \frac{1}{\cos x} \cdot \frac{\partial z}{\partial x}+\frac{1}{\cos y} \cdot \frac{\partial z}{\partial y}=$
	...
	- $\displaystyle \xrightarrow[]{\text{note}}\frac{1}{\cos x} z_x+\frac{1}{\cos y} z_y=-f^{\prime}+\frac{y}{\cos x}+f^{\prime}+\frac{x}{\cos y}$<br>有时候可以省略掉自变量本体，有时候不行
...
- 原式$2 y y^{\prime}-y^{2}-2=0$
	...
	- $\displaystyle \xrightarrow[\text{note}]{\text{separate variables}} \int \frac{2y}{y^2 + 2} dy = \int dx$
...
- 原式为$\displaystyle  \ln(y^2 + 2) = x + \ln 3$，因为答案要求$\displaystyle y=$
	- $\displaystyle \xrightarrow[]{\text{note}e}e^{\ln \left(y^2+2\right)}=e^{x+\ln 3}$
	- $\displaystyle \xrightarrow[]{\text{simplify}} y^2 + 2 = 3e^x$
		- $\displaystyle  y^2 = 3e^x - 2 \xrightarrow[]{\text{note}} \quad y = \pm\sqrt{3e^x - 2}$
...
- - $\displaystyle  \cos x=\sum_{n=0}^{\infty}(-1)^n \frac{x^{2 n}}{(2 n)!}=1-\frac{x^2}{2!}+\frac{x^4}{4!}-\frac{x^6}{6!}+\cdots+(-1)^n \frac{x^{2 n}}{(2 n)!}+\cdots,-\infty<x<+\infty .$
	...
	- 所以$\displaystyle \sum_{n=0}^{\infty} \frac{(-1)^n}{(2 n) !} x^n\xlongequal[]{\text{note}}\sum_{n=0}^{\infty} \frac{(-1)^n}{(2 n) !}(\sqrt{x})^{2 n}\xlongequal[]{\text{note}}\cos \sqrt{x}(0<x<+\infty) .$
- 方法2
...
\begin{tikzpicture}[tdplot_main_coords,scale=1.55]
  \begin{scope}
    \clip plot[domain=0:360,samples=120,variable=\t]
      ({2*cos(\t)},{2*sin(\t)},0) -- cycle;
    \fill[gray!10] (-2.1,-2.1,0) rectangle (2.1,2.1,0);
    \foreach \s in {-2.8,-2.4,...,2.8} {
      \draw[gray!70,thin] (-2.2,\s,0) -- (2.2,{\s+2.2},0);
    }
  \end{scope}
  \draw[thick] plot[domain=0:360,samples=120,variable=\t]
    ({2*cos(\t)},{2*sin(\t)},0);
  \foreach \p in {0,30,...,330} {
    \draw[blue!70,thin] plot[domain=0:90,samples=45,variable=\u]
      ({2*sin(\u)*cos(\p)},{2*sin(\u)*sin(\p)},{cos(\u)});
  }
  \draw[thick] plot[domain=-90:90,samples=80,variable=\u]
    ({2*sin(\u)},0,{cos(\u)});
  \draw[->] (0,0,0) -- (2.7,0,0) node[below right] {$x$};
  \draw[->] (0,0,0) -- (0,2.7,0) node[below left] {$y$};
  \draw[->] (0,0,0) -- (0,0,1.5) node[above] {$z$};
  \node at (0.5,0.55,0.70) {$\Sigma$};
  \node at (0.7,-0.35,0) {$D_{xy}$};
...
- 本题就是转换投影后的一个平面<br>$\displaystyle  \iint_{\Sigma} P \, dy \, dz + Q \, dx \, dz + R \, dx \, dy$
	- $\displaystyle  \xlongequal[]{\text{note}n}\iint_{D} (-Pz_x - Qz_y + R) \, dx \, dy$
...
- 因此 $\displaystyle  r(A) = 2$，进而解向量的个数为 $\displaystyle  n - r(A) = 1$
	- $\displaystyle$由$\alpha_3 = -\alpha_1 + 2\alpha_2,$得$\alpha_1 - 2\alpha_2 + \alpha_3 = 0.$
		- $\displaystyle  \xrightarrow[]{\text{note}}(\alpha_1, \alpha_2, \alpha_3) \begin{pmatrix} 1 \\ -2 \\ 1 \end{pmatrix} \neq 0.$
	- $\displaystyle$故通解为$x = k(1, -2, 1)^T, k \in \mathbb{R}.$
## (14)
 设随机变量 $X$ 的概率密度为 $\displaystyle f(x)= \begin{cases}\frac{x}{2}, & 0<x<2,  \\ 0, & \text{otherwise,}\end{cases}$， $\displaystyle F(x)$为 $\displaystyle X$ 的分布函数, $E(X)$ 为$X$的数学期望, 则 $P\{F(X)>E(X)-1\}=$
### (14)
- 解
解法 1:
- 求期望
...
- $\displaystyle \begin{aligned}\xlongequal[]{\text{note}}P\left\{\frac{X^2}{4}>\frac{1}{3}, 0 \leqslant X<2\right\}+P\{X \geqslant 2\}\end{aligned}$
- $\displaystyle \begin{aligned} =P\left\{\frac{2}{\sqrt{3}}<X<2\right\}+\underbrace{\int_2^{+\infty} f(x) \mathrm{d} x}_{=0\text{note}0}\xlongequal[]{\text{note}}1-\int_0^{\frac{2}{\sqrt{3}}} \frac{x}{2} \mathrm{~d} x+0 \end{aligned}$
...
- $\displaystyle  \xlongequal[]{\text{note}}0+P\left\{0 \leqslant x<2, F(x)>\frac{1}{3}\right\}+P\{2 \leqslant x\}$
- $\displaystyle  \xlongequal[]{\text{note}F(x)=\frac{X^2}{4}}P\left\{0 \leqslant x<2, \frac{x^2}{4}>\frac{1}{3}\right\}+P\{2 \leqslant x\}$
- $\displaystyle  \xlongequal[]{\text{note}}P\left\{\frac{2}{\sqrt{3}}<x<2\right\}+P\{2 \leqslant x\}\xrightarrow[]{\text{note}}P\left\{x>\frac{2}{\sqrt{3}}\right\}$
#### 2024版
- 计算 $P\{F(X) > E(X) - 1\}$:将分布函数$F(X)$和$E(X)$代入
	- $\displaystyle P\{F(X) > E(X) - 1\} \xlongequal[\text{substitute}]{EX=\frac34}P\left\{F(X)>\frac{1}{3}\right\}$
		- $\xlongequal[]{\text{note}F(x)=\frac{X^2}{4}}P\left\{\frac{X^2}{4}>\frac{1}{3}\right\}\xlongequal[]{\text{note}}P\left\{X>\frac{2}{\sqrt{3}}\right\}$
		- $\displaystyle \xlongequal[\text{note}x\text{note}2]{\text{note},\text{note}}\int_{\frac{2}{\sqrt{3}}}^2 \frac{x}{2} dx = \frac{2}{3}$
# 三、解答题 
...
- - (I) 计算函数 $z=2+a x^2+b y^2$ 在点 $(3,4)$ 处的梯度.<br>$\left.\operatorname{grad} z(x, y)\right|_{(3,4)}=\left.\left(z_x^{\prime}, z_y^{\prime}\right)\right|_{(3,4)}=\left.(2 a x, 2 b y)\right|_{(3,4)}=(6 a, 8 b) .$
	- 由 $6 a i+8 b j$ 与 $-3 i-4 j$ 同向可得 $\frac{6 a}{-3}=\frac{8 b}{-4}>0\xrightarrow[]{\text{note}}a=b<0$
	- 方向导数最大值为 $\sqrt{(6 a)^2+(8 b)^2}=10$.
		- $(6 a)^2+(8 b)^2=100 a^2=100$
		- $\displaystyle \xrightarrow[]{\text{simplify}} a= \pm 1 \Rightarrow a=-1=b$
- 曲面 $\Sigma$ 与投影圆盘 $D_{xy}$ 如下：
...
\begin{tikzpicture}[tdplot_main_coords,scale=1.7]
  ...
  \begin{scope}
    \clip plot[domain=0:360,samples=120,variable=\t]
      ({\R*cos(\t)},{\R*sin(\t)},0) -- cycle;
    \fill[gray!10] (-1.5,-1.5,0) rectangle (1.5,1.5,0);
    \foreach \s in {-2.2,-1.9,...,2.2} {
      \draw[gray!70,thin] (-1.6,\s,0) -- (1.6,{\s+1.6},0);
    }
  \end{scope}
  \draw[thick] plot[domain=0:360,samples=120,variable=\t]
    ({\R*cos(\t)},{\R*sin(\t)},0);
  \foreach \p in {0,30,...,330} {
    \draw[blue!70,thin] plot[domain=0:\R,samples=55,variable=\r]
      ({\r*cos(\p)},{\r*sin(\p)},{2-\r*\r});
  }
  \draw[thick] plot[domain=-\R:\R,samples=90,variable=\r]
    (\r,0,{2-\r*\r});
  \draw[->] (0,0,0) -- (2.0,0,0) node[below right] {$x$};
  \draw[->] (0,0,0) -- (0,2.0,0) node[below left] {$y$};
  \draw[->] (0,0,0) -- (0,0,2.5) node[above] {$z$};
  \node at (0.48,0.42,1.25) {$\Sigma$};
  \node at (0.55,-0.32,0) {$D_{xy}$};
  \node[right] at (0,0,2) {$2$};
  \node[right] at (0.72,0.65,1.75) {$z=2-x^2-y^2$};
\end{tikzpicture}
\end{document}
```
...
- 令 $z=0$, 可得 $\displaystyle \Sigma$ 在 $x O y$ 面的投影区域为 $D_{x y}=\left\{\left\{(x, y) \mid x^2+y^2 \leqslant 2\right\}\right.$.
	- $\displaystyle  S=\iint_{\Sigma} \mathrm{d} S  =\iint_{D_{x y}} \sqrt{1+\left(z_x^{\prime}\right)^2+\left(z_y^{\prime}\right)^2} \mathrm{~d} x \mathrm{~d} y$
		...
		- $\displaystyle  \xlongequal{\text{note}} \int_0^{2 \pi} \mathrm{~d} \theta \int_0^{\sqrt{2}} \sqrt{1+4 r^2} \cdot r \mathrm{~d} r$
...
## (17)
- #### 小崔版
- 曲线及其与 $x$ 轴围成的各个区域如下（振幅按原函数精确衰减）：

```tikz
\usepackage{tikz}
\begin{document}
\begin{tikzpicture}[xscale=0.56,yscale=3.2]
  \begin{scope}
    \clip (0,0) -- plot[domain=0:3.1416,samples=80,variable=\x]
      (\x,{exp(-\x)*sin(\x r)}) -- (3.1416,0) -- cycle;
    \fill[blue!10] (0,0) rectangle (3.1416,0.4);
    \foreach \x in {0.25,0.5,...,3.0} {
      \draw[blue!70,thin] (\x,0) -- (\x,{exp(-\x)*sin(\x r)});
    }
  \end{scope}
  \foreach \x in {3.4,3.7,...,6.1} {
    \draw[blue!70,thin] (\x,0) -- (\x,{exp(-\x)*sin(\x r)});
  }
  \draw[->] (-0.2,0) -- (13.1,0) node[right] {$x$};
  \draw[->] (0,-0.08) -- (0,0.47) node[above] {$y$};
  \draw[thick,smooth,domain=0:12.5664,samples=260,variable=\x]
    plot (\x,{exp(-\x)*sin(\x r)});
  \foreach \x/\lab in {3.1416/\pi,6.2832/2\pi,9.4248/3\pi,12.5664/4\pi} {
    \draw (\x,0.012) -- (\x,-0.012) node[below] {$\lab$};
  }
  \node[above right] at (0.7,0.31) {$y=e^{-x}\sin x$};
  \node[below left] at (0,0) {$O$};
\end{tikzpicture}
\end{document}
```
- $\displaystyle \xrightarrow[\text{note}]{\text{note}}\int_0^{+\infty}\left|e^{-x} \sin x\right| d x$
	- $\displaystyle = \int_0^\pi e^{-x} \sin x d x-\int_\pi^{2 \pi} e^{-x} \sin x d x+\int_{2 \pi}^{3 \pi}-\int_{3 \pi}^{4 \pi}+\cdots$
- $\displaystyle  S=  \sum_{n=0}^{\infty}(-1)^n \underline{\int_{n \pi}^{(n+1) \pi} e^{-x} \sin x d x }\xrightarrow[]{\text{note}}\sum_{n=0}^{\infty}(-1)^n u_n$
	- $\displaystyle   \int_{n \pi}^{(n+1)\pi} e^{-x} \sin x d x=-\int_{n \pi}^{(n+1) \pi} \sin x d\left(e^{-x}\right)$
	- $\displaystyle  \xlongequal[]{\text{note}}\underbrace{-e^{-x} \sin \pi |_{n \pi}^{(\pi+1) \pi}}_{=0-0=0}+\int_{n \pi}^{(n+1) \pi} e^{-x} \cdot \cos x d x$
	- $\displaystyle  =-\int_{n \pi}^{(n+1) \pi} \cos x d\left(e^{-x}\right)$
	- $\displaystyle \xlongequal[]{\text{note}}-e^{-x} \cos x \int_{n \pi}^{(n+1) \pi}-\int_{n \pi}^{(n+1) \pi} e^{-x} \sin x d x$
...
- $\displaystyle  S  =\sum_{n=0}^{\infty} \frac{1}{2}\left[e^{-(n+1) \pi}(-1)^{2 n+2}-e^{-n \pi}(-1)^{2 n+1}\right]$
	- $\displaystyle  =\sum_{n=0}^{\infty} \frac{1}{2}\left(e^{-(n+1) \pi}+e^{-n \pi}\right)$
		...
		- 等比数列求和
	- $\displaystyle \xlongequal[]{1\text{note}}\frac{1}{2} \frac{e^{-\pi}+1}{1-e^{-\pi}}=\frac{1+e^\pi}{2\left(e^\pi-1\right)} .$
#### 郭伟版
- 计算面积 $S$。
	...
	- 将积分区间分解为无穷多个小区间。
		- 每个小区间的积分计算为 $\displaystyle \int_{k \pi}^{(k+1) \pi} \mathrm{e}^{-x} \sin x \mathrm{~d} x$
			- 整个区间的面积$\displaystyle S_n=\sum_{k=1}^{n}\int_{k \pi}^{(k+1) \pi} \mathrm{e}^{-x}|\sin x| \mathrm{d} x\xlongequal[\text{note}n\text{note}n-1]{k\text{note}1\text{note}0}\sum_{k=0}^{n-1} \int_{k \pi}^{(k+1) \pi} \mathrm{e}^{-x}|\sin x| \mathrm{d} x$
			- 计算每个小区间的积分： $\displaystyle \int_{k \pi}^{(k+1) \pi} \mathrm{e}^{-x} \sin x \mathrm{~d} x \xlongequal[d x=d t]{x=t+k \pi}\int_0^\pi \mathrm{e}^{-(t+k \pi)}|\sin (t+k \pi)| \mathrm{d} t\xlongequal[\text{note}\sin (t+k \pi)=\sin t]{\text{note}e^{-k\pi}}=\mathrm{e}^{-k \pi} \int_0^\pi \mathrm{e}^{-t} \sin t \mathrm{~d} t$
				- 其中换积分限：由 $t=x-k \pi$得：$\displaystyle \left\{\begin{array}{l}\text{note}(k+1) \pi-k \pi=\pi \\\text{note}: k \pi-k \pi=0\end{array}\right.$
			- 转换为：单独计算第二部分：同时出现e和sin，连续分布两次，以暴制暴
				- 第一次分部：$\displaystyle \left(\int_0^\pi \mathrm{e}^{-t} \sin t \mathrm{~d} t\right)=-\int_0^\pi \sin t \mathrm{~d}\left(\mathrm{e}^{-t}\right)\xlongequal[]{\text{note}}-\left(\underbrace{\left.\sin t \mathrm{e}^{-t}\right|_0 ^\pi}_{0-0=0} -\int_0^\pi\mathrm{e}^{-t} \cos t \mathrm{~d} t\right)=\int_0^\pi \mathrm{e}^{-t} \cos t \mathrm{~d} t$
				...
				- 整理头尾得：$\displaystyle \int_0^\pi e^{-t} \cdot \sin t d t\xlongequal[]{\text{note}}\mathrm{e}^{-\pi}+1-\int_0^\pi \mathrm{e}^{-t} \sin t \mathrm{~d} t .$
	...
	- 使用级数求和公式
		- $\displaystyle S_n=\frac{1}{2} \sum_{k=0}^{n-1}\left[e^{-(k+1) \pi}+e^{-k \pi}\right]\xlongequal[]{\text{note}}\frac{1}{2}\left[\underbrace{\sum_{k=1}^{n-1} e^{-(k+1) \pi}}_{\sum_{k=1}^{n} e^{-(k) \pi}}+\sum_{k=0}^{n-1} e^{-k \pi}\right]$
			- 级数配平
				- 配$\displaystyle \sum_{k=1}^{n} e^{-(k) \pi}=$
				- 配$\displaystyle \sum_{k=0}^{n-1} e^{-k \pi}\xlongequal[-e^0+e^0]{k\text{note}0\text{note}1}\sum_{k=1}^{n-1} e^{-k \pi}+1\xlongequal[\text{note}e^{-n\pi}\text{note}]{k\text{note}n-1\text{note}n}\sum_{k=1}^{n} e^{-(k) \pi}+1-e^{-n\pi}$
			- 合并配平项，得$\displaystyle =\frac{1}{2} \cdot\left[2 \sum_{k=1}^n e^{-k \pi}+1-e^{-n \pi}\right]\xlongequal[]{\text{note}}\frac{1}{2} \cdot\left[\frac{1-e^{-n \pi}}{e^\pi-1}+1-e^{-n \pi}\right]$
				- 等比求和：$\displaystyle \sum_{k=1}^n e^{-k \pi}\xlongequal[]{\frac{a_1\left(1-q^n\right)}{1-q}}\frac{e^{-\pi} \cdot\left(1-e^{-n \pi}\right)}{1-e^{-\pi}}\xlongequal[]{\text{note}e^{\pi}}\frac{1-e^{-n \pi}}{e^\pi-1}$
...
- - 证明数列 $\left\{a_{n}\right\}$ 单调递减
	...
	- $\displaystyle \begin{aligned} a_{n+1}-a_n=\int_0^1 \underbrace{(x^{n+1}-x^n)}_{\text{note}=\text{note}}\sqrt{1-x^2} d x<0 \end{aligned}$
	- $\displaystyle \begin{aligned} a_{n+1}<a_n \quad a_n \searrow \end{aligned}$，单调递减
(II) 
- $\displaystyle \begin{aligned} a_n & =\int_0^1 x^n \sqrt{1-x^2} d x \xlongequal[\text{note}]{x=\sin t}\int_0^{\frac{\pi}{2}} \sin ^n t \cdot \cos ^2 t d t\xrightarrow[]{\cos ^2 t =\left(1-\sin ^2 t\right)} \end{aligned}$
	- $\displaystyle \begin{aligned} =\int_0^{\frac{\pi}{2}} \sin ^n t d t-\int_0^{\frac{\pi}{2}} \sin ^{n+2} t d t \end{aligned}$
	- $\displaystyle \begin{aligned} \xlongequal[]{\int_0^{\frac{\pi}{2}} \sin ^n t d t\text{note}I_n}I_n-\underbrace{\frac{n+1}{n+2} I_n}_{\text{note}}=\frac{1}{n+2} I_n \end{aligned}$
		- $\displaystyle \int_0^{\frac{\pi}{2}} \sin ^{n+2} t d t=\frac{n+1}{n+2} \cdot \frac{n-1}{n} \cdots \frac{1}{2} \cdot \frac{\pi}{2}$
- $\displaystyle \begin{aligned}a_{n-2} \xlongequal[]{\text{note}a_n\text{note}}\int_0^{\frac{\pi}{2}} \sin ^{n-2} d t-\int_0^{\frac{\pi}{2}} \sin ^n t d t \quad(n \geqslant 2)\end{aligned}$
...
- 由第( I) 问可知 $\left\{a_n\right\}$ 单调递减, 故 $0<a_n<a_{n-1}<a_{n-2}$.，对分母进行放缩
	- 求 $\displaystyle \begin{aligned}\lim _{n \rightarrow \infty} \frac{a_{n}}{a_{n-1}}\xrightarrow[]{\text{note}}\end{aligned}$
...
- 考察基下坐标$\displaystyle  \gamma= (\alpha_1, \alpha_2, \alpha_3) \xi$
	-  $\displaystyle   \left(\begin{array}{l}1 \\1 \\1\end{array}\right)=\left(\begin{array}{lll}\alpha_1  \alpha_2  \alpha_3\end{array}\right)\left(\begin{array}{l}b \\c \\1\end{array}\right) \xrightarrow[]{\text{substitute}\alpha_1 \text{,} \alpha_2 \text{,}\alpha_3}\left(\begin{array}{lll}1  1  1 \\2  3  a \\1  2  3\end{array}\right)\left(\begin{array}{l}b \\c \\1\end{array}\right)=\left(\begin{array}{l}1 \\1 \\1\end{array}\right)$
	- $\displaystyle  \xrightarrow[]{\text{note}}\left\{\begin{array} { l } { (1)b + c + 1 = 1 , } \\{(2) 2 b + 3 c + a = 1 , } \\{(3) b + 2 c + 3 = 1 , }\end{array} \right.$
		- (3) 式 - (1) 式可得 $c=-2$, 
		- 再由 (1) 式可得 $b=2$, 
		- 然后代入(2) 式可得 $a=3$
	- 因此, $a=3, b=2, c=-2$.
...
- 由于 $\mathrm{R}^3$ 的维数是 3 ，故要证明 $\alpha_2, \alpha_3, \beta$ 为 $\mathrm{R}^3$ 的一个基，只需证明 $\alpha_2, \alpha_3, \beta$ 线性无关。
	...
	- $\displaystyle$计算$\left|\alpha_2, \alpha_3, \boldsymbol{\beta}\right|=\left|\begin{array}{lll}1  1  1 \\3  3  1 \\2  3  1\end{array}\right|\xlongequal[]{c2-c1}\left|\begin{array}{lll}1  0  1 \\3  0  1 \\2  11\end{array}\right|=(-1)^{3+2} \cdot 1 \cdot\left|\begin{array}{cc}1  1 \\3  1\end{array}\right|$
		- $\displaystyle  =2 \neq 0$
		- 行列式不等于0，秩为3，则线性无关
- 要计算从 $\underbrace{\alpha_2, \alpha_3, \beta}_{\text{note}}$到 $\underbrace{\alpha_1, \alpha_0, \alpha_3}_{\text{note}}$ 的过渡矩阵，即求可逆矩阵 $P$ ，使得 $\left(\boldsymbol{\alpha}_1, \boldsymbol{\alpha}_2, \boldsymbol{\alpha}_3\right)=\left(\boldsymbol{\alpha}_2, \boldsymbol{\alpha}_3, \boldsymbol{\beta}\right) P$.
- 对 $\left(\alpha_2, \alpha_3, \boldsymbol{\beta}, \alpha_1, \alpha_2, \alpha_3\right)$ 作初等行变换.
	- $\displaystyle \begin{aligned} & \left(\underbrace{\alpha_2, \alpha_3, \boldsymbol{\beta}}_{\text{note}}, \underbrace{\alpha_1, \alpha_2, \alpha_3}_{\text{note}}\right)=\left(\begin{array}{lll:lll}1 & 1 & 1 & 1 & 1 &1\\3 & 3 & 1 & 2 & 3 & 3 \\2 & 3 & 1 & 1 & 2 & 3\end{array}\right) \xrightarrow[]{\begin{aligned}& r_2-3 r_1 \\& r_3-2 r_1\end{aligned}}\left(\begin{array}{cccccc}1 & 1 & 1 & 1 & 1 & 1 \\0 & 0 & -2 & -1 & 0 & 0 \\0 & 1 & -1 & -1 & 0 & 1\end{array}\right) \end{aligned}$
	- $\displaystyle \begin{aligned} \xrightarrow[\text{note}]{2\text{,}3\text{note}}\left(\begin{array}{ccc:ccc}1 & 1 & 1 & 1 & 1 & 1 \\0 & 1 & -1 & -1 & 0 & 1 \\0 & 0 & -2 & -1 & 0 & 0\end{array}\right) \xrightarrow[]{r1-r2}\left(\begin{array}{ccc:ccc}1 & 0 & 2 & 2 & 1 & 0 \\0 & 1 & -1 & -1 & 0 & 1 \\0 & 0 & 1 & \frac{1}{2} & 0 & 0\end{array}\right)\end{aligned}$
...
- 解 (1) 由 $\displaystyle A \sim B \Rightarrow\left\{\begin{aligned}|A| & =|B| \\ \operatorname{tr}(A) & =\operatorname{tr}(B) \\ r(A) & =r(B)\end{aligned}\right.$
	...
	- 解方程组：$\displaystyle \begin{aligned} \begin{cases}\text{note}4x - 8 = -2y \\\text{note}:x - 4 = y + 1\end{cases}\end{aligned}$
		- 解得 $x=3, y=-2$.
- 第二问思想:相似矩阵的传递性
...
- $\displaystyle   P_1^{-1} A P_1=P_2^{-1} B P_2 \xrightarrow[\text{note}]{\text{note}}\quad\left(P_1 P_2^{-1}\right)^{-1} A\left(P_1 P_2^{-1}\right)=B$
...
(1)
- $Z$ 的分布函数为(三件套)
	- $\left.F_Z(z)\xlongequal[]{\text{note}}P\{Z \leqslant z\}\xlongequal[]{\text{note}Z=XY}P\{X Y \leqslant z\}\right.$
		- $\xlongequal[]{\text{note}}P(X Y \leqslant z, Y=-1)+P\{X Y \leqslant z, Y=1\}$
		- $\xlongequal[]{\text{substitute}Y\text{note}}P\{-X \leqslant z, Y=-1\}+P\{X \leqslant z, Y=1\}$
		- $\xlongequal[]{X\text{note}Y\text{note}}P\{X \geqslant-z\} P\{Y=-1\}+P\{X \leqslant z\} P\{Y=1\}$
		- $\xlongequal[]{\text{note}}p P\{X \geqslant-z\}+(1-p) P\{X \leqslant z\}$
...
- 设 $x_{1}, x_{2}, \cdots, x_{n}$ 为样本值, 求最大似然估计
	...
	- 取对数:$\displaystyle \ln L\left(\sigma^2\right)=n \ln \left(\sqrt{\frac{2}{\pi}}\right)-\underbrace{n \ln \frac{1}{\sigma}}_{\text{note}\frac{n}{2} \ln \left(\sigma^2\right)}-\frac{1}{2 \sigma^2} \sum_{i=1}^n\left(x_i-\mu\right)^2$
	...
	- 移项: $\displaystyle \xrightarrow[]{\text{note}}\frac{\sum_{i=1}^n\left(x_i-\mu\right)^2}{2\sigma^4}=\frac{n \sigma^2}{2\sigma^4}$解得 $\displaystyle \sigma^{2}=\frac{1}{n} \sum_{i=1}^{n}\left(x_{i}-\mu\right)^{2}$.
