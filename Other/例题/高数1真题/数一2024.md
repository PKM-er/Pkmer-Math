...
- 解：展开的关键在“变形”，由于<br>$\displaystyle  \ln(2+x) \xlongequal[]{\text{恒等变形}}\ln 2 \left(1 + \frac{x}{2}\right) = \ln 2 + \ln \left(1 + \frac{x}{2}\right)$
	...
	- $\displaystyle  = \sum_{n=0}^{\infty} a_n x^n \xlongequal[]{\text{拆出}1\text{到∞}} a_0 + \sum_{n=1}^{\infty} a_n x^n$
- 所以 $\displaystyle  a_0 = \ln 2$，$\displaystyle  a_n = (-1)^{n-1} \frac{1}{n \cdot 2^n}, n = 1, 2, \cdots$
	-  $\displaystyle  \xrightarrow[]{\text{将}n\text{换成}2n} \sum_{n=0}^{\infty} n a_{2n} \xlongequal[0\text{项没用}]{0\text{×}a_{2n} =0}\sum_{n=1}^{\infty} n a_{2n} = \sum_{n=1}^{\infty} n \cdot (-1)^{2n-1} \frac{1}{2n \cdot 2^{2n}}$
...
- 得出特征值0有两个方法
	...
	- $\displaystyle  A\alpha = 0$ ($\displaystyle  a \neq 0$) → $\displaystyle  a$ 是 $\displaystyle  A$ 的属于0的一个特征向量。
- 求另外两个特征值
	- $\displaystyle  \beta^T \alpha = 0$ → $\displaystyle  \alpha^T \beta = 0$ ($\displaystyle  \beta$ 是 $\displaystyle  \alpha^T x = 0$ 的解)
...
- ★正态分布的标准化: 若 $\displaystyle X \sim N\left(\mu, \sigma^{2}\right)$, 则 $\displaystyle Z=\frac{X-\mu}{\sigma} \sim N(0,1)$. 于是有
	- $X\text{≤}x$
		- $\displaystyle F(x)=P\{X \leq x\}=P\left\{\frac{X-\mu}{\sigma} \leq \frac{x-\mu}{\sigma}\right\}=\Phi\left(\frac{x-\mu}{\sigma}\right)$
			...
			- 举例2:$\displaystyle \int_{-\infty}^{+\infty} \text{φ}\left(\frac{x-2}{4}\right) d x\underbrace{=4 \int_{-\infty}^{+\infty} \frac{1}{4} \text{φ}\left(\frac{x-2}{4}\right) d x}_{F_x(x) \sim N\left(2,4^2\right)}=4$
	- $a<X \leq b$
...
- 设 $\displaystyle  Z$ 的分布函数为 $\displaystyle  F_Z(z)$，则 $\displaystyle  F_Z(z) = P\{Z \leqslant z\} = P\{\mid X-Y \mid \leqslant z\}$。
	- ① 当 $\displaystyle  z < 0$ 时$\displaystyle  \xrightarrow[\text{因此概率为零}]{\text{绝对值不可能小于一个负数}}F_Z(z) = 0$；
	- ② 当 $\displaystyle  z \geqslant 0$ 时，$\displaystyle  F_Z(z) = P\left\{|X-Y| \leq z\right\} \xlongequal[]{\text{去绝对值}}P\left\{-z \leq X-Y \leq z\right\}$
		...
		- $\displaystyle  = \iint_D \lambda^2 e^{-\lambda(x+y)} dx dy \xlongequal[]{\text{关于}y=x\text{对称}} 2 \int_0^{+\infty} \lambda e^{-\lambda y} dy \int_y^{y+z} \lambda e^{-\lambda x} dx$
			- $\displaystyle \xlongequal[]{e^{-\lambda x} \rightarrow-\frac{1}{\text{λ}} e^{-\lambda x}}-\left.2 \lambda \int_0^{+\infty} e^{-\lambda y} \cdot e^{-\lambda x}\right|_{x=y} ^{x=y+z} d y$
			...
			- $\displaystyle  \xlongequal[]{\text{提出}z}-2 \lambda\left(e^{-\lambda z}-1\right) \int_0^{+\infty} e^{-2\lambda y} d y\xlongequal[]{\text{积分}-2\text{λ}e^{-2\lambda y} }\left.\left(e^{-\lambda z}-1\right) \cdot e^{-2 \lambda y}\right|_0 ^{+\infty}$
			- $\displaystyle  =\left(e^{-\lambda z}-1\right) \cdot(-1)=1-e^{-\lambda z} .$
			- <br>![|200](/_assets_/Images/20241205164125.png)
- $\displaystyle F_z(z)=\left\{\begin{array}{l}1-e^{-\lambda z}, \quad z\text{≥}0 \\ 0,z\text{≤}0\end{array} \Rightarrow f_z(z)=\left\{\begin{array}{cc}\lambda e^{-\lambda z}, & z>0 \\ 0, & \text{其他}\end{array}\right.\right.$
- 从而选D
### (11)
...
- $\displaystyle  (\alpha^T A \beta)^2 \leq \alpha^T A \alpha \cdot \beta^T A \beta \xrightarrow[]{\text{分别计算三者的行列式}}$
	- $\displaystyle  \alpha^T A \beta \xlongequal[]{\text{代入题中给出的行列式}} (x_1, x_2) \begin{pmatrix}a+1  a \\a  a\end{pmatrix}\begin{pmatrix}y_1 \\y_2\end{pmatrix}$
		...
		- $\displaystyle  \xlongequal[]{A\text{的行乘}B\text{的列}} [a x_1 + a x_2, \, a x_1 + a x_2] \begin{pmatrix}y_1 \\y_2\end{pmatrix}+ (x_1, 0) \begin{pmatrix}y_1 \\y_2\end{pmatrix}$
		- $\displaystyle  \xlongequal[]{A\text{的行×}B\text{的列}}a(x_1 + x_2) \cdot (y_1 + y_2) + x_1 y_1$
...
- 分别写出 $\displaystyle  f(x)$ 在 $\displaystyle  x = 0$ 与 $\displaystyle  x = 1$ 处的一阶泰勒公式
	- $\displaystyle  f(x) = f(0) + f'(0)x + \frac{f''(\xi_1)}{2}x^2, \quad \xi_1 \in (0, x) \xrightarrow[]{\text{×}2}$
	- $\displaystyle  f(x) = f(1) + f'(1)(x-1) + \frac{f''(\xi_2)}{2}(x-1)^2, \quad \xi_2 \in (x, 1) \xrightarrow[]{\text{×}(x-1)}$
- (2) 式 $\times 2$ - (1) 式 $\text{×}(x-1)$ ，结合 $f^{\prime}(0)=f^{\prime}(1)$ 可得
...
- 求每个特征值对应的特征向量:$|A-\text{λ} \cdot E| x=0$
