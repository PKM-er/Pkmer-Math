...
\begin{tikzpicture}
    ...
    \begin{scope}[tdplot_main_coords]
        ...
        % 绘制灰色圆环（投影在XY平面）
        \fill[gray!50,opacity=0.7] (0,0,0) circle (2);
        \fill[white] (0,0,0) circle (1);

        % 绘制蓝色顶面（正确贴合锥面）
        \fill[blue!50,opacity=0.5] 
            plot[domain=0:360,variable=\t] ({2*cos(\t)}, {2*sin(\t)}, {2})
            -- cycle;
        ...
        \node[right] at (0,0,2) {$\Sigma$};
        \node[right] at (1,0,0) {$1$};
        \node[right] at (2,0,0) {$2$};

        % 法向量
        \draw[->,thick] (1,1,1.5) -- (1.5,1.5,2) node[above] {$\mathbf{n}$};
