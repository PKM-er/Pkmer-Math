...
author:
  - PaulSun
---
考虑[[平稳随机过程的概念|宽平稳随机过程]] $X\left(t\right)$ 的 Fourier 展开
...
\end{aligned}\tag1$$
称 $S_X\left(\omega\right)$ 为功率谱密度。Bochner 定理是指：一个函数是正定函数，当且仅当它是某个非负谱密度函数的 Fourier 变换。从功率谱密度函数不难看出其恒正，那么相关函数是正定函数。

> [!theorem] Wiener-Khinchine 定理
> 任意一个宽平稳随机过程的功率谱密度是其[[相关函数]]的 Fourier 变换。
...
考察功率谱密度函数是否具有线性性。假设 $\alpha\in\mathbb R$
$$\begin{aligned}
S_{\alpha X}\left(\omega\right) & = \int^{+\infty}_{-\infty} R_{\alpha X}\left(\tau\right)\exp\left\{-j\omega \tau\right\}\,\mathrm d\tau
\\\\ & = \int^{+\infty}_{-\infty} \mathrm E\left(\alpha X\left(t\right)\cdot\alpha X\left(s\right)\right)\exp\left\{-j\omega \tau\right\}\,\mathrm d\tau
\\\\ & = \int^{+\infty}_{-\infty} \left|\alpha\right|^2\,\mathrm E\left(X\left(t\right)\cdot X\left(s\right)\right)\exp\left\{-j\omega \tau\right\}\,\mathrm d\tau
