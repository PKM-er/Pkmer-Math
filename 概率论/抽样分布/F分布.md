...
F分布是统计学中用来比较两个样本方差是否显著不同的一种概率分布，通常在方差分析（ANOVA）及回归分析等领域中广泛应用。这个分布由统计学家罗纳德·费希尔（Ronald Fisher）首先引入。它可由两个相互独立的[[卡方分布|卡方变量]]各自除以其自由度后的比值构造。
...
1. **方差齐性检验**：比较两个或多个样本的方差是否有统计学上的显著差异，即[[F检验]]。
2. **方差分析（ANOVA）**：用来分析多个样本均值之间的差异是否显著（见[[单因素试验的方差分析]]）。
3. **回归分析**：测试回归模型中某些自变量的影响是否显著。

## 性质

- 当 $d_1$ 和 $d_2$ 足够大时，F分布会逐渐接近正态分布。
- F分布是非对称的，右偏，其形状由自由度 $d_1$ 和 $d_2$ 控制。
