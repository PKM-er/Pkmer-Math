...
---
## 简介
$t$ 分布广泛应用于小样本数据分析中的假设检验和置信区间的建立，其构造依赖于[[标准正态分布]]与[[卡方分布]]。
...
## 构造
$t$ 分布可以通过[[标准正态分布]]和[[卡方分布]]构造。当 $X \sim N(0,1)$ 与 $Y \sim \chi^2(k)$ 相互独立时，$Z = \dfrac{X}{\sqrt{Y/k}}$ 服从自由度为 $k$ 的 $t$ 分布。
...
$t$ 分布是处理小样本统计推断中不可或缺的工具，尤其在数据量不足以准确估计标准差时，它提供了一个有效的解决方案。在应用中，它是[[t检验]]与均值[[区间估计]]的理论基础。