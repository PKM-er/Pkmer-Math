...
dlink:
  - "[[---方差分析及回归---]]"
---
一元线性回归是一种统计方法，用于分析两个连续变量之间的线性关系，是[[多元线性回归]]在单一自变量时的特例。它通过拟合一条直线来最小化预测值与实际值之间的误差。具体来说，一元线性回归模型可以描述如下：
...
$R^2$ 取值范围为0到1，值越接近1，表示模型对数据的解释能力越强；在一元线性回归中 $R^2$ 恰为[[相关系数]]的平方 $r^2$。
...
- 检验 $\beta_1$ 是否为零（即自变量 $x$ 对因变量 $y$ 是否有显著影响）。通常使用[[t检验]]。
